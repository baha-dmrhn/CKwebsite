...
let currentCurrency = localStorage.getItem('ck-market-currency') || 'TRY';
let priceChartMode = localStorage.getItem('ck-market-chart-mode') || 'full';
const SUITE_FONT = 'DM Sans, Inter, system-ui, sans-serif';
const API_BASE_URL = String(window.CK_CONFIG?.apiBaseUrl || '').replace(/\/$/, '');
const DATA_CACHE_PREFIX='ck-market-v6:';
...
function ensureChartEngine(){if(window.ApexCharts)return Promise.resolve();if(chartEngineLoadPromise)return chartEngineLoadPromise;const existing=document.querySelector('script[data-chart-engine]')?.getAttribute('src');const candidates=[...new Set([existing,'/piyasa-charts.js?v=20','piyasa-charts.js?v=20'].filter(Boolean))];chartEngineLoadPromise=(async()=>{for(const src of candidates){try{await loadChartEngineScript(src);if(window.ApexCharts)return}catch{}}chartEngineLoadPromise=null;throw new Error('Grafik motoru yüklenemedi.')})();return chartEngineLoadPromise}
...
function readMarketUrlState(){const params=new URLSearchParams(window.location.search);const currency=params.get('currency');const view=params.get('view');if(currency&&CURRENCIES[currency]){currentCurrency=currency;localStorage.setItem('ck-market-currency',currency)}if(view&&['full','simple','ptf'].includes(view)){priceChartMode=view;localStorage.setItem('ck-market-chart-mode',view)}return{date:params.get('date')||'',compare:params.get('compare')||'',compareDate:params.get('compareDate')||'',currency:currency||'',view:view||''}}
...
function selectedTheme(){return localStorage.getItem('ck-theme')||(systemDark.matches?'dark':'light')}
...
const desktopSidebar=window.matchMedia('(min-width:821px)'),desktopLock=window.matchMedia('(min-width:1025px) and (hover:hover) and (pointer:fine)'),sidebarStorageKey='ck-sidebar-collapsed',sidebarPinnedStorageKey='ck-sidebar-pinned';
...
document.querySelectorAll('[data-currency]').forEach(button=>button.addEventListener('click',()=>{if(button.disabled||button.dataset.currency===currentCurrency)return;currentCurrency=button.dataset.currency;localStorage.setItem('ck-market-currency',currentCurrency);syncMarketUrl();if(currentRows.length)renderDashboard(currentData,comparisonData);else updateCurrencyUi(currentData.currencyInfo);if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)}));
document.querySelectorAll('[data-chart-mode]').forEach(button=>button.addEventListener('click',()=>{if(button.dataset.chartMode===priceChartMode)return;priceChartMode=button.dataset.chartMode;localStorage.setItem('ck-market-chart-mode',priceChartMode);syncMarketUrl();updateChartModeUi();if(currentRows.length)renderMarketCharts();updateCurrencyUi(currentData.currencyInfo)}));
$('theme-toggle').onclick=()=>{const theme=document.documentElement.dataset.theme==='dark'?'light':'dark';localStorage.setItem('ck-theme',theme);applyTheme(theme);if(currentRows.length)renderMarketCharts();if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)};
applyTheme(selectedTheme());
systemDark.addEventListener('change',()=>{if(!localStorage.getItem('ck-theme')){applyTheme(selectedTheme());if(currentRows.length)renderMarketCharts();if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)}});
window.addEventListener('ck:themechange',event=>{const theme=event.detail?.theme||selectedTheme();applyTheme(theme);if(currentRows.length)renderMarketCharts();if(nextDayPtfData.date)renderNextDayPtf(nextDayPtfData)});
