...
const STATIC_ASSETS = [
  ...
  "/suite-loading.js",
  "/panel-hazirlaniyor",
  "/panel-loading.js?v=6",
  "/manifest.webmanifest",
  "/suite-assets/ck-logo.png?v=6",
  "/suite-assets/ck-logo-white.png?v=1",
  "/suite-assets/ck-mark.png?v=3",
  "/suite-assets/icon-192.png?v=7",
  "/suite-assets/icon-512.png?v=7",
  "/suite-assets/apple-touch-icon.png?v=7",
];
...
function shouldUseLoadingFallback(url) {
  return !url.searchParams.has("ck_ready");
}

function isCKPanelResponse(response) {
...
self.addEventListener("fetch", (event) => {
  ...
  if (event.request.mode === "navigate") {
    ...
    const network = fetch(event.request)
      .then((response) => {
        if (useLoadingFallback && !isCKPanelResponse(response)) {
