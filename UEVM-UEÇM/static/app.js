...
const DASHBOARD_CACHE_KEY = "ck-production-dashboard-v2";
...
function publishConnectionState(state = "live", detail = "") {
  ...
  window.dispatchEvent(
    new CustomEvent("ck:connectionstate", {
...
function render(data, requestedPeriod = null) {
  ...
  const normalizedGroups = normalizeGroupShares(data.groups);
  renderGroups(normalizedGroups);
  renderDonut(normalizedGroups);
  renderTable(data.sources);
  renderProductionInsight({...data, groups: normalizedGroups});
  window.CKTracking?.publish({
    module: "uretim",
...
function renderTrend(rawSeries) {
  ...
  const width = Math.max(Math.round(elements.trend.clientWidth - 48), 720);
  const height = Math.max(Math.round(elements.trend.clientHeight - 30), 390);
  ...
  const xTickCount = width >= 1400 ? 5 : width >= 900 ? 4 : 3;
  const xTickIndexes = [...new Set(
    Array.from({length: xTickCount}, (_, index) =>
      Math.round(((series.length - 1) * index) / Math.max(xTickCount - 1, 1)),
    ),
  )];
  ...
  elements.trend.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${activeSeries.map((definition) => definition.label).join(", ")} saatlik çizgi grafiği">
      <defs>
...
function normalizeGroupShares(groups) {
  const items = Array.isArray(groups) ? groups : [];
  const total = items.reduce(
    (sum, group) => sum + (Number.isFinite(Number(group.value)) ? Number(group.value) : 0),
    0,
  );
  return items.map((group) => ({
    ...group,
    share: total ? (Number(group.value || 0) / total) * 100 : null,
  }));
}
...
async function downloadReport(range, button, format, idleLabel, successMessage) {
  ...
  try {
    ...
    const filename =
      filenameMatch?.[1] ||
      `ck-uretim-epias-${range.start}-${range.end}.${selectedFormat}`;
