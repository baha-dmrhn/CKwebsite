"""Core EPİAŞ UEVM / UEÇM services used by the unified CK Enerji site.

The module intentionally uses only Python's standard library and is imported
by ``CK-ENERJI-WEBSITE/app.py``. EPİAŞ passwords are exchanged for temporary
...
def _json_cache_enabled() -> bool:
    return os.getenv("CK_EPIAS_JSON_CACHE_ENABLED", "true").strip().lower() not in {
        "0",
        "false",
        "no",
    }
...
def _force_refresh_min_seconds() -> int:
    return _safe_cache_seconds_env(
        "CK_EPIAS_FORCE_REFRESH_MIN_SECONDS",
        DEFAULT_EPIAS_FORCE_REFRESH_MIN_SECONDS,
    )


def _json_cache_dir() -> Path:
    raw = os.getenv("CK_EPIAS_JSON_CACHE_DIR")
    if raw:
        ...
        self.ttl_seconds = int(
            ...
            * (
                ...
                if ttl_minutes is not None
                    ...
                    os.getenv(
                        "CK_URETIM_SESSION_MINUTES",
                        str(DEFAULT_AUTH_SESSION_MINUTES),
                    )
                )
            )
        )
        self.cookie_secure = os.getenv("CK_URETIM_COOKIE_SECURE", "auto").lower()
        ...
        with self._token_lock:
            ...
            status, raw = self._request(
                ...
                headers={
                    ...
                    "Accept": "text/plain",
                    "User-Agent": "CK-Uretim-Epias-Dashboard/1.0",
                },
            )
...
def build_xlsx(dashboard: dict[str, Any]) -> bytes:
    ...
    summary_rows: list[list[tuple[Any, int]]] = [
        [("CK Üretim — EPİAŞ UEVM / UEÇM Raporu", 4), (None, 0)],
...
class DashboardService:
    def __init__(self, *, cache_ttl_seconds: float | None = None) -> None:
        configured_ttl = (
            ...
            else float(os.getenv("CK_URETIM_DATA_CACHE_SECONDS", "300"))
        )
