import importlib.util
import http.client
import gzip
import json
import io
import os
import sys
import threading
import unittest
import urllib.request
import zipfile
from datetime import date, datetime, timedelta
from http.server import ThreadingHTTPServer
from pathlib import Path
from unittest import mock


ROOT = Path(__file__).resolve().parents[1]
os.environ.setdefault("CK_FORECAST_LEDGER_ENABLED", "false")
os.environ.setdefault("CK_BARAJ_REFRESH_ON_ACCESS", "false")
SPEC = importlib.util.spec_from_file_location("ck_suite_test_app", ROOT / "app.py")
APP = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = APP
SPEC.loader.exec_module(APP)


class SuiteHelpersTests(unittest.TestCase):
    def test_tv_preferences_are_isolated_per_user_and_keep_profiles(self):
        stored_documents = {}
        store = APP.TvSettingsStore(ROOT / ".test-tv-settings.json")
        store._read = lambda: dict(stored_documents)
        store._write = lambda value: stored_documents.update(value)
        alice = store.save(
                "alice@ck.local",
                {
                    "activeProfile": "Piyasa",
                    "profiles": {
                        "Operasyon": {
                            "mode": "rotate",
                            "slides": ["overview", "production"],
                        },
                        "Piyasa": {
                            "mode": "fixed",
                            "slides": ["market"],
                            "fixedSlide": "market",
                        },
                    },
                },
        )
        bob = store.get("bob@ck.local")
        alice_reloaded = store.get("alice@ck.local")

        self.assertEqual(alice["preferences"]["activeProfile"], "Piyasa")
        self.assertEqual(alice_reloaded["settings"]["fixedSlide"], "market")
        self.assertFalse(bob["stored"])
        self.assertEqual(bob["preferences"]["activeProfile"], "Varsayılan")
        self.assertIn("currency", bob["settings"]["slides"])
        self.assertNotEqual(alice["userKey"], bob["userKey"])

    def test_investing_fx_quotes_are_parsed_and_cached(self):
        calls = []

        def opener(request, timeout):
            calls.append((request.full_url, timeout))
            if request.full_url.endswith("today.xml"):
                return b'''<Tarih_Date Date="13.08.2026">
                  <Currency CurrencyCode="USD"><ForexBuying>40.1000</ForexBuying><ForexSelling>40.2000</ForexSelling></Currency>
                  <Currency CurrencyCode="EUR"><ForexBuying>46.5000</ForexBuying><ForexSelling>46.7000</ForexSelling></Currency>
                  <Currency CurrencyCode="GBP"><ForexBuying>50.0000</ForexBuying><ForexSelling>51.0000</ForexSelling></Currency>
                </Tarih_Date>'''
            is_brent = "brent-oil" in request.full_url
            is_usd = "usd-try" in request.full_url
            is_eur = "eur-try" in request.full_url
            price = "47.7518" if is_usd else "55,0933" if is_eur else "88.91" if is_brent else "60,855"
            change = "+0.0271" if is_usd else "-0,0412" if is_eur else "-0.07" if is_brent else "+2,120"
            percent = "(+0.06%)" if is_usd else "(-0,07%)" if is_eur else "(-0.08%)" if is_brent else "(+3,61%)"
            return (
                '<div data-test="instrument-price-last">' + price + "</div>"
                '<span data-test="instrument-price-change">' + change + "</span>"
                '<span data-test="instrument-price-change-percent">' + percent + "</span>"
                '<time data-test="trading-time-label">Canlı · 14:32:10</time>'
            ).encode("utf-8")

        service = APP.InvestingFxService(opener=opener, cache_seconds=300)
        first = service.dashboard()
        second = service.dashboard()

        self.assertEqual(first["availablePairs"], 2)
        self.assertEqual(first["availableCommodities"], 2)
        self.assertEqual(first["availableInvestingPairs"], 2)
        self.assertAlmostEqual(first["pairs"][0]["selling"], 40.2)
        self.assertAlmostEqual(first["pairs"][1]["selling"], 46.7)
        self.assertEqual(first["commodities"][0]["direction"], "down")
        self.assertEqual(first["commodities"][1]["direction"], "up")
        self.assertEqual({pair["base"] for pair in first["pairs"]}, {"USD", "EUR"})
        self.assertTrue(second["cached"])
        self.assertEqual(len(calls), 5)

    def test_android_viewport_is_forced_to_device_width(self):
        existing_viewport = (
            '<html><head><meta name="viewport" content="width=980"></head>'
            "<body></body></html>"
        )
        missing_viewport = "<html><head></head><body></body></html>"

        rewritten_existing = APP._ensure_android_viewport(existing_viewport)
        rewritten_missing = APP._ensure_android_viewport(missing_viewport)

        self.assertIn(APP.ANDROID_VIEWPORT_META, rewritten_existing)
        self.assertNotIn("width=980", rewritten_existing)
        self.assertIn(APP.ANDROID_VIEWPORT_META, rewritten_missing)

    def test_epias_client_is_reused_for_the_complete_web_session(self):
        token = APP.AUTH.create_session(
            "cache-test@ck.local",
            "TGT-cache-test",
        )
        try:
            session = APP.AUTH.get_session(token)
            first = APP._epias_client_for_session(token, session)
            second = APP._epias_client_for_session(token, session)

            self.assertIs(first, second)
            self.assertEqual(first.get_tgt(), "TGT-cache-test")
            self.assertEqual(first.username, "")
            self.assertEqual(first.password, "")

            APP._drop_epias_session_client(token)
            third = APP._epias_client_for_session(token, session)
            self.assertIsNot(first, third)
        finally:
            APP._drop_epias_session_client(token)
            APP.AUTH.revoke(token)

    def test_turkey_calendar_marks_holidays_eves_and_bridge_days(self):
        holiday = APP._turkey_calendar_context(date(2026, 7, 15))
        religious_eve = APP._turkey_calendar_context(date(2026, 5, 26))
        bridge_day = APP._turkey_calendar_context(date(2026, 4, 24))

        self.assertTrue(holiday["isHoliday"])
        self.assertEqual(holiday["dayType"], "national_holiday")
        self.assertTrue(religious_eve["isEve"])
        self.assertEqual(religious_eve["holidayKey"], "sacrifice-eve")
        self.assertTrue(bridge_day["isBridgeDay"])

    def test_system_direction_always_publishes_best_direction(self):
        low_confidence = APP._system_direction_decision(
            "deficit",
            44,
            {"deficit": 54, "surplus": 38, "balanced": 8},
        )
        narrow_margin = APP._system_direction_decision(
            "surplus",
            62,
            {"deficit": 35, "surplus": 39, "balanced": 26},
        )
        confident = APP._system_direction_decision(
            "surplus",
            68,
            {"deficit": 20, "surplus": 68, "balanced": 12},
        )

        self.assertEqual(low_confidence["category"], "deficit")
        self.assertFalse(low_confidence["abstained"])
        self.assertTrue(low_confidence["lowConfidence"])
        self.assertEqual(narrow_margin["category"], "surplus")
        self.assertFalse(narrow_margin["abstained"])
        self.assertTrue(narrow_margin["lowConfidence"])
        self.assertEqual(confident["category"], "surplus")
        self.assertFalse(confident["abstained"])
        self.assertFalse(confident["lowConfidence"])

    def test_system_direction_quality_contains_daily_trend_and_coverage(self):
        target_day = date(2026, 7, 29)
        records = [
            {
                "sampleDay": date(2026, 7, 27),
                "date": "2026-07-27",
                "hour": 0,
                "actual": "deficit",
                "correct": True,
                "decided": True,
                "confidence": 70,
            },
            {
                "sampleDay": date(2026, 7, 27),
                "date": "2026-07-27",
                "hour": 1,
                "actual": "surplus",
                "correct": False,
                "decided": False,
                "confidence": 45,
            },
            {
                "sampleDay": date(2026, 7, 28),
                "date": "2026-07-28",
                "hour": 0,
                "actual": "deficit",
                "correct": False,
                "decided": True,
                "confidence": 64,
            },
        ]

        quality = APP._summarize_system_direction_quality(
            records,
            target_day,
        )

        self.assertEqual(len(quality["daily"]), 2)
        self.assertEqual(quality["windows"]["lastDay"]["date"], "2026-07-28")
        self.assertEqual(quality["windows"]["last7"]["coverage"], 66.7)
        self.assertEqual(quality["windows"]["last7"]["abstainedHours"], 1)

    def test_holiday_calendar_matches_moving_religious_holiday(self):
        target = date(2026, 3, 20)
        matching = APP._matching_holiday_date(target, 2025)
        self.assertEqual(matching, date(2025, 3, 30))

        same_holiday_weight = APP._system_direction_sample_weight(
            target,
            matching,
            {"holiday_calendar"},
        )
        regular_weight = APP._system_direction_sample_weight(
            target,
            date(2025, 3, 31),
            {"calendar_yearly"},
        )
        self.assertGreater(same_holiday_weight, regular_weight)

    def test_system_direction_ledger_keeps_first_published_hour(self):
        original_enabled = APP.SYSTEM_DIRECTION_LEDGER_ENABLED
        original_path = APP.SYSTEM_DIRECTION_LEDGER_PATH
        ledger_path = ROOT / "tests" / ".system-direction-ledger-test.sqlite3"
        try:
            ledger_path.unlink(missing_ok=True)
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = True
            APP.SYSTEM_DIRECTION_LEDGER_PATH = ledger_path
            payload = {
                "targetDate": "2026-07-30",
                "forecastMode": "day_ahead",
                "generatedAt": "2026-07-29T15:00:00Z",
                "calendarContext": {"label": "Normal iş günü"},
                "forecastRows": [
                    {
                        "hour": 0,
                        "category": "deficit",
                        "label": "Enerji Açığı",
                        "confidence": 72.0,
                        "probabilities": {
                            "deficit": 72,
                            "surplus": 20,
                            "balanced": 8,
                        },
                        "observed": False,
                    }
                ],
            }
            first = APP._system_direction_ledger_store(payload)
            payload["forecastRows"][0]["category"] = "surplus"
            payload["forecastRows"][0]["label"] = "Enerji Fazlası"
            second = APP._system_direction_ledger_store(payload)
            stored = APP._system_direction_ledger_read(
                date(2026, 7, 30)
            )

            self.assertEqual(first["insertedRows"], 1)
            self.assertEqual(second["insertedRows"], 0)
            self.assertEqual(stored["rowCount"], 1)
            self.assertEqual(stored["rows"][0]["category"], "deficit")
        finally:
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = original_enabled
            APP.SYSTEM_DIRECTION_LEDGER_PATH = original_path
            ledger_path.unlink(missing_ok=True)

    def test_forecast_and_validation_share_locked_forecast_rows(self):
        original_enabled = APP.SYSTEM_DIRECTION_LEDGER_ENABLED
        original_path = APP.SYSTEM_DIRECTION_LEDGER_PATH
        ledger_path = (
            ROOT / "tests" / ".system-direction-shared-ledger-test.sqlite3"
        )
        try:
            ledger_path.unlink(missing_ok=True)
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = True
            APP.SYSTEM_DIRECTION_LEDGER_PATH = ledger_path
            locked_payload = {
                "targetDate": "2026-07-29",
                "forecastMode": "intraday",
                "generatedAt": "2026-07-29T06:00:00Z",
                "calendarContext": {"label": "Normal iş günü"},
                "forecastRows": [
                    {
                        "hour": hour,
                        "time": f"{hour:02}:00",
                        "category": (
                            "deficit" if hour < 12 else "surplus"
                        ),
                        "label": (
                            "Enerji Açığı"
                            if hour < 12
                            else "Enerji Fazlası"
                        ),
                        "confidence": 70.0,
                        "probabilities": {
                            "deficit": 70 if hour < 12 else 20,
                            "surplus": 20 if hour < 12 else 70,
                            "balanced": 10,
                        },
                        "observed": False,
                    }
                    for hour in range(24)
                ],
            }
            stored = APP._system_direction_ledger_store(locked_payload)
            self.assertEqual(stored["rowCount"], 24)

            recalculated_payload = {
                **locked_payload,
                "forecastRows": [
                    {
                        **row,
                        "category": "balanced",
                        "label": "Dengede",
                        "observed": True,
                        "actualPublished": True,
                    }
                    for row in locked_payload["forecastRows"]
                ],
                "summary": {},
                "schedule": {},
            }
            status = APP._system_direction_apply_ledger(
                recalculated_payload,
                APP._system_direction_ledger_read(date(2026, 7, 29)),
            )

            self.assertEqual(status["status"], "locked")
            self.assertEqual(
                recalculated_payload["forecastSource"],
                "locked_ledger",
            )
            self.assertEqual(
                recalculated_payload["forecastRows"][0]["category"],
                "deficit",
            )
            self.assertEqual(
                recalculated_payload["forecastRows"][12]["category"],
                "surplus",
            )
            self.assertTrue(
                all(
                    row["observed"] is False
                    for row in recalculated_payload["forecastRows"]
                )
            )
            self.assertIn(
                "01 ve 02",
                recalculated_payload["schedule"]["detail"],
            )
        finally:
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = original_enabled
            APP.SYSTEM_DIRECTION_LEDGER_PATH = original_path
            ledger_path.unlink(missing_ok=True)

    def test_partial_direction_ledger_is_completed_without_target_actuals(self):
        original_enabled = APP.SYSTEM_DIRECTION_LEDGER_ENABLED
        original_path = APP.SYSTEM_DIRECTION_LEDGER_PATH
        ledger_path = ROOT / "tests" / ".system-direction-hybrid-test.sqlite3"
        target_day = datetime.now(APP.URETIM.TR_TZ).date() - timedelta(days=2)

        class StubClient:
            def _post_json(self, endpoint, body):
                if not endpoint.endswith("/system-direction"):
                    raise AssertionError(endpoint)
                start = date.fromisoformat(body["startDate"][:10])
                end = date.fromisoformat(body["endDate"][:10])
                items = []
                current = start
                while current <= end:
                    for hour in range(24):
                        direction = (
                            "Enerji Açığı"
                            if hour < 7
                            else "Enerji Fazlası"
                            if hour < 22
                            else "Dengede"
                        )
                        items.append(
                            {
                                "date": (
                                    f"{current.isoformat()}T"
                                    f"{hour:02}:00:00+03:00"
                                ),
                                "hour": hour,
                                "systemDirection": direction,
                            }
                        )
                    current += timedelta(days=1)
                return {"items": items}

        try:
            ledger_path.unlink(missing_ok=True)
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = True
            APP.SYSTEM_DIRECTION_LEDGER_PATH = ledger_path
            partial = {
                "targetDate": target_day.isoformat(),
                "forecastMode": "intraday",
                "generatedAt": f"{target_day.isoformat()}T10:00:00Z",
                "calendarContext": {"label": "Normal iş günü"},
                "forecastRows": [
                    {
                        "hour": hour,
                        "category": "balanced",
                        "label": "Dengede",
                        "confidence": 61.0,
                        "probabilities": {
                            "deficit": 15,
                            "surplus": 24,
                            "balanced": 61,
                        },
                        "observed": False,
                    }
                    for hour in range(10, 24)
                ],
            }
            stored = APP._system_direction_ledger_store(partial)
            self.assertEqual(stored["rowCount"], 14)

            with APP.SYSTEM_DIRECTION_HISTORY_CACHE_LOCK:
                APP.SYSTEM_DIRECTION_HISTORY_CACHE.clear()
            with APP.SYSTEM_DIRECTION_FORECAST_CACHE_LOCK:
                APP.SYSTEM_DIRECTION_FORECAST_CACHE.clear()
            with APP.SYSTEM_DIRECTION_MODEL_CACHE_LOCK:
                APP.SYSTEM_DIRECTION_MODEL_CACHE.clear()

            forecast = APP._system_direction_forecast(
                target_day.isoformat(),
                StubClient(),
                force_refresh=True,
                allow_past=True,
            )
            self.assertEqual(forecast["forecastSource"], "hybrid_ledger")
            self.assertEqual(forecast["forecastLedger"]["status"], "hybrid")
            self.assertEqual(len(forecast["forecastRows"]), 24)
            self.assertEqual(forecast["summary"]["forecastHours"], 24)
            self.assertTrue(forecast["forecastRows"][0]["reconstructed"])
            self.assertEqual(
                forecast["forecastRows"][0]["category"],
                "deficit",
            )
            self.assertTrue(forecast["forecastRows"][10]["recorded"])
            self.assertEqual(
                forecast["forecastRows"][10]["category"],
                "balanced",
            )

            validation = APP._system_direction_validation(
                target_day.isoformat(),
                StubClient(),
                force_refresh=True,
            )
            self.assertEqual(
                validation["forecastSource"],
                "hybrid_ledger",
            )
            self.assertEqual(validation["summary"]["comparedHours"], 24)
            self.assertNotIn(
                "missing",
                {
                    row["forecastCategory"]
                    for row in validation["rows"]
                },
            )
        finally:
            APP.SYSTEM_DIRECTION_LEDGER_ENABLED = original_enabled
            APP.SYSTEM_DIRECTION_LEDGER_PATH = original_path
            ledger_path.unlink(missing_ok=True)

    def test_username_login_limit_default_stays_below_epias_failed_attempt_limit(self):
        self.assertEqual(APP.LOGIN_USERNAME_LIMITER.max_attempts, 3)
        self.assertEqual(APP.LOGIN_USERNAME_LIMITER.window_seconds, 10)

    def test_login_rate_limiter_blocks_and_recovers_after_cooldown(self):
        now = [100.0]
        limiter = APP.LoginRateLimiter(
            max_attempts=3,
            window_seconds=60,
            block_seconds=30,
            clock=lambda: now[0],
        )
        self.assertEqual(limiter.record_failure("127.0.0.1"), 0)
        self.assertEqual(limiter.record_failure("127.0.0.1"), 0)
        self.assertEqual(limiter.record_failure("127.0.0.1"), 30)
        self.assertEqual(limiter.retry_after("127.0.0.1"), 30)
        now[0] += 31
        self.assertEqual(limiter.retry_after("127.0.0.1"), 0)

    def test_send_content_ignores_client_abort(self):
        class ClosedSocket:
            def write(self, _body):
                raise ConnectionAbortedError("client closed")

        handler = object.__new__(APP.RequestHandler)
        calls = []
        handler._encoded_content = lambda content, _content_type: (content, {})
        handler.send_response = lambda status: calls.append(("status", status))
        handler.send_header = lambda name, value: calls.append((name, value))
        handler.end_headers = lambda: calls.append(("end", None))
        handler.wfile = ClosedSocket()

        APP.RequestHandler._send_content(
            handler,
            b"payload",
            "application/json; charset=utf-8",
        )

        self.assertIn(("status", APP.HTTPStatus.OK), calls)

    def test_background_refresh_defaults_use_safe_intervals(self):
        service = APP.BackgroundRefreshService()
        snapshot = service.snapshot()

        self.assertTrue(snapshot["enabled"])
        self.assertFalse(snapshot["running"])
        self.assertGreaterEqual(snapshot["taskIntervals"]["piyasa"], 60)
        self.assertGreaterEqual(snapshot["taskIntervals"]["ertesiPtf"], 120)
        self.assertGreaterEqual(snapshot["taskIntervals"]["tuketim"], 120)
        self.assertGreaterEqual(snapshot["taskIntervals"]["uretim"], 900)
        self.assertNotIn("baraj", snapshot["taskIntervals"])
        self.assertNotIn("barajSchedule", snapshot)
        self.assertEqual(snapshot["barajRefresh"]["cadence"], "on-access")
        self.assertEqual(
            snapshot["barajRefresh"]["trigger"],
            "login-and-baraj-page",
        )
        self.assertFalse(hasattr(service, "_baraj_schedule_slot"))

    def test_background_loop_never_runs_a_scheduled_baraj_task(self):
        with mock.patch.dict(
            os.environ,
            {"CK_BARAJ_REFRESH_ON_ACCESS": "true"},
        ):
            service = APP.BackgroundRefreshService()
        executed = []
        service._run_task = lambda name, username, runner: executed.append(name)
        session = mock.Mock(username="user@ck.local", tgt="TGT-access-only")

        with mock.patch.object(APP.AUTH, "latest_session", return_value=session):
            service._wake()

        self.assertEqual(
            executed,
            ["piyasa", "ertesiPtf", "tuketim", "uretim"],
        )
        self.assertNotIn("baraj", executed)

    def test_successful_login_queues_immediate_baraj_refresh(self):
        with mock.patch.dict(
            os.environ,
            {"CK_BARAJ_REFRESH_ON_ACCESS": "true"},
        ):
            service = APP.BackgroundRefreshService()
        captured = {}

        class StubClient:
            def __init__(self, *, tgt):
                captured["tgt"] = tgt

        def active_fullness(client, *, force_refresh=False):
            captured["force_refresh"] = force_refresh
            return {"items": [], "cached": False}

        with (
            mock.patch.object(APP.URETIM, "EpiasClient", StubClient),
            mock.patch.object(APP, "_active_fullness", active_fullness),
        ):
            worker = service.queue_baraj_refresh(
                "user@ck.local",
                "TGT-login-refresh",
            )
            self.assertIsNotNone(worker)
            worker.join(timeout=2)

        self.assertFalse(worker.is_alive())
        self.assertEqual(captured["tgt"], "TGT-login-refresh")
        self.assertTrue(captured["force_refresh"])
        task = service.snapshot()["tasks"]["baraj"]
        self.assertEqual(task["status"], "refreshed")
        self.assertEqual(task["successCount"], 1)

    def test_simultaneous_logins_share_one_baraj_refresh(self):
        with mock.patch.dict(
            os.environ,
            {"CK_BARAJ_REFRESH_ON_ACCESS": "true"},
        ):
            service = APP.BackgroundRefreshService()
        started = threading.Event()
        release = threading.Event()

        class StubClient:
            def __init__(self, *, tgt):
                self.tgt = tgt

        def active_fullness(client, *, force_refresh=False):
            started.set()
            release.wait(timeout=2)
            return {"items": [], "cached": False}

        with (
            mock.patch.object(APP.URETIM, "EpiasClient", StubClient),
            mock.patch.object(APP, "_active_fullness", active_fullness),
        ):
            first = service.queue_baraj_refresh("first", "TGT-first")
            self.assertTrue(started.wait(timeout=2))
            second = service.queue_baraj_refresh("second", "TGT-second")
            release.set()
            first.join(timeout=2)

        self.assertIsNotNone(first)
        self.assertIsNone(second)

    def test_next_day_ptf_publication_changes_at_13_and_14(self):
        target_day = date(2026, 7, 23)
        timezone = APP.URETIM.TR_TZ
        waiting = APP._next_day_ptf_publication(
            target_day,
            datetime(2026, 7, 22, 12, 59, tzinfo=timezone),
        )
        preliminary = APP._next_day_ptf_publication(
            target_day,
            datetime(2026, 7, 22, 13, 0, tzinfo=timezone),
        )
        final = APP._next_day_ptf_publication(
            target_day,
            datetime(2026, 7, 22, 14, 0, tzinfo=timezone),
        )
        self.assertEqual(waiting["status"], "waiting")
        self.assertEqual(preliminary["status"], "preliminary")
        self.assertEqual(preliminary["label"], "Kesinleşmemiş PTF")
        self.assertEqual(final["status"], "final")
        self.assertEqual(final["label"], "Kesinleşmiş PTF")
        self.assertIsNone(final["nextRefreshAt"])

    def test_rewrites_only_root_relative_string_paths(self):
        source = (
            'fetch("/api/session"); const external = "https://example.com/x"; '
            'const relative = "assets/logo.png";'
        )
        result = APP._rewrite_paths(source, "/piyasa")
        self.assertIn('fetch("/piyasa/api/session")', result)
        self.assertIn('"https://example.com/x"', result)
        self.assertIn('"assets/logo.png"', result)

    def test_rewrite_paths_keeps_shared_suite_assets_at_the_site_root(self):
        source = '"/suite-assets/icon-192.png"'
        self.assertEqual(
            APP._rewrite_paths(source, "/uretim"),
            '"/suite-assets/icon-192.png?v=7"',
        )
        chart_source = '"/piyasa-charts.js?v=20"'
        self.assertEqual(APP._rewrite_paths(chart_source, "/piyasa"), chart_source)
        self.assertEqual(APP._rewrite_paths('"/login"', "/piyasa"), '"/login"')

    def test_extracts_nested_epias_items(self):
        payload = {"body": {"items": [{"hour": 1}, {"hour": 2}]}}
        self.assertEqual(len(APP._items(payload)), 2)

    def test_hour_key_accepts_iso_and_market_hour(self):
        self.assertEqual(APP._hour_key({"hour": "2026-07-20T04:00:00+03:00"}, 0), 4)
        self.assertEqual(APP._hour_key({"hour": 24}, 0), 23)
        self.assertEqual(APP._hour_key({"date": "2026-07-21T03:00:00+03:00"}, 0), 3)

    def test_suite_navigation_marks_active_module(self):
        navigation = APP._suite_navigation("baraj")
        self.assertIn('href="/baraj/" aria-current="page"', navigation)
        self.assertIn('href="/uretim/"', navigation)
        self.assertIn('href="/tuketim/"', navigation)
        self.assertIn('href="/sistem-yonu-tahmini/"', navigation)
        self.assertIn('class="suite-nav-forecast"', navigation)
        self.assertIn("Sistem Yönü Tahmini", navigation)
        self.assertNotIn("suite-nav-short", navigation)
        self.assertIn("Tüketim", navigation)
        self.assertNotIn("Ana Sayfa", navigation)
        self.assertLess(
            navigation.index('href="/sistem-yonu-tahmini/"'),
            navigation.index('href="/baraj/"'),
        )

    def test_module_sidebar_uses_matching_sections(self):
        baraj = APP._module_sidebar("baraj")
        uretim = APP._module_sidebar("uretim")
        sistem = APP._module_sidebar("sistem")
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-summary"', baraj)
        self.assertIn(
            'href="/baraj/" data-suite-scroll="#baraj-regime-analysis"',
            baraj,
        )
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-map"', baraj)
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-dam-history"', baraj)
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-compare"', baraj)
        self.assertIn("Karşılaştır", baraj)
        self.assertIn('href="/baraj/" data-suite-scroll="#baraj-list"', baraj)
        self.assertNotIn('href="#basin-risk"', baraj)
        self.assertIn('href="/uretim/" data-suite-scroll="#trendTitle"', uretim)
        self.assertIn('href="/uretim/" data-suite-scroll="#detailsTitle"', uretim)
        tuketim = APP._module_sidebar("tuketim")
        self.assertIn(
            'href="/tuketim/" data-suite-scroll="#consumption-summary"',
            tuketim,
        )
        self.assertIn(
            'href="/tuketim/" data-suite-scroll="#consumption-chart"',
            tuketim,
        )
        self.assertIn(
            'href="/tuketim/" data-suite-scroll="#consumption-table"',
            tuketim,
        )
        self.assertIn(
            'href="/sistem-yonu-tahmini/" data-suite-scroll="#system-timeline"',
            sistem,
        )
        self.assertIn(
            'href="/sistem-yonu-tahmini/" data-suite-scroll="#system-samples"',
            sistem,
        )
        self.assertIn(
            'href="/sistem-yonu-tahmini/" data-suite-scroll="#system-validation"',
            sistem,
        )
        self.assertIn("Doğrulama", sistem)
        self.assertIn("Sistem Yönü Tahmini", sistem)
        self.assertIn('src="/suite-assets/ck-mark.png?v=3"', uretim)
        self.assertIn("EPİAŞ · EPİAŞ canlı", baraj)
        self.assertIn("EPİAŞ · EPİAŞ canlı", uretim)
        self.assertIn("EPİAŞ · EPİAŞ canlı", sistem)
        self.assertIn('class="suite-menu-close"', baraj)
        self.assertIn('class="suite-menu-close"', uretim)
        self.assertIn('class="suite-menu-close"', sistem)
        self.assertIn('id="sidebar-lock"', baraj)
        self.assertIn('id="sidebar-lock"', uretim)
        self.assertIn('id="sidebar-lock"', sistem)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', baraj)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', uretim)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', sistem)
        self.assertIn('aria-label="Yan menüyü sabitle"', baraj)
        self.assertNotIn("<small>", baraj)
        self.assertNotIn("<small>", uretim)

    def test_market_dashboard_combines_price_quantity_and_direction(self):
        class StubClient:
            def _post_json(self, endpoint, body):
                if endpoint.endswith("/mcp"):
                    return {
                        "items": [
                            {
                                "hour": 1,
                                "price": 100,
                                "priceEur": 4,
                                "priceUsd": 5,
                            }
                        ],
                        "statistic": {"priceAvg": 100},
                    }
                if endpoint.endswith("/system-marginal-price"):
                    return {
                        "items": [{"hour": 1, "systemMarginalPrice": 110}],
                        "statistics": {"smpArithmeticalAverage": 110},
                    }
                if endpoint.endswith("/order-summary-up"):
                    return {
                        "items": [
                            {
                                "hour": 1,
                                "upRegulationZeroCoded": 1,
                                "upRegulationOneCoded": 2,
                                "upRegulationTwoCoded": 3,
                            }
                        ],
                        "statistics": {
                            "upRegulationZeroCodedTotal": 1,
                            "upRegulationOneCodedTotal": 2,
                            "upRegulationTwoCodedTotal": 3,
                        },
                    }
                if endpoint.endswith("/order-summary-down"):
                    return {
                        "items": [
                            {
                                "hour": 1,
                                "downRegulationZeroCoded": -2,
                                "downRegulationOneCoded": -3,
                                "downRegulationTwoCoded": -4,
                            }
                        ]
                    }
                if endpoint.endswith("/system-direction"):
                    return {
                        "items": [{"hour": 1, "systemDirection": "Enerji Açığı"}]
                    }
                raise AssertionError(endpoint)

        APP.MARKET_CACHE.pop("2026-07-19", None)
        dashboard = APP._market_dashboard("2026-07-19", StubClient())
        self.assertEqual(dashboard["summary"]["ptfAverage"], 100)
        self.assertEqual(dashboard["summary"]["smfAverage"], 110)
        self.assertEqual(dashboard["summary"]["yalTotal"], 6)
        self.assertEqual(dashboard["summary"]["yatTotal"], 9)
        self.assertEqual(dashboard["rows"][0]["time"], "01:00")
        self.assertEqual(dashboard["rows"][0]["direction"], "Enerji Açığı")
        self.assertEqual(
            dashboard["rows"][0]["ptfByCurrency"],
            {"TRY": 100, "EUR": 4, "USD": 5},
        )
        self.assertEqual(
            dashboard["rows"][0]["smfByCurrency"],
            {"TRY": 110},
        )
        self.assertEqual(
            dashboard["currencyInfo"]["available"], ["TRY", "EUR", "USD"]
        )
        self.assertEqual(
            dashboard["currencyInfo"]["mode"],
            "epias-ptf-direct",
        )
        self.assertEqual(dashboard["currencyInfo"]["appliesTo"], "PTF")
        self.assertNotIn("tryPerUnit", dashboard["currencyInfo"])
        self.assertEqual(
            dashboard["summary"]["smfAverageByCurrency"],
            {"TRY": 110},
        )
        self.assertEqual(dashboard["summary"]["ptfSmfCommonHours"], 1)
        self.assertEqual(dashboard["summary"]["ptfCommonAverage"], 100)
        self.assertEqual(dashboard["summary"]["smfCommonAverage"], 110)
        self.assertEqual(
            dashboard["summary"]["smfPtfAverageDifference"], 10
        )

    def test_system_direction_forecast_uses_historical_direction_patterns(self):
        class StubClient:
            def _post_json(self, endpoint, body):
                if not endpoint.endswith("/system-direction"):
                    raise AssertionError(endpoint)
                start = date.fromisoformat(body["startDate"][:10])
                end = date.fromisoformat(body["endDate"][:10])
                items = []
                current = start
                while current <= end:
                    for hour in range(24):
                        if hour < 7:
                            direction = "Enerji Açığı"
                        elif hour < 22:
                            direction = "Enerji Fazlası"
                        else:
                            direction = "Dengede"
                        items.append(
                            {
                                "date": f"{current.isoformat()}T{hour:02}:00:00+03:00",
                                "hour": hour,
                                "systemDirection": direction,
                            }
                        )
                    current += timedelta(days=1)
                return {"items": items}

        with APP.SYSTEM_DIRECTION_HISTORY_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_HISTORY_CACHE.clear()
        with APP.SYSTEM_DIRECTION_FORECAST_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_FORECAST_CACHE.clear()
        with APP.SYSTEM_DIRECTION_MODEL_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_MODEL_CACHE.clear()

        target_day = datetime.now(APP.URETIM.TR_TZ).date() + timedelta(days=1)
        forecast = APP._system_direction_forecast(
            target_day.isoformat(),
            StubClient(),
            force_refresh=True,
        )

        self.assertEqual(forecast["targetDate"], target_day.isoformat())
        self.assertEqual(len(forecast["forecastRows"]), 24)
        self.assertEqual(forecast["forecastRows"][0]["category"], "deficit")
        self.assertEqual(forecast["forecastRows"][9]["category"], "surplus")
        self.assertEqual(forecast["forecastRows"][23]["category"], "balanced")
        self.assertEqual(
            forecast["summary"]["predictedCounts"],
            {"deficit": 7, "surplus": 15, "balanced": 2},
        )
        self.assertGreater(forecast["summary"]["sampleCount"], 10)
        self.assertGreater(forecast["summary"]["averageConfidence"], 65)
        self.assertEqual(forecast["forecastMode"], "day_ahead")
        self.assertTrue(
            forecast["modelSummary"]["machineLearning"]["available"]
        )
        self.assertGreater(
            forecast["modelSummary"]["machineLearning"][
                "validationAccuracy"
            ],
            90,
        )
        self.assertGreater(
            forecast["modelSummary"]["averageWeights"]["ml"],
            0,
        )
        self.assertGreater(
            forecast["modelSummary"]["averageWeights"][
                "transition"
            ],
            0,
        )
        self.assertIn("learning", forecast["modelSummary"]["averageWeights"])
        self.assertIn("errorLearning", forecast["modelSummary"])
        self.assertIn("last7", forecast["qualityMetrics"]["windows"])
        self.assertEqual(len(forecast["qualityMetrics"]["hours"]), 24)
        bucket_labels = [
            bucket["label"] for bucket in forecast["method"]["sourceBuckets"]
        ]
        self.assertIn("Geçmiş yıllar aynı ay/hafta/gün", bucket_labels)

    def test_system_direction_samples_include_same_month_weekday_in_past_years(self):
        target_day = date(2026, 7, 14)  # Temmuz ayının 2. Salı günü
        specs = APP._system_direction_sample_specs(
            target_day,
            target_day - timedelta(days=1),
        )

        self.assertEqual(
            APP._same_month_weekday_in_year(target_day, 2025),
            date(2025, 7, 8),
        )
        self.assertEqual(
            APP._same_month_weekday_in_year(target_day, 2024),
            date(2024, 7, 9),
        )
        self.assertIn(
            "calendar_yearly",
            specs[date(2025, 7, 8)],
        )
        self.assertIn(
            "calendar_yearly",
            specs[date(2024, 7, 9)],
        )
        self.assertEqual(
            APP._same_month_weekday_in_year(date(2026, 3, 31), 2025),
            date(2025, 3, 25),
        )
        self.assertIn("recent", specs[target_day - timedelta(days=28)])
        self.assertIn("weekly", specs[target_day - timedelta(days=12 * 7)])
        self.assertIn(
            "seasonal_yearly",
            specs[date(2025, 7, 8) - timedelta(days=7)],
        )

    def test_system_direction_schedule_switches_at_18_turkey_time(self):
        before = APP._system_direction_schedule(
            datetime(2026, 7, 29, 17, 59, tzinfo=APP.URETIM.TR_TZ)
        )
        after = APP._system_direction_schedule(
            datetime(2026, 7, 29, 18, 0, tzinfo=APP.URETIM.TR_TZ)
        )

        self.assertEqual(before["phase"], "today")
        self.assertEqual(before["targetDate"], "2026-07-29")
        self.assertEqual(before["validationDate"], "2026-07-29")
        self.assertEqual(after["phase"], "tomorrow")
        self.assertEqual(after["targetDate"], "2026-07-30")
        self.assertEqual(after["validationDate"], "2026-07-29")

    def test_direction_features_keep_target_actual_out_of_day_ahead_model(self):
        target_day = date(2026, 7, 29)
        history = {}
        for offset in range(1, 36):
            sample_day = target_day - timedelta(days=offset)
            history[sample_day] = {
                hour: {
                    "hour": hour,
                    "category": (
                        "deficit"
                        if hour < 8
                        else "surplus"
                        if hour < 21
                        else "balanced"
                    ),
                }
                for hour in range(24)
            }
        first_index = {
            **history,
            target_day: {
                hour: {"hour": hour, "category": "surplus"}
                for hour in range(24)
            },
        }
        second_index = {
            **history,
            target_day: {
                hour: {"hour": hour, "category": "deficit"}
                for hour in range(24)
            },
        }

        first = APP._direction_day_features(target_day, 12, first_index)
        second = APP._direction_day_features(target_day, 12, second_index)
        intraday = APP._direction_day_features(
            target_day,
            12,
            first_index,
            observed_by_hour=first_index[target_day],
            observed_cutoff=9,
        )

        self.assertEqual(first, second)
        self.assertGreater(len(intraday), len(first))

    def test_historical_forecast_never_requests_target_day_actuals(self):
        target_day = datetime.now(APP.URETIM.TR_TZ).date() - timedelta(days=3)
        requested_ranges = []

        class StubClient:
            def _post_json(self, endpoint, body):
                if not endpoint.endswith("/system-direction"):
                    raise AssertionError(endpoint)
                start = date.fromisoformat(body["startDate"][:10])
                end = date.fromisoformat(body["endDate"][:10])
                requested_ranges.append((start, end))
                items = []
                current = start
                while current <= end:
                    for hour in range(24):
                        direction = (
                            "Enerji Açığı"
                            if hour < 7
                            else "Enerji Fazlası"
                            if hour < 22
                            else "Dengede"
                        )
                        items.append(
                            {
                                "date": (
                                    f"{current.isoformat()}T"
                                    f"{hour:02}:00:00+03:00"
                                ),
                                "hour": hour,
                                "systemDirection": direction,
                            }
                        )
                    current += timedelta(days=1)
                return {"items": items}

        with APP.SYSTEM_DIRECTION_HISTORY_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_HISTORY_CACHE.clear()
        with APP.SYSTEM_DIRECTION_FORECAST_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_FORECAST_CACHE.clear()
        with APP.SYSTEM_DIRECTION_MODEL_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_MODEL_CACHE.clear()

        forecast = APP._system_direction_forecast(
            target_day.isoformat(),
            StubClient(),
            force_refresh=True,
            allow_past=True,
        )

        self.assertTrue(requested_ranges)
        self.assertTrue(
            all(end < target_day for _start, end in requested_ranges)
        )
        self.assertTrue(forecast["historicalTest"])
        self.assertEqual(
            forecast["trainingCutoffDate"],
            (target_day - timedelta(days=1)).isoformat(),
        )
        self.assertFalse(forecast["leakageGuard"]["targetActualUsed"])
        guard_inputs = [
            item
            for item in forecast["method"]["inputs"]
            if item.get("key") == "historical-leakage-guard"
        ]
        self.assertEqual(guard_inputs[0]["statusLabel"], "Kopya yok")

    def test_softmax_direction_model_learns_repeating_classes(self):
        features = []
        labels = []
        for index in range(180):
            category_index = index % 3
            features.append(
                [
                    1.0 if category_index == 0 else 0.0,
                    1.0 if category_index == 1 else 0.0,
                    1.0 if category_index == 2 else 0.0,
                ]
            )
            labels.append(APP.DIRECTION_CATEGORIES[category_index])

        model = APP._softmax_direction_model(features, labels)
        probabilities = APP._softmax_direction_probabilities(
            model,
            [0.0, 1.0, 0.0],
        )

        self.assertTrue(model["available"])
        self.assertGreater(model["validationAccuracy"], 95)
        self.assertEqual(
            max(probabilities, key=probabilities.get),
            "surplus",
        )

    def test_direction_ml_model_reuses_training_cache_until_history_changes(self):
        target_day = date(2026, 7, 29)
        history = {
            target_day - timedelta(days=offset): {
                hour: {
                    "hour": hour,
                    "category": "deficit" if hour < 8 else "surplus",
                }
                for hour in range(24)
            }
            for offset in range(1, 50)
        }
        with APP.SYSTEM_DIRECTION_MODEL_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_MODEL_CACHE.clear()

        first = APP._cached_direction_ml_model(
            target_day,
            history,
            "day_ahead",
        )
        second = APP._cached_direction_ml_model(
            target_day,
            history,
            "day_ahead",
        )

        self.assertFalse(first["trainingCacheHit"])
        self.assertTrue(second["trainingCacheHit"])
        self.assertEqual(first["sampleCount"], second["sampleCount"])

        changed_day = target_day - timedelta(days=1)
        history[changed_day][0]["category"] = "balanced"
        changed = APP._cached_direction_ml_model(
            target_day,
            history,
            "day_ahead",
        )
        self.assertFalse(changed["trainingCacheHit"])

    def test_system_direction_quality_has_windows_categories_and_hours(self):
        target_day = date(2026, 7, 29)
        records = []
        for days_ago in range(1, 11):
            sample_day = target_day - timedelta(days=days_ago)
            for hour in range(24):
                actual = "deficit" if hour < 8 else "surplus"
                records.append(
                    {
                        "sampleDay": sample_day,
                        "date": sample_day.isoformat(),
                        "hour": hour,
                        "actual": actual,
                        "predicted": actual if hour != 3 else "balanced",
                        "confidence": 80,
                        "correct": hour != 3,
                    }
                )

        quality = APP._summarize_system_direction_quality(
            records,
            target_day,
        )

        self.assertEqual(quality["windows"]["last7"]["dayCount"], 7)
        self.assertEqual(quality["windows"]["last30"]["dayCount"], 10)
        self.assertEqual(len(quality["hours"]), 24)
        self.assertEqual(quality["hours"][3]["accuracy"], 0)
        self.assertEqual(quality["hours"][4]["accuracy"], 100)
        self.assertEqual(
            quality["categories"]["surplus"]["accuracy"],
            100,
        )

    def test_transition_model_uses_regime_duration(self):
        target_day = date(2026, 7, 29)
        history = {}
        for offset in range(1, 43):
            sample_day = target_day - timedelta(days=offset)
            history[sample_day] = {
                hour: {
                    "hour": hour,
                    "category": (
                        "deficit" if hour < 9 else "surplus"
                    ),
                }
                for hour in range(24)
            }

        model = APP._system_direction_transition_model(
            target_day,
            history,
        )
        probabilities = APP._transition_probabilities(
            model,
            6,
            "deficit",
            6,
        )

        self.assertGreater(model["accuracy"], 90)
        self.assertGreater(
            probabilities["deficit"],
            probabilities["surplus"],
        )

    def test_operational_signal_combines_balancing_price_and_demand_error(self):
        rows = [
            {
                "hour": 8,
                "yal": 180,
                "yat": 20,
                "ptf": 2000,
                "smf": 2600,
            }
        ]
        consumption_rows = [
            {
                "hour": 8,
                "forecast": 45_000,
                "actual": 49_000,
            }
        ]

        probabilities = (
            APP._system_direction_operational_probabilities(
                rows,
                8,
                9,
                consumption_rows=consumption_rows,
            )
        )

        self.assertIsNotNone(probabilities)
        self.assertGreater(
            probabilities["deficit"],
            probabilities["surplus"],
        )

    def test_ensemble_gives_better_validated_model_more_weight(self):
        weights = APP._system_direction_ensemble_weights(
            52,
            81,
            63,
        )

        self.assertGreater(weights["ml"], weights["history"])
        self.assertGreater(weights["ml"], weights["transition"])
        self.assertAlmostEqual(sum(weights.values()), 1.0, places=3)

    def test_system_direction_context_combines_demand_supply_and_weather(self):
        scores = {
            hour: {"deficit": 10.0, "surplus": 10.0, "balanced": 10.0}
            for hour in range(24)
        }
        context = {
            "weather": {
                "rows": [
                    {
                        "hour": 1,
                        "temperature_2m": 35,
                        "wind_speed_10m": 1,
                        "wind_gusts_10m": 2,
                        "cloud_cover": 20,
                        "precipitation": 0,
                        "shortwave_radiation": 0,
                    }
                ]
            },
            "kgup": {
                "rows": [
                    {"hour": 0, "renewableShare": 50},
                    {"hour": 1, "renewableShare": 10},
                ]
            },
            "consumption": {
                "rows": [
                    {"hour": 0, "forecast": 100},
                    {"hour": 1, "forecast": 200},
                ]
            },
        }

        adjusted, details = APP._apply_system_direction_context(scores, context)

        self.assertGreater(adjusted[1]["deficit"], adjusted[1]["surplus"])
        self.assertGreater(details[1]["balancePressure"], 0.2)
        labels = [signal["label"] for signal in details[1]["signals"]]
        self.assertIn("Yüksek tüketim", labels)
        self.assertIn("Düşük yenilenebilir planı", labels)
        self.assertIn("Sıcaklık kaynaklı talep", labels)

    def test_system_direction_calibration_corrects_repeated_early_transition(self):
        scores = {
            hour: {"deficit": 4.0, "surplus": 3.0, "balanced": 1.0}
            for hour in range(24)
        }
        scores[3] = {"deficit": 4.0, "surplus": 6.0, "balanced": 1.0}
        calibration = {
            "confusion": {
                3: {
                    "surplus": {
                        "deficit": 6.0,
                        "surplus": 1.0,
                        "balanced": 0.0,
                    }
                }
            },
            "transitionHolds": {
                3: {"deficit": 5.0, "surplus": 0.0, "balanced": 0.0}
            },
        }

        adjusted, details = APP._apply_system_direction_calibration(
            scores,
            calibration,
        )

        self.assertGreater(adjusted[3]["deficit"], adjusted[3]["surplus"])
        self.assertTrue(details[3]["applied"])
        self.assertTrue(details[3]["changedDirection"])
        self.assertGreater(details[3]["transitionBlend"], 0)

    def test_error_learning_ml_learns_repeated_hourly_mistakes(self):
        target_day = date(2026, 7, 29)
        records = []
        for days_ago in range(30, 0, -1):
            sample_day = target_day - timedelta(days=days_ago)
            for hour in range(24):
                predicted = "surplus"
                actual = "deficit" if hour in {3, 4} else "surplus"
                records.append(
                    {
                        "sampleDay": sample_day,
                        "date": sample_day.isoformat(),
                        "hour": hour,
                        "predicted": predicted,
                        "previousPredicted": predicted,
                        "actual": actual,
                        "probabilities": {
                            "deficit": 0.18,
                            "surplus": 0.74,
                            "balanced": 0.08,
                        },
                        "correct": predicted == actual,
                    }
                )

        model = APP._system_direction_error_learning_model(
            target_day,
            {"records": records},
        )
        mistake_hour = APP._softmax_direction_probabilities(
            model,
            APP._direction_error_learning_features(
                target_day,
                3,
                {"deficit": 0.18, "surplus": 0.74, "balanced": 0.08},
                "surplus",
                "surplus",
            ),
        )
        normal_hour = APP._softmax_direction_probabilities(
            model,
            APP._direction_error_learning_features(
                target_day,
                12,
                {"deficit": 0.18, "surplus": 0.74, "balanced": 0.08},
                "surplus",
                "surplus",
            ),
        )

        self.assertTrue(model["available"])
        self.assertEqual(model["mistakeCount"], 60)
        self.assertGreater(model["validationAccuracy"], 80)
        self.assertGreater(mistake_hour["deficit"], mistake_hour["surplus"])
        self.assertGreater(normal_hour["surplus"], normal_hour["deficit"])

    def test_system_direction_miss_reason_detects_delayed_regime_change(self):
        forecast = {
            2: {"hour": 2, "category": "deficit"},
            3: {"hour": 3, "category": "surplus"},
            4: {"hour": 4, "category": "surplus"},
        }
        actual = {
            2: {"hour": 2, "category": "deficit"},
            3: {"hour": 3, "category": "deficit"},
            4: {"hour": 4, "category": "deficit"},
        }

        first_miss = APP._system_direction_miss_reason(3, forecast, actual)
        second_miss = APP._system_direction_miss_reason(4, forecast, actual)

        self.assertEqual(first_miss["code"], "early_transition")
        self.assertIn("1 saat daha sürdü", first_miss["text"])
        self.assertEqual(second_miss["code"], "early_transition")
        self.assertIn("2 saat daha sürdü", second_miss["text"])

    def test_system_direction_kgup_reads_hourly_generation_mix(self):
        class StubClient:
            def _post_json(self, endpoint, body):
                self.endpoint = endpoint
                self.body = body
                return {
                    "items": [
                        {
                            "date": "2026-07-30T00:00:00+03:00",
                            "toplam": 50_000,
                            "tuketim": 49_000,
                            "ruzgar": 5_000,
                            "gunes": 0,
                            "akarsu": 2_000,
                            "barajli": 3_000,
                            "biokutle": 500,
                            "jeotermal": 1_000,
                        }
                    ]
                }

        client = StubClient()
        result = APP._system_direction_kgup(date(2026, 7, 30), client)

        self.assertEqual(client.endpoint, "/v1/generation/data/dpp")
        self.assertEqual(client.body["region"], "TR1")
        self.assertEqual(result["availableHours"], 1)
        self.assertEqual(result["rows"][0]["renewable"], 11_500)
        self.assertEqual(result["rows"][0]["renewableShare"], 23.0)

    def test_system_direction_weather_aggregates_hourly_forecast(self):
        request_options = {}

        class FakeResponse:
            def __enter__(self):
                return self

            def __exit__(self, *_args):
                return False

            def read(self):
                hourly = {
                    "time": [
                        f"2026-07-30T{hour:02}:00"
                        for hour in range(24)
                    ]
                }
                for field in APP.SYSTEM_DIRECTION_WEATHER_FIELDS:
                    hourly[field] = [20 + hour for hour in range(24)]
                return json.dumps({"hourly": hourly}).encode("utf-8")

        original_urlopen = APP.urllib.request.urlopen
        def fake_urlopen(*_args, **kwargs):
            request_options.update(kwargs)
            return FakeResponse()

        APP.urllib.request.urlopen = fake_urlopen
        try:
            with APP.SYSTEM_DIRECTION_WEATHER_CACHE_LOCK:
                APP.SYSTEM_DIRECTION_WEATHER_CACHE.clear()
            result = APP._system_direction_weather(
                date(2026, 7, 30),
                force_refresh=True,
            )
        finally:
            APP.urllib.request.urlopen = original_urlopen

        self.assertEqual(result["availableHours"], 24)
        self.assertEqual(result["pointCount"], 1)
        self.assertEqual(result["rows"][0]["temperature_2m"], 20)
        self.assertEqual(result["rows"][23]["wind_speed_80m"], 43)
        self.assertEqual(request_options["timeout"], 8)
        self.assertEqual(request_options["context"].verify_mode, APP.ssl.CERT_REQUIRED)
        self.assertTrue(request_options["context"].check_hostname)

    def test_system_direction_validation_compares_forecast_with_actual_hours(self):
        validation_day = datetime.now(APP.URETIM.TR_TZ).date()

        class StubClient:
            actual_day = validation_day

            def _post_json(self, endpoint, body):
                if not endpoint.endswith("/system-direction"):
                    raise AssertionError(endpoint)
                start = date.fromisoformat(body["startDate"][:10])
                end = date.fromisoformat(body["endDate"][:10])
                items = []
                current = start
                while current <= end:
                    for hour in range(24):
                        if hour < 7:
                            direction = "Enerji Açığı"
                        elif hour < 22:
                            direction = "Enerji Fazlası"
                        else:
                            direction = "Dengede"
                        if current == self.actual_day and hour == 9:
                            direction = "Enerji Açığı"
                        items.append(
                            {
                                "date": f"{current.isoformat()}T{hour:02}:00:00+03:00",
                                "hour": hour,
                                "systemDirection": direction,
                            }
                        )
                    current += timedelta(days=1)
                return {"items": items}

        with APP.SYSTEM_DIRECTION_HISTORY_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_HISTORY_CACHE.clear()
        with APP.SYSTEM_DIRECTION_FORECAST_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_FORECAST_CACHE.clear()
        with APP.SYSTEM_DIRECTION_MODEL_CACHE_LOCK:
            APP.SYSTEM_DIRECTION_MODEL_CACHE.clear()

        validation = APP._system_direction_validation(
            validation_day.isoformat(),
            StubClient(),
            force_refresh=True,
        )

        self.assertEqual(validation["date"], validation_day.isoformat())
        self.assertEqual(len(validation["rows"]), 24)
        self.assertEqual(validation["summary"]["publishedHours"], 24)
        self.assertEqual(validation["summary"]["comparedHours"], 24)
        self.assertEqual(validation["summary"]["correctHours"], 23)
        self.assertEqual(validation["summary"]["wrongHours"], 1)
        self.assertEqual(validation["summary"]["accuracy"], 95.8)
        self.assertEqual(validation["summary"]["status"], "ready")
        self.assertFalse(validation["rows"][9]["match"])
        self.assertEqual(validation["rows"][9]["forecastCategory"], "surplus")
        self.assertEqual(validation["rows"][9]["actualCategory"], "deficit")
        self.assertTrue(validation["rows"][9]["reason"])
        self.assertTrue(validation["analysis"]["learningEnabled"])
        self.assertTrue(validation["leakageGuard"]["enabled"])
        self.assertFalse(
            validation["leakageGuard"]["targetActualUsedForForecast"]
        )
        self.assertTrue(
            validation["leakageGuard"]["actualLoadedAfterForecast"]
        )
        self.assertIn("last30", validation["qualityMetrics"]["windows"])
        self.assertEqual(len(validation["qualityMetrics"]["hours"]), 24)
        self.assertIn("sapma analiz edildi", validation["note"])

    def test_basin_depletion_requires_enough_reliable_observations(self):
        short_series = [
            {"date": "2026-07-20", "average": 70},
            {"date": "2026-07-21", "average": 69},
        ]
        short_analysis = APP._basin_regime_analysis(short_series)
        self.assertIsNone(short_analysis["projectedDepletionDate"])
        self.assertEqual(short_analysis["confidence"], "yetersiz veri")
        self.assertIn("En az 7 yayın", short_analysis["projectionStatus"])

        start = date(2026, 7, 1)
        reliable_series = [
            {
                "date": (start + timedelta(days=index * 2)).isoformat(),
                "average": 80 - index * 2,
            }
            for index in range(8)
        ]
        reliable_analysis = APP._basin_regime_analysis(reliable_series)
        self.assertIsNotNone(reliable_analysis["projectedDepletionDate"])
        self.assertEqual(reliable_analysis["confidence"], "yüksek")
        self.assertIn("Deneysel", reliable_analysis["projectionStatus"])

    def test_basin_regime_analysis_includes_weekly_and_monthly_drop(self):
        start = date(2026, 7, 1)
        series = [
            {
                "date": (start + timedelta(days=index)).isoformat(),
                "average": 90 - index,
            }
            for index in range(35)
        ]
        analysis = APP._basin_regime_analysis(series)
        daily = analysis["periodComparisons"]["daily"]
        weekly = analysis["periodComparisons"]["weekly"]
        monthly = analysis["periodComparisons"]["monthly"]
        self.assertTrue(daily["available"])
        self.assertTrue(weekly["available"])
        self.assertTrue(monthly["available"])
        self.assertEqual(daily["actualDays"], 1)
        self.assertAlmostEqual(daily["drop"], 1)
        self.assertEqual(weekly["actualDays"], 7)
        self.assertEqual(monthly["actualDays"], 30)
        self.assertAlmostEqual(weekly["drop"], 7)
        self.assertAlmostEqual(monthly["drop"], 30)
        self.assertAlmostEqual(weekly["change"], -7)

    def test_basin_regime_daily_comparison_requires_previous_calendar_day(self):
        analysis = APP._basin_regime_analysis(
            [
                {"date": "2026-07-01", "average": 70},
                {"date": "2026-07-03", "average": 68},
            ]
        )

        daily = analysis["periodComparisons"]["daily"]
        self.assertFalse(daily["available"])
        self.assertIsNone(daily["baselineDate"])
        self.assertIsNone(daily["drop"])

    def test_basin_risk_combines_fullness_decline_and_critical_horizon(self):
        start = date(2026, 7, 1)
        declining = [
            {
                "date": (start + timedelta(days=index * 2)).isoformat(),
                "average": 52 - index * 3,
            }
            for index in range(8)
        ]
        analysis = APP._basin_regime_analysis(declining)
        risk = APP._basin_risk_analysis(declining, analysis)
        self.assertEqual(risk["level"], "Yüksek")
        self.assertGreater(risk["score"], 0)
        self.assertIsNotNone(risk["daysToCritical"])
        self.assertIsNotNone(risk["criticalDate"])
        self.assertIn("weeklyDrop", risk)
        self.assertIn("monthlyDrop", risk)
        self.assertIn("günde ortalama", risk["reason"])
        self.assertIn("%30 seviyesine", risk["reason"])

        stable = [
            {"date": (start + timedelta(days=index * 2)).isoformat(), "average": 82}
            for index in range(8)
        ]
        stable_analysis = APP._basin_regime_analysis(stable)
        stable_risk = APP._basin_risk_analysis(stable, stable_analysis)
        self.assertEqual(stable_risk["level"], "Düşük")
        self.assertIsNone(stable_risk["daysToCritical"])

    def test_market_dashboard_does_not_derive_missing_hourly_currencies(self):
        class StubClient:
            def _post_json(self, endpoint, body):
                if endpoint.endswith("/mcp"):
                    return {
                        "items": [{"hour": 1, "price": 1_000}],
                        "statistic": {
                            "priceAvg": 1_000,
                            "priceEurAvg": 20,
                            "priceUsdAvg": 25,
                        },
                    }
                if endpoint.endswith("/system-marginal-price"):
                    return {
                        "items": [{"hour": 1, "systemMarginalPrice": 1_100}],
                        "statistics": {"smpArithmeticalAverage": 1_100},
                    }
                return {"items": []}

        APP.MARKET_CACHE.pop("2026-07-18", None)
        dashboard = APP._market_dashboard("2026-07-18", StubClient())
        self.assertEqual(
            dashboard["currencyInfo"]["available"],
            ["TRY", "EUR", "USD"],
        )
        self.assertIsNone(dashboard["rows"][0]["ptfByCurrency"]["EUR"])
        self.assertIsNone(dashboard["rows"][0]["ptfByCurrency"]["USD"])
        self.assertEqual(
            dashboard["summary"]["ptfAverageByCurrency"]["EUR"],
            20,
        )
        self.assertEqual(
            dashboard["summary"]["ptfAverageByCurrency"]["USD"],
            25,
        )
        self.assertEqual(
            dashboard["rows"][0]["smfByCurrency"],
            {"TRY": 1_100},
        )

    def test_market_today_incomplete_smf_uses_short_refresh_policy(self):
        selected_date = APP.datetime.now(APP.URETIM.TR_TZ).date().isoformat()
        APP.MARKET_CACHE.pop(selected_date, None)

        class StubClient:
            def _post_json(self, endpoint, body):
                if endpoint.endswith("/mcp"):
                    return {
                        "items": [
                            {"hour": hour, "price": 1_000 + hour}
                            for hour in range(24)
                        ]
                    }
                if endpoint.endswith("/system-marginal-price"):
                    return {
                        "items": [
                            {"hour": 0, "systemMarginalPrice": 0},
                            {"hour": 1, "systemMarginalPrice": 1_100},
                        ]
                    }
                return {"items": []}

        before = APP.time.time()
        dashboard = APP._market_dashboard(selected_date, StubClient())
        self.assertEqual(dashboard["freshness"]["smfPublishedHours"], 2)
        self.assertTrue(dashboard["freshness"]["smfIncomplete"])
        self.assertEqual(dashboard["freshness"]["clientCacheMs"], 15_000)
        self.assertEqual(dashboard["freshness"]["nextRefreshMs"], 45_000)
        self.assertEqual(dashboard["summary"]["ptfSmfCommonHours"], 2)
        self.assertEqual(dashboard["summary"]["smfAverage"], 550)
        self.assertLessEqual(
            APP.MARKET_CACHE[selected_date]["expires"] - before,
            25,
        )

    def test_market_historical_zero_smf_is_a_published_value(self):
        selected_date = "2026-07-11"
        APP.MARKET_CACHE.pop(selected_date, None)

        class StubClient:
            def _post_json(self, endpoint, body):
                if endpoint.endswith("/mcp"):
                    return {
                        "items": [
                            {
                                "hour": "2026-07-11T12:00:00+03:00",
                                "price": 2_000,
                            }
                        ]
                    }
                if endpoint.endswith("/system-marginal-price"):
                    return {
                        "items": [
                            {
                                "hour": "2026-07-11T12:00:00+03:00",
                                "systemMarginalPrice": 0,
                            }
                        ]
                    }
                return {"items": []}

        dashboard = APP._market_dashboard(selected_date, StubClient())

        self.assertEqual(dashboard["rows"][0]["hour"], 12)
        self.assertEqual(dashboard["rows"][0]["smf"], 0)
        self.assertTrue(dashboard["rows"][0]["smfPublished"])
        self.assertEqual(dashboard["freshness"]["smfPublishedHours"], 1)
        self.assertEqual(dashboard["summary"]["ptfSmfCommonHours"], 1)
        self.assertEqual(dashboard["summary"]["smfAverage"], 0)

    def test_market_dashboard_rejects_future_dates_before_epias_call(self):
        future = (
            APP.datetime.now(APP.URETIM.TR_TZ).date() + timedelta(days=1)
        ).isoformat()

        class FailClient:
            def _post_json(self, endpoint, body):
                raise AssertionError("Gelecek tarih için EPİAŞ çağrılmamalı")

        with self.assertRaisesRegex(ValueError, "Bugünden ileri"):
            APP._market_dashboard(future, FailClient())

    def test_next_day_ptf_uses_epias_interim_mcp_during_preliminary_window(self):
        selected_day = date(2026, 7, 22)
        target_day = selected_day + timedelta(days=1)
        APP.NEXT_DAY_PTF_CACHE.pop(target_day.isoformat(), None)

        class StubClient:
            def __init__(self):
                self.calls = []

            def _post_json(self, endpoint, body):
                self.calls.append((endpoint, body))
                return {
                    "items": [
                        {
                            "hour": "00:00",
                            "marketTradePrice": 2_500,
                        },
                        {
                            "hour": "01:00",
                            "marketTradePrice": 3_000,
                        },
                    ],
                    "statistic": {
                        "interimMcpAvg": 2_750,
                    },
                }

        client = StubClient()
        payload = APP._next_day_ptf_dashboard(
            selected_day.isoformat(),
            client,
            now_tr=datetime(
                2026, 7, 22, 13, 30, tzinfo=APP.URETIM.TR_TZ
            ),
        )

        self.assertEqual(len(client.calls), 1)
        self.assertTrue(
            client.calls[0][0].endswith("/markets/dam/data/interim-mcp")
        )
        self.assertEqual(
            client.calls[0][1]["startDate"],
            f"{target_day.isoformat()}T00:00:00+03:00",
        )
        self.assertNotIn("endDate", client.calls[0][1])
        self.assertEqual(payload["date"], target_day.isoformat())
        self.assertTrue(payload["published"])
        self.assertEqual(payload["publication"]["status"], "preliminary")
        self.assertEqual(payload["publication"]["source"], "interim-mcp")
        self.assertIn("preliminaryAt", payload["publication"])
        self.assertIn("finalAt", payload["publication"])
        self.assertEqual(payload["summary"]["publishedHours"], 2)
        self.assertEqual(
            payload["summary"]["ptfAverageByCurrency"],
            {"TRY": 2_750, "EUR": None, "USD": None},
        )
        self.assertEqual(
            payload["currencyInfo"]["available"],
            ["TRY"],
        )

    def test_next_day_ptf_does_not_query_interim_before_13(self):
        selected_day = date(2026, 7, 22)
        target_day = selected_day + timedelta(days=1)
        APP.NEXT_DAY_PTF_CACHE.pop(target_day.isoformat(), None)

        class StubClient:
            def _post_json(self, endpoint, body):
                raise AssertionError("13.00 öncesinde PTF servisi çağrılmamalı")

        payload = APP._next_day_ptf_dashboard(
            selected_day.isoformat(),
            StubClient(),
            now_tr=datetime(
                2026, 7, 22, 12, 59, tzinfo=APP.URETIM.TR_TZ
            ),
        )

        self.assertEqual(payload["rows"], [])
        self.assertEqual(payload["publication"]["status"], "waiting")
        self.assertIsNone(payload["publication"]["source"])

    def test_next_day_ptf_uses_final_mcp_after_14(self):
        selected_day = date(2026, 7, 22)
        target_day = selected_day + timedelta(days=1)
        APP.NEXT_DAY_PTF_CACHE.pop(target_day.isoformat(), None)

        class StubClient:
            def __init__(self):
                self.calls = []

            def _post_json(self, endpoint, body):
                self.calls.append((endpoint, body))
                return {
                    "items": [{"hour": "00:00", "price": 3_000}],
                    "statistic": {"priceAvg": 3_000},
                }

        client = StubClient()
        payload = APP._next_day_ptf_dashboard(
            selected_day.isoformat(),
            client,
            now_tr=datetime(
                2026, 7, 22, 14, 5, tzinfo=APP.URETIM.TR_TZ
            ),
        )

        self.assertEqual(len(client.calls), 1)
        self.assertTrue(client.calls[0][0].endswith("/markets/dam/data/mcp"))
        self.assertIn("endDate", client.calls[0][1])
        self.assertEqual(payload["publication"]["status"], "final")
        self.assertEqual(payload["publication"]["source"], "mcp")

    def test_next_day_ptf_does_not_fall_back_to_interim_after_14(self):
        selected_day = date(2026, 7, 22)
        target_day = selected_day + timedelta(days=1)
        APP.NEXT_DAY_PTF_CACHE.pop(target_day.isoformat(), None)

        class StubClient:
            def __init__(self):
                self.calls = []

            def _post_json(self, endpoint, body):
                self.calls.append(endpoint)
                return {"items": []}

        client = StubClient()
        payload = APP._next_day_ptf_dashboard(
            selected_day.isoformat(),
            client,
            now_tr=datetime(
                2026, 7, 22, 14, 1, tzinfo=APP.URETIM.TR_TZ
            ),
        )

        self.assertEqual(len(client.calls), 1)
        self.assertTrue(client.calls[0].endswith("/markets/dam/data/mcp"))
        self.assertEqual(payload["rows"], [])
        self.assertEqual(payload["publication"]["status"], "final")
        self.assertEqual(payload["publication"]["label"], "Kesinleşmiş PTF")
        self.assertEqual(payload["publication"]["source"], "mcp")
        self.assertEqual(
            payload["publication"]["nextRefreshAt"],
            "2026-07-22T14:03:00+03:00",
        )

    def test_consumption_dashboard_normalizes_epias_hourly_values(self):
        selected_date = "2026-07-20"
        APP.CONSUMPTION_CACHE.pop(selected_date, None)

        class StubClient:
            def _post_json(self, endpoint, body):
                self.endpoint = endpoint
                self.body = body
                return {
                    "items": [
                        {
                            "date": "2026-07-20T00:00:00+03:00",
                            "consumption": 31_000,
                        },
                        {
                            "time": "2026-07-20T01:00:00+03:00",
                            "consumption": 32_500,
                        },
                    ],
                    "statistics": {
                        "consumptionAvg": 31_750,
                        "consumptionMax": 32_500,
                        "consumptionMin": 31_000,
                        "consumptionTotal": 63_500,
                    },
                }

        client = StubClient()
        payload = APP._consumption_dashboard(selected_date, client)
        self.assertEqual(
            client.endpoint,
            "/v1/consumption/data/realtime-consumption",
        )
        self.assertEqual(client.body["startDate"], "2026-07-20T00:00:00+03:00")
        self.assertEqual(client.body["endDate"], "2026-07-20T00:00:00+03:00")
        self.assertEqual(len(payload["rows"]), 24)
        self.assertEqual(payload["rows"][0]["consumption"], 31_000)
        self.assertEqual(payload["rows"][1]["consumption"], 32_500)
        self.assertIsNone(payload["rows"][2]["consumption"])
        self.assertEqual(payload["summary"]["latest"], 32_500)
        self.assertEqual(payload["summary"]["latestHour"], "01:00")
        self.assertEqual(payload["summary"]["availableHours"], 2)
        self.assertEqual(payload["summary"]["missingHours"], 22)
        self.assertEqual(payload["publicationDelayHours"], 2)

    def test_consumption_dashboard_rejects_future_dates(self):
        future = (
            APP.datetime.now(APP.URETIM.TR_TZ).date() + timedelta(days=1)
        ).isoformat()
        with self.assertRaisesRegex(ValueError, "Bugünden ileri"):
            APP._consumption_dashboard(future, object())

    def test_consumption_forecast_builds_24_hour_weighted_profile(self):
        today = datetime.now(APP.URETIM.TR_TZ).date()
        training_start = today - timedelta(days=13)

        class StubClient:
            def __init__(self):
                self.calls = []

            def _post_json(self, endpoint, body):
                self.calls.append((endpoint, body))
                items = []
                for day_offset in range(14):
                    current = training_start + timedelta(days=day_offset)
                    for hour in range(24):
                        items.append({
                            "date": f"{current.isoformat()}T{hour:02d}:00:00+03:00",
                            "consumption": 40_000 + hour * 100,
                        })
                return {"items": items}

        APP.CONSUMPTION_FORECAST_CACHE.clear()
        APP.CONSUMPTION_FORECAST_LOCKED_ROWS.clear()
        client = StubClient()
        payload = APP._consumption_forecast(today.isoformat(), client)
        self.assertEqual(payload["date"], (today + timedelta(days=1)).isoformat())
        self.assertEqual(payload["summary"]["trainingDays"], 14)
        self.assertEqual(payload["summary"]["forecastHours"], 24)
        self.assertEqual(payload["summary"]["maximumHour"], "23:00")
        self.assertEqual(payload["rows"][3]["forecast"], 40_300)
        self.assertEqual(len(client.calls), 1)

    def test_consumption_forecast_can_target_selected_day_and_marks_locks(self):
        today = datetime.now(APP.URETIM.TR_TZ).date()
        training_end = today - timedelta(days=1)
        training_start = training_end - timedelta(days=13)

        class StubClient:
            def __init__(self):
                self.calls = []

            def _post_json(self, endpoint, body):
                self.calls.append((endpoint, body))
                return {"items": [
                    {
                        "date": f"{(training_start + timedelta(days=day_offset)).isoformat()}T{hour:02d}:00:00+03:00",
                        "consumption": 39_000 + hour * 80,
                    }
                    for day_offset in range(14)
                    for hour in range(24)
                ]}

        APP.CONSUMPTION_FORECAST_CACHE.clear()
        APP.CONSUMPTION_FORECAST_LOCKED_ROWS.clear()
        client = StubClient()
        payload = APP._consumption_forecast(
            today.isoformat(),
            client,
            target_date=today.isoformat(),
        )
        self.assertEqual(payload["date"], today.isoformat())
        self.assertEqual(
            client.calls[0][1]["endDate"],
            f"{training_end.isoformat()}T00:00:00+03:00",
        )
        self.assertEqual(payload["summary"]["forecastHours"], 24)
        self.assertIn("lockedHours", payload["summary"])
        self.assertIn("finalUpdateAt", payload["rows"][0])
        self.assertIn("hourStartsAt", payload["rows"][0])

    def test_consumption_forecast_compares_published_actual_hours(self):
        today = datetime.now(APP.URETIM.TR_TZ).date()
        base_day = today - timedelta(days=1)
        training_start = base_day - timedelta(days=13)

        class StubClient:
            def _post_json(self, endpoint, body):
                start = str(body["startDate"])[:10]
                end = str(body["endDate"])[:10]
                if start == end:
                    return {"items": [
                        {"date": f"{today.isoformat()}T00:00:00+03:00", "consumption": 41_000},
                        {"date": f"{today.isoformat()}T01:00:00+03:00", "consumption": 41_100},
                    ]}
                return {"items": [
                    {
                        "date": f"{(training_start + timedelta(days=day_offset)).isoformat()}T{hour:02d}:00:00+03:00",
                        "consumption": 40_000 + hour * 100,
                    }
                    for day_offset in range(14)
                    for hour in range(24)
                ]}

        APP.CONSUMPTION_CACHE.clear()
        APP.CONSUMPTION_FORECAST_CACHE.clear()
        APP.CONSUMPTION_FORECAST_LOCKED_ROWS.clear()
        payload = APP._consumption_forecast(base_day.isoformat(), StubClient())
        self.assertEqual(payload["summary"]["actualHours"], 2)
        self.assertEqual(payload["summary"]["comparedHours"], 2)
        self.assertEqual(payload["summary"]["meanAbsoluteError"], 1_000)
        self.assertEqual(payload["rows"][0]["actual"], 41_000)

    def test_consumption_xlsx_contains_summary_and_hourly_sheets(self):
        workbook = APP._consumption_xlsx(
            {
                "date": "2026-07-20",
                "source": "EPİAŞ Şeffaflık Platformu",
                "summary": {
                    "latest": 32_500,
                    "latestHour": "01:00",
                    "average": 31_750,
                    "maximum": 32_500,
                    "maximumHour": "01:00",
                    "minimum": 31_000,
                    "minimumHour": "00:00",
                    "total": 63_500,
                    "availableHours": 2,
                },
                "rows": [
                    {"time": "00:00", "consumption": 31_000},
                    {"time": "01:00", "consumption": 32_500},
                    {"time": "02:00", "consumption": None},
                ],
            }
        )
        self.assertTrue(workbook.startswith(b"PK"))
        with zipfile.ZipFile(io.BytesIO(workbook)) as archive:
            workbook_xml = archive.read("xl/workbook.xml").decode("utf-8")
            detail_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertIn('name="Özet"', workbook_xml)
            self.assertIn('name="Saatlik Tüketim"', workbook_xml)
            self.assertIn("32500", detail_xml)
            self.assertIn("Veri bekleniyor", detail_xml)

    def test_market_force_refresh_bypasses_server_cache_and_keeps_smf_hour(self):
        selected_date = "2026-07-17"
        APP.MARKET_CACHE[selected_date] = {
            "payload": {"rows": [{"hour": 2, "smf": 99}]},
            "expires": APP.time.time() + 600,
        }

        class StubClient:
            calls = 0

            def _post_json(self, endpoint, body):
                self.calls += 1
                if endpoint.endswith("/system-marginal-price"):
                    return {
                        "items": [
                            {
                                "date": "2026-07-17T03:00:00+03:00",
                                "systemMarginalPrice": 222,
                            }
                        ]
                    }
                return {"items": []}

        client = StubClient()
        cached = APP._market_dashboard(selected_date, client)
        self.assertTrue(cached["cached"])
        self.assertEqual(client.calls, 0)

        fresh = APP._market_dashboard(
            selected_date,
            client,
            force_refresh=True,
        )
        self.assertGreater(client.calls, 0)
        self.assertFalse(fresh["cached"])
        self.assertEqual(fresh["rows"][0]["hour"], 3)
        self.assertEqual(fresh["rows"][0]["smf"], 222)

    def test_active_fullness_normalizes_epias_fields(self):
        class StubClient:
            def _post_json(self, endpoint, body):
                self.endpoint = endpoint
                self.body = body
                return {
                    "body": {
                        "items": [
                            {
                                "damName": "Örnek Barajı",
                                "basinName": "Örnek Havzası",
                                "activeFullnessAmount": 72.5,
                                "date": "2026-07-20T00:00:00+03:00",
                            }
                        ]
                    }
                }

        client = StubClient()
        payload = APP._active_fullness(client)
        self.assertEqual(client.endpoint, "/v1/dams/data/active-fullness")
        self.assertEqual(payload["items"][0]["dam"], "Örnek Barajı")
        self.assertEqual(payload["items"][0]["basin"], "Örnek Havzası")
        self.assertEqual(payload["availableDates"], ["2026-07-20"])

    def test_active_fullness_sends_live_items_to_excel_archive(self):
        class StubClient:
            def _post_json(self, endpoint, body):
                return {
                    "items": [
                        {
                            "damName": "BARAJ A",
                            "basinName": "Havza A",
                            "activeFullnessAmount": 61.5,
                            "date": "2026-08-17T00:00:00+03:00",
                        }
                    ]
                }

        with mock.patch.object(APP, "_archive_live_baraj_items") as archive:
            payload = APP._active_fullness(StubClient(), force_refresh=True)

        archive.assert_called_once_with(payload["items"])

    def test_baraj_archive_reads_pivot_dates_and_raw_basin_mapping(self):
        archive = APP._load_baraj_archive(APP.BARAJ_ARCHIVE_XLSX)
        self.assertEqual(archive["availableDates"][0], "2026-06-24")
        self.assertGreaterEqual(archive["availableDates"][-1], "2026-07-17")
        self.assertEqual(
            len(archive["availableDates"]),
            len(archive["byDate"]),
        )
        self.assertEqual(
            archive["recordCount"],
            sum(len(items) for items in archive["byDate"].values()),
        )
        adatepe = next(
            item
            for item in archive["byDate"]["2026-06-24"]
            if item["dam"] == "ADATEPE"
        )
        self.assertEqual(adatepe["basin"], "Ceyhan")
        self.assertAlmostEqual(adatepe["activeFullnessAmount"], 71.828125)
        self.assertEqual(adatepe["source"], "excel")

    def test_baraj_archive_appends_only_a_new_complete_day(self):
        import openpyxl

        target = ROOT / "tests" / f".baraj-archive-{os.getpid()}.xlsx"
        workbook = openpyxl.Workbook()
        raw = workbook.active
        raw.title = "Aktif Doluluk"
        raw.append(["Tarih", "Havza", "Baraj", "Aktif Doluluk (%)"])
        raw.append([datetime(2026, 8, 4), "Havza A", "BARAJ A", 60])
        raw.append([datetime(2026, 8, 4), "Havza B", "BARAJ B", 40])
        pivot = workbook.create_sheet("Pivot")
        pivot.cell(3, 1).value = "Ortalama Aktif Doluluk (%)"
        pivot.cell(3, 2).value = "Tarih"
        pivot.cell(4, 1).value = "Barajlar"
        pivot.cell(4, 2).value = datetime(2026, 8, 4)
        pivot.cell(4, 3).value = "Genel Ortalama"
        pivot.cell(5, 1).value = "BARAJ A"
        pivot.cell(5, 2).value = 60
        pivot.cell(5, 3).value = 60
        pivot.cell(6, 1).value = "BARAJ B"
        pivot.cell(6, 2).value = 40
        pivot.cell(6, 3).value = 40
        pivot.cell(7, 1).value = "Genel Ortalama"
        pivot.cell(7, 2).value = 50
        pivot.cell(7, 3).value = 50
        workbook.save(target)
        workbook.close()

        rows = [
            {
                "date": "2026-08-04T00:00:00+03:00",
                "basin": "Havza A",
                "dam": "BARAJ A",
                "activeFullnessAmount": 999,
            },
            {
                "date": "2026-08-05T00:00:00+03:00",
                "basin": "Havza A",
                "dam": "BARAJ A",
                "activeFullnessAmount": 59,
            },
            {
                "date": "2026-08-05T00:00:00+03:00",
                "basin": "Havza B",
                "dam": "BARAJ B",
                "activeFullnessAmount": 41,
            },
        ]
        try:
            first = APP.append_active_fullness_days(
                target,
                rows,
                minimum_records=2,
                latest_allowed_date=date(2026, 8, 5),
            )
            second = APP.append_active_fullness_days(
                target,
                rows,
                minimum_records=2,
                latest_allowed_date=date(2026, 8, 5),
            )
            archive = APP._load_baraj_archive(target)
            self.assertTrue(first.updated)
            self.assertEqual(first.added_dates, ("2026-08-05",))
            self.assertEqual(first.added_rows, 2)
            self.assertFalse(second.updated)
            self.assertEqual(archive["availableDates"], ["2026-08-04", "2026-08-05"])
            self.assertEqual(len(archive["byDate"]["2026-08-05"]), 2)
            old_baraj = next(
                item
                for item in archive["byDate"]["2026-08-04"]
                if item["dam"] == "BARAJ A"
            )
            self.assertEqual(old_baraj["activeFullnessAmount"], 60)
        finally:
            target.unlink(missing_ok=True)

    def test_manual_baraj_archive_refresh_replaces_only_latest_day(self):
        import openpyxl

        target = ROOT / "tests" / f".baraj-manual-refresh-{os.getpid()}.xlsx"
        workbook = openpyxl.Workbook()
        raw = workbook.active
        raw.title = "Aktif Doluluk"
        raw.append(["Tarih", "Havza", "Baraj", "Aktif Doluluk (%)"])
        raw.append([datetime(2026, 8, 4), "Havza A", "BARAJ A", 60])
        raw.append([datetime(2026, 8, 4), "Havza B", "BARAJ B", 40])
        raw.append([datetime(2026, 8, 5), "Havza A", "BARAJ A", 58])
        raw.append([datetime(2026, 8, 5), "Havza B", "BARAJ B", 42])
        pivot = workbook.create_sheet("Pivot")
        pivot.cell(3, 1).value = "Ortalama Aktif Doluluk (%)"
        pivot.cell(3, 2).value = "Tarih"
        pivot.cell(4, 1).value = "Barajlar"
        pivot.cell(4, 2).value = datetime(2026, 8, 4)
        pivot.cell(4, 3).value = datetime(2026, 8, 5)
        pivot.cell(4, 4).value = "Genel Ortalama"
        pivot.cell(5, 1).value = "BARAJ A"
        pivot.cell(5, 2).value = 60
        pivot.cell(5, 3).value = 58
        pivot.cell(5, 4).value = 59
        pivot.cell(6, 1).value = "BARAJ B"
        pivot.cell(6, 2).value = 40
        pivot.cell(6, 3).value = 42
        pivot.cell(6, 4).value = 41
        pivot.cell(7, 1).value = "Genel Ortalama"
        pivot.cell(7, 2).value = 50
        pivot.cell(7, 3).value = 50
        pivot.cell(7, 4).value = 50
        workbook.save(target)
        workbook.close()

        refreshed_rows = [
            {
                "date": "2026-08-05T00:00:00+03:00",
                "basin": "Havza A",
                "dam": "BARAJ A",
                "activeFullnessAmount": 61,
            },
            {
                "date": "2026-08-05T00:00:00+03:00",
                "basin": "Havza B",
                "dam": "BARAJ B",
                "activeFullnessAmount": 39,
            },
        ]
        try:
            result = APP.refresh_active_fullness_day(
                target,
                refreshed_rows,
                minimum_records=2,
                latest_allowed_date=date(2026, 8, 5),
            )
            stale_result = APP.refresh_active_fullness_day(
                target,
                [
                    {
                        "date": "2026-08-04T00:00:00+03:00",
                        "basin": "Havza A",
                        "dam": "BARAJ A",
                        "activeFullnessAmount": 999,
                    },
                    {
                        "date": "2026-08-04T00:00:00+03:00",
                        "basin": "Havza B",
                        "dam": "BARAJ B",
                        "activeFullnessAmount": 999,
                    },
                ],
                minimum_records=2,
                latest_allowed_date=date(2026, 8, 5),
            )
            archive = APP._load_baraj_archive(target)

            self.assertTrue(result.updated)
            self.assertFalse(stale_result.updated)
            self.assertEqual(result.refreshed_dates, ("2026-08-05",))
            self.assertEqual(result.refreshed_rows, 2)
            self.assertEqual(result.added_dates, ())
            self.assertEqual(len(archive["byDate"]["2026-08-05"]), 2)
            latest = {
                item["dam"]: item["activeFullnessAmount"]
                for item in archive["byDate"]["2026-08-05"]
            }
            self.assertEqual(latest, {"BARAJ A": 61, "BARAJ B": 39})
            old = {
                item["dam"]: item["activeFullnessAmount"]
                for item in archive["byDate"]["2026-08-04"]
            }
            self.assertEqual(old, {"BARAJ A": 60, "BARAJ B": 40})

            workbook = openpyxl.load_workbook(target, data_only=True)
            try:
                self.assertEqual(workbook["Pivot"].cell(5, 3).value, 61)
                self.assertEqual(workbook["Pivot"].cell(6, 3).value, 39)
                self.assertEqual(workbook["Pivot"].cell(7, 3).value, 50)
            finally:
                workbook.close()
        finally:
            target.unlink(missing_ok=True)

    def test_baraj_archive_date_does_not_call_epias(self):
        class FailClient:
            def _post_json(self, endpoint, body):
                raise AssertionError("Arşiv tarihi EPİAŞ'a gitmemeli")

        payload = APP._baraj_data(FailClient(), "2026-07-17")
        self.assertEqual(payload["source"], "excel")
        self.assertEqual(payload["selectedDate"], "2026-07-17")
        self.assertEqual(payload["sourceLabel"], "Arşiv")
        self.assertGreater(len(payload["items"]), 0)

    def test_baraj_basin_history_merges_archive_and_live_regime(self):
        archive = APP._load_baraj_archive(APP.BARAJ_ARCHIVE_XLSX)
        latest_archive_date = date.fromisoformat(archive["availableDates"][-1])
        live_date = (latest_archive_date + timedelta(days=1)).isoformat()
        latest_archive_date_text = latest_archive_date.isoformat()
        archive_adatepe = next(
            item
            for item in archive["byDate"][latest_archive_date_text]
            if item["dam"] == "ADATEPE"
        )

        class StubClient:
            def _post_json(self, endpoint, body):
                self.endpoint = endpoint
                return {
                    "items": [
                        {
                            "dam": "ADATEPE",
                            "basin": "Ceyhan",
                            "activeFullnessAmount": 1,
                            "date": f"{latest_archive_date_text}T00:00:00+03:00",
                        },
                        {
                            "dam": "ADATEPE",
                            "basin": "Ceyhan",
                            "activeFullnessAmount": 65,
                            "date": f"{live_date}T00:00:00+03:00",
                        }
                    ]
                }

        payload = APP._baraj_basin_history(StubClient())
        ceyhan = next(
            basin for basin in payload["basins"] if basin["name"] == "Ceyhan"
        )
        self.assertEqual(payload["startDate"], "2026-06-24")
        self.assertEqual(payload["endDate"], live_date)
        self.assertEqual(ceyhan["points"][0]["date"], "2026-06-24")
        self.assertEqual(ceyhan["points"][-1]["date"], live_date)
        self.assertEqual(ceyhan["points"][-1]["average"], 65)
        adatepe = next(dam for dam in ceyhan["dams"] if dam["name"] == "ADATEPE")
        overlap_point = next(
            point
            for point in adatepe["points"]
            if point["date"] == latest_archive_date_text
        )
        self.assertEqual(
            overlap_point["activeFullnessAmount"],
            archive_adatepe["activeFullnessAmount"],
        )
        self.assertEqual(overlap_point["source"], "Arşiv")
        self.assertEqual(adatepe["points"][-1]["date"], live_date)
        self.assertEqual(adatepe["points"][-1]["activeFullnessAmount"], 65)
        self.assertEqual(adatepe["points"][-1]["source"], "EPİAŞ")
        self.assertIn(
            ceyhan["analysis"]["regime"],
            {"Azalan rejim", "Yükselen rejim", "Dengeli rejim"},
        )
        self.assertIn(ceyhan["risk"]["level"], {"Yüksek", "Orta", "Düşük"})
        self.assertIn("criticalLevel", ceyhan["risk"])
        self.assertIn("periodComparisons", ceyhan["analysis"])
        self.assertIn("weekly", ceyhan["analysis"]["periodComparisons"])
        self.assertIn("monthly", ceyhan["analysis"]["periodComparisons"])
        self.assertIn("doğrusal eğilim", payload["methodNote"])

    def test_baraj_basin_xlsx_contains_individual_dam_history(self):
        payload = {
            "startDate": "2026-06-24",
            "endDate": "2026-07-21",
            "basins": [
                {
                    "name": "Ceyhan",
                    "points": [
                        {
                            "date": "2026-07-21",
                            "average": 65,
                            "minimum": 60,
                            "maximum": 70,
                            "damCount": 2,
                        }
                    ],
                    "analysis": {
                        "regime": "Azalan rejim",
                        "slopePerDay": -0.2,
                        "periodComparisons": {
                            "weekly": {
                                "available": True,
                                "drop": 1.5,
                                "baselineDate": "2026-07-14",
                            },
                            "monthly": {
                                "available": True,
                                "drop": 4.2,
                                "baselineDate": "2026-06-24",
                            },
                        },
                    },
                    "dams": [
                        {
                            "name": "ADATEPE",
                            "points": [
                                {
                                    "date": "2026-07-21",
                                    "activeFullnessAmount": 65,
                                    "source": "EPİAŞ",
                                }
                            ],
                        }
                    ],
                }
            ],
        }
        workbook = APP._baraj_basin_xlsx(payload, "Ceyhan")
        self.assertTrue(workbook.startswith(b"PK"))
        with zipfile.ZipFile(io.BytesIO(workbook)) as archive:
            workbook_xml = archive.read("xl/workbook.xml").decode("utf-8")
            summary_xml = archive.read("xl/worksheets/sheet1.xml").decode("utf-8")
            detail_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertIn('name="Baraj Dolulukları"', workbook_xml)
            self.assertIn('name="Havza Ortalaması"', workbook_xml)
            self.assertIn("Haftalık düşüş", summary_xml)
            self.assertIn("Aylık düşüş", summary_xml)
            self.assertIn("ADATEPE", detail_xml)
            self.assertIn("2026-07-21", detail_xml)
            self.assertIn("EPİAŞ", detail_xml)

    def test_baraj_dam_xlsx_contains_only_selected_dam_history(self):
        payload = {
            "startDate": "2026-06-24",
            "endDate": "2026-07-21",
            "basins": [{
                "name": "Ceyhan",
                "dams": [
                    {"name": "ADATEPE", "points": [
                        {"date": "2026-06-24", "activeFullnessAmount": 70, "source": "Arşiv"},
                        {"date": "2026-07-21", "activeFullnessAmount": 65, "source": "EPİAŞ"},
                    ]},
                    {"name": "ASLANTAŞ", "points": [
                        {"date": "2026-07-21", "activeFullnessAmount": 55, "source": "EPİAŞ"},
                    ]},
                ],
            }],
        }
        workbook = APP._baraj_dam_xlsx(payload, "Ceyhan", "ADATEPE")
        self.assertTrue(workbook.startswith(b"PK"))
        with zipfile.ZipFile(io.BytesIO(workbook)) as archive:
            summary_xml = archive.read("xl/worksheets/sheet1.xml").decode("utf-8")
            history_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertIn("ADATEPE", summary_xml)
            self.assertIn("2026-06-24", history_xml)
            self.assertIn("2026-07-21", history_xml)
            self.assertNotIn("ASLANTAŞ", history_xml)

    def test_baraj_sorting_supports_fullness_and_turkish_name_order(self):
        items = [
            {"dam": "Zamantı", "activeFullnessAmount": 20},
            {"dam": "Çamlıdere", "activeFullnessAmount": 80},
            {"dam": "Altınkaya", "activeFullnessAmount": None},
        ]
        self.assertEqual(
            [item["dam"] for item in APP._sort_dams(items, "fullness-desc")],
            ["Çamlıdere", "Zamantı", "Altınkaya"],
        )
        self.assertEqual(
            [item["dam"] for item in APP._sort_dams(items, "fullness-asc")],
            ["Zamantı", "Çamlıdere", "Altınkaya"],
        )
        self.assertEqual(
            [item["dam"] for item in APP._sort_dams(items, "name-asc")],
            ["Altınkaya", "Çamlıdere", "Zamantı"],
        )
        self.assertEqual(
            [item["dam"] for item in APP._sort_dams(items, "name-desc")],
            ["Zamantı", "Çamlıdere", "Altınkaya"],
        )
        turkish_i_names = [
            {"dam": "İkizdere", "activeFullnessAmount": 10},
            {"dam": "Ilısu", "activeFullnessAmount": 20},
        ]
        self.assertEqual(
            [
                item["dam"]
                for item in APP._sort_dams(turkish_i_names, "name-asc")
            ],
            ["Ilısu", "İkizdere"],
        )

    def test_baraj_xlsx_contains_summary_and_sorted_list_sheets(self):
        workbook = APP._baraj_xlsx(
            {
                "availableDates": ["2026-07-20"],
                "items": [
                    {
                        "dam": "Zamantı",
                        "basin": "Seyhan",
                        "activeFullnessAmount": 20,
                        "date": "2026-07-20T00:00:00+03:00",
                    },
                    {
                        "dam": "Altınkaya",
                        "basin": "Kızılırmak",
                        "activeFullnessAmount": 70,
                        "date": "2026-07-20T00:00:00+03:00",
                    },
                ],
            },
            "name-asc",
        )
        self.assertTrue(workbook.startswith(b"PK"))
        with zipfile.ZipFile(io.BytesIO(workbook)) as archive:
            workbook_xml = archive.read("xl/workbook.xml").decode("utf-8")
            list_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertIn('name="Özet"', workbook_xml)
            self.assertIn('name="Baraj Listesi"', workbook_xml)
            self.assertLess(list_xml.index("Altınkaya"), list_xml.index("Zamantı"))
            self.assertIn("Aktif doluluk (%)", list_xml)

    def test_market_xlsx_contains_summary_and_hourly_sheets(self):
        dashboard = {
            "date": "2026-07-20",
            "summary": {
                "ptfAverage": 100,
                "smfAverage": 110,
                "ptfAverageByCurrency": {
                    "TRY": 100,
                    "EUR": 4,
                    "USD": 5,
                },
                "smfAverageByCurrency": {"TRY": 110},
                "yalTotal": 6,
                "yatTotal": 9,
            },
            "rows": [
                {
                    "time": "01:00",
                    "ptf": 100,
                    "smf": 110,
                    "ptfByCurrency": {"TRY": 100, "EUR": 4, "USD": 5},
                    "smfByCurrency": {"TRY": 110},
                    "yal": 6,
                    "yat": -9,
                    "direction": "Enerji Açığı",
                }
            ],
        }
        workbook = APP._market_xlsx(dashboard)
        self.assertTrue(workbook.startswith(b"PK"))
        with zipfile.ZipFile(io.BytesIO(workbook)) as archive:
            self.assertIn("xl/worksheets/sheet1.xml", archive.namelist())
            self.assertIn("xl/worksheets/sheet2.xml", archive.namelist())
            workbook_xml = archive.read("xl/workbook.xml").decode("utf-8")
            hourly_xml = archive.read("xl/worksheets/sheet2.xml").decode(
                "utf-8"
            )
            self.assertIn('name="Özet"', workbook_xml)
            self.assertIn('name="Saatlik Veri"', workbook_xml)
            self.assertIn("PTF (EUR/MWh)", hourly_xml)
            self.assertNotIn("SMF (EUR/MWh)", hourly_xml)
            self.assertNotIn("SMF (USD/MWh)", hourly_xml)

    def test_executive_report_builds_branded_html_and_five_sheet_xlsx(self):
        report = {
            "date": "2026-07-22",
            "generatedAt": "2026-07-22T14:20:00+03:00",
            "errors": {},
            "availableModules": ["market", "nextDayPtf", "dams", "production", "consumption"],
            "comparisons": [
                {
                    "key": "selected",
                    "label": "Seçili gün",
                    "date": "2026-07-22",
                    "note": "Rapor tarihi",
                    "ptfAverage": 3000,
                    "smfAverage": 2900,
                    "damAverage": 72.5,
                    "uevmTotal": 1000,
                    "consumptionAverage": 43000,
                    "delta": {},
                },
                {
                    "key": "lastYearSameWeekday",
                    "label": "Geçen yıl aynı hafta-gün",
                    "date": "2025-07-23",
                    "note": "Aynı ISO hafta ve aynı hafta günü",
                    "ptfAverage": 2800,
                    "smfAverage": 2850,
                    "damAverage": 70,
                    "uevmTotal": 930,
                    "consumptionAverage": 41000,
                    "delta": {
                        "ptfAverage": 200,
                        "smfAverage": 50,
                        "damAverage": 2.5,
                        "uevmTotal": 70,
                        "consumptionAverage": 2000,
                    },
                },
            ],
            "damSummary": {
                "count": 2,
                "average": 72.5,
                "highest": {"name": "Gölova", "value": 98.2},
                "lowest": {"name": "Yenice", "value": 46.8},
                "source": "EPİAŞ Şeffaflık Platformu",
                "date": "2026-07-22",
                "previousDate": "2026-07-21",
                "previousAverage": 72.92,
                "dailyChange": -0.42,
            },
            "modules": {
                "market": {
                    "summary": {
                        "ptfAverageByCurrency": {"TRY": 3000},
                        "smfAverage": 2900,
                        "yalTotal": 120,
                        "yatTotal": 80,
                    },
                    "rows": [{"time": "00:00", "ptf": 3000, "smf": 2900, "yal": 5, "yat": 3, "direction": "Enerji Açığı"}],
                },
                "nextDayPtf": {
                    "publication": {"label": "Kesinleşmiş PTF"},
                    "summary": {"ptfAverageByCurrency": {"TRY": 3150}},
                },
                "dams": {"items": [
                    {"dam": "Gölova", "basin": "Yeşilırmak", "activeFullnessAmount": 98.2, "date": "2026-07-22"},
                    {"dam": "Yenice", "basin": "Sakarya", "activeFullnessAmount": 46.8, "date": "2026-07-22"},
                ]},
                "production": {
                    "summary": {"uevmTotal": 1000, "uecmTotal": 980, "difference": 20, "deviationPct": 2.04},
                    "groups": [{"label": "Yenilenebilir", "value": 700, "share": 70}],
                    "series": [{"timestamp": "2026-07-22T00:00:00+03:00", "uevm": 50, "uecm": 49, "renewable": 35, "thermal": 12, "naturalGas": 3}],
                },
                "consumption": {
                    "summary": {"latest": 41000, "average": 43000, "maximum": 48000, "maximumHour": "18:00", "availableHours": 20},
                    "rows": [{"time": "00:00", "consumption": 41000}],
                },
            },
        }
        html = APP._executive_report_html(report, auto_print=True)
        self.assertIn("GÜNLÜK YÖNETİCİ RAPORU", html)
        self.assertIn('data-auto-print="true"', html)
        self.assertIn("PDF / Yazdır", html)
        self.assertIn("↓ XLSX", html)
        self.assertIn("Öne çıkan gelişmeler", html)
        highlights = html.split(
            '<section class="report-card report-highlights">', 1
        )[1]
        self.assertIn("SİSTEM DENGESİ", highlights)
        self.assertIn("Enerji Açığı baskın", highlights)
        self.assertIn("ÜRETİM KARMASI", highlights)
        self.assertIn("Yenilenebilir · %70,0", highlights)
        self.assertIn("+%2,04", html)
        self.assertIn("48.000,00 MWh", html)
        self.assertIn("Günlük değişim", html)
        self.assertIn("↓ −0,42 puan", html)
        self.assertIn("21.07.2026 tarihine göre", html)
        self.assertIn("TAKVİM KIYASI", html)
        self.assertIn("Geçen yıl aynı hafta-gün", html)
        self.assertIn("+200,00 fark", html)
        self.assertNotIn("Takvim kıyasını yükle", html)
        self.assertNotIn("PİYASA ZİRVESİ", highlights)
        self.assertNotIn("KRİTİK DOLULUK", highlights)
        self.assertNotIn("Kaynak durumu", html)
        self.assertNotIn("VERİ KALİTESİ", html)
        workbook = APP._executive_xlsx(report)
        self.assertTrue(workbook.startswith(b"PK"))
        with zipfile.ZipFile(io.BytesIO(workbook)) as archive:
            workbook_xml = archive.read("xl/workbook.xml").decode("utf-8")
            self.assertIn('name="Yönetici Özeti"', workbook_xml)
            self.assertIn('name="Takvim Kıyası"', workbook_xml)
            self.assertIn('name="Piyasa"', workbook_xml)
            self.assertIn('name="Barajlar"', workbook_xml)
            self.assertIn('name="Üretim"', workbook_xml)
            self.assertIn('name="Tüketim"', workbook_xml)

    def test_executive_dashboard_combines_all_modules_for_one_date(self):
        originals = (
            APP._market_dashboard,
            APP._next_day_ptf_dashboard,
            APP._baraj_data,
            APP.URETIM_SERVICE.dashboard,
            APP._consumption_dashboard,
        )
        calls = []
        def fake_market_dashboard(selected, client):
            calls.append(("market", selected))
            baseline = 100 if selected == "2026-07-20" else 90
            return {
                "summary": {
                    "ptfAverageByCurrency": {"TRY": baseline},
                    "smfAverage": baseline - 5,
                },
                "rows": [],
            }

        APP._market_dashboard = fake_market_dashboard
        APP._next_day_ptf_dashboard = lambda selected, client: calls.append(("next", selected)) or {"summary": {}, "rows": []}
        def fake_baraj_data(client, selected):
            calls.append(("dams", selected))
            values = (80, 60) if selected == "2026-07-20" else (79, 59)
            return {
                "items": [
                    {"dam": "A", "activeFullnessAmount": values[0]},
                    {"dam": "B", "activeFullnessAmount": values[1]},
                ],
                "availableDates": ["2026-07-19", "2026-07-20"],
                "sourceLabel": "Arşiv",
                "selectedDate": selected,
            }

        APP._baraj_data = fake_baraj_data
        def fake_production_dashboard(date_range, client):
            selected = date_range.start.isoformat()
            calls.append(("production", selected, date_range.end.isoformat()))
            return {
                "summary": {
                    "uevmTotal": 1000 if selected == "2026-07-20" else 900,
                },
                "series": [],
            }

        APP.URETIM_SERVICE.dashboard = fake_production_dashboard
        APP._consumption_dashboard = lambda selected, client: calls.append(("consumption", selected)) or {"summary": {"average": 40000 if selected == "2026-07-20" else 39000}, "rows": []}
        try:
            report = APP._executive_dashboard("2026-07-20", object())
        finally:
            (
                APP._market_dashboard,
                APP._next_day_ptf_dashboard,
                APP._baraj_data,
                APP.URETIM_SERVICE.dashboard,
                APP._consumption_dashboard,
            ) = originals

        self.assertEqual(report["damSummary"]["average"], 70)
        self.assertEqual(report["damSummary"]["highest"]["name"], "A")
        self.assertEqual(report["damSummary"]["lowest"]["name"], "B")
        self.assertEqual(report["damSummary"]["previousDate"], "2026-07-19")
        self.assertEqual(report["damSummary"]["dailyChange"], 1)
        self.assertEqual(len(report["availableModules"]), 5)
        self.assertEqual(len(report["comparisons"]), 4)
        self.assertEqual(report["comparisons"][0]["label"], "Seçili gün")
        self.assertEqual(report["comparisons"][3]["key"], "lastYearSameWeekday")
        self.assertEqual(report["comparisons"][3]["date"], "2025-07-21")
        self.assertEqual(report["comparisons"][1]["delta"]["ptfAverage"], 10)
        self.assertIn(("production", "2026-07-20", "2026-07-20"), calls)

    def test_command_center_uses_latest_shared_market_payload(self):
        originals = (APP._executive_dashboard, APP._market_dashboard, APP.INVESTING_FX)
        dashboard_options = []
        APP._executive_dashboard = lambda selected, client, **kwargs: dashboard_options.append(kwargs) or {
            "date": selected,
            "generatedAt": "2026-07-22T09:00:00+03:00",
            "modules": {"market": {"rows": [{"time": "09:00", "smf": None}]}},
            "errors": {"market": "Eski piyasa kopyası"},
            "availableModules": [],
            "cached": True,
        }
        APP._market_dashboard = lambda selected, client: {
            "updatedAt": "2026-07-22T06:01:00Z",
            "rows": [{"time": "09:00", "smf": 2450, "smfPublished": True}],
            "freshness": {"smfIncomplete": True, "nextRefreshMs": 45000},
            "cached": True,
        }
        class FakeInvestingFx:
            cache_seconds = 300

            @staticmethod
            def dashboard():
                return {
                    "source": "Investing.com · TCMB",
                    "updatedAt": "2026-07-22T06:02:00Z",
                    "pairs": [
                        {"base": "USD", "value": 40.25, "source": "TCMB"},
                        {"base": "EUR", "value": 46.75, "source": "TCMB"},
                    ],
                    "investingPairs": [
                        {"base": "USD", "value": 40.30, "source": "Investing.com"},
                        {"base": "EUR", "value": 46.85, "source": "Investing.com"},
                    ],
                    "commodities": [
                        {"base": "BRENT", "value": 88.91, "source": "Investing.com"},
                        {"base": "TTF", "value": 60.855, "source": "Investing.com"},
                    ],
                    "availablePairs": 2,
                    "availableInvestingPairs": 2,
                    "availableCommodities": 2,
                    "available": True,
                    "errors": [],
                    "cached": True,
                    "stale": False,
                }
        APP.INVESTING_FX = FakeInvestingFx()
        try:
            report = APP._command_center_dashboard("2026-07-22", object())
        finally:
            APP._executive_dashboard, APP._market_dashboard, APP.INVESTING_FX = originals
        self.assertEqual(report["modules"]["market"]["rows"][0]["smf"], 2450)
        self.assertTrue(report["modules"]["market"]["rows"][0]["smfPublished"])
        self.assertNotIn("market", report["errors"])
        self.assertIn("market", report["availableModules"])
        self.assertEqual(report["modules"]["currency"]["investingPairCount"], 2)
        self.assertFalse(report["modules"]["currency"]["fallback"])
        self.assertIn("currency", report["availableModules"])
        self.assertEqual(dashboard_options, [{"include_comparisons": False}])


class SuiteHttpTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.token = APP.AUTH.create_session("test@ck.local", "TGT-test")
        cls.server = ThreadingHTTPServer(("127.0.0.1", 0), APP.RequestHandler)
        cls.thread = threading.Thread(target=cls.server.serve_forever, daemon=True)
        cls.thread.start()
        cls.base_url = f"http://127.0.0.1:{cls.server.server_port}"

    @classmethod
    def tearDownClass(cls):
        APP.AUTH.revoke(cls.token)
        cls.server.shutdown()
        cls.server.server_close()
        cls.thread.join(timeout=2)

    @classmethod
    def get(cls, path):
        request = urllib.request.Request(
            cls.base_url + path,
            headers={
                "Cookie": f"{APP.AUTH.cookie_name}={cls.token}",
                "Accept": "text/html,application/json",
            },
        )
        with urllib.request.urlopen(request, timeout=5) as response:
            return response.status, response.read(), response.headers

    def test_shared_session_is_visible_to_all_modules(self):
        for path in (
            "/piyasa/api/session",
            "/baraj/api/session",
            "/uretim/api/session",
            "/tuketim/api/session",
        ):
            status, content, _ = self.get(path)
            payload = json.loads(content)
            self.assertEqual(status, 200)
            self.assertTrue(payload["authenticated"])
            self.assertEqual(payload["username"], "test@ck.local")

    def test_epias_guard_api_reports_safe_limits(self):
        status, content, _ = self.get("/api/epias-limits")
        payload = json.loads(content)

        self.assertEqual(status, 200)
        self.assertEqual(payload["configured"]["tgt"]["perMinute"], 10)
        self.assertEqual(payload["configured"]["api"]["perMinute"], 24)
        self.assertEqual(
            payload["configured"]["usernameFailedAttempts"]["maxAttempts"],
            3,
        )
        self.assertTrue(payload["configured"]["singleFlight"]["enabled"])
        self.assertIn("background", payload)
        self.assertIn("taskIntervals", payload["background"])
        self.assertGreaterEqual(payload["background"]["taskIntervals"]["uretim"], 900)

    def test_epias_guard_page_is_available_to_authenticated_users(self):
        status, content, _ = self.get("/epias-koruma")
        html = content.decode("utf-8")

        self.assertEqual(status, 200)
        self.assertIn("Limit ve koruma paneli", html)
        self.assertIn("Arka plan veri toplama", html)
        self.assertIn("/epias-guard.js", html)
        self.assertIn("EPİAŞ koruma", html)

    def test_large_text_assets_are_gzip_compressed_when_requested(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        try:
            connection.request(
                "GET",
                "/baraj/turkiye-havzalari.geojson",
                headers={
                    "Cookie": f"{APP.AUTH.cookie_name}={self.token}",
                    "Accept-Encoding": "gzip",
                },
            )
            response = connection.getresponse()
            compressed = response.read()
            decompressed = gzip.decompress(compressed)
            self.assertEqual(response.status, 200)
            self.assertEqual(response.getheader("Content-Encoding"), "gzip")
            self.assertEqual(response.getheader("Vary"), "Accept-Encoding")
            self.assertLess(len(compressed), len(decompressed))
            self.assertIn(b'"FeatureCollection"', decompressed)
        finally:
            connection.close()

    def test_versioned_assets_use_long_browser_cache(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        try:
            connection.request(
                "GET",
                "/module-suite.css?v=64",
                headers={"Cookie": f"{APP.AUTH.cookie_name}={self.token}"},
            )
            response = connection.getresponse()
            response.read()
            self.assertEqual(response.status, 200)
            self.assertEqual(
                response.getheader("Cache-Control"),
                "public, max-age=31536000, immutable",
            )
        finally:
            connection.close()

    def test_unchanged_static_asset_returns_not_modified(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        headers = {"Cookie": f"{APP.AUTH.cookie_name}={self.token}"}
        try:
            connection.request("GET", "/module-suite.css?v=64", headers=headers)
            first = connection.getresponse()
            first.read()
            etag = first.getheader("ETag")
            self.assertEqual(first.status, 200)
            self.assertTrue(etag)

            connection.request(
                "GET",
                "/module-suite.css?v=64",
                headers={**headers, "If-None-Match": etag},
            )
            second = connection.getresponse()
            content = second.read()
            self.assertEqual(second.status, 304)
            self.assertEqual(content, b"")
            self.assertEqual(second.getheader("ETag"), etag)
        finally:
            connection.close()

    def test_first_visit_redirects_to_single_shared_login(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        connection.request("GET", "/")
        response = connection.getresponse()
        response.read()
        self.assertEqual(response.status, 303)
        self.assertEqual(response.getheader("Location"), "/login")
        connection.close()

    def test_login_endpoint_rate_limits_repeated_failures_by_ip(self):
        original = APP.LOGIN_LIMITER
        limiter = APP.LoginRateLimiter(
            max_attempts=1,
            window_seconds=60,
            block_seconds=45,
        )
        limiter.record_failure("127.0.0.1")
        APP.LOGIN_LIMITER = limiter
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        try:
            body = json.dumps(
                {"username": "test@example.com", "password": "wrong"}
            )
            connection.request(
                "POST",
                "/api/login",
                body=body,
                headers={"Content-Type": "application/json"},
            )
            response = connection.getresponse()
            payload = json.loads(response.read())
            self.assertEqual(response.status, 429)
            self.assertEqual(response.getheader("Retry-After"), "45")
            self.assertIn("Çok fazla", payload["error"])
        finally:
            APP.LOGIN_LIMITER = original
            connection.close()

    def test_login_endpoint_rate_limits_repeated_failures_by_username(self):
        original = APP.LOGIN_USERNAME_LIMITER
        limiter = APP.LoginRateLimiter(
            max_attempts=1,
            window_seconds=10,
            block_seconds=60,
        )
        limiter.record_failure("username:test@example.com")
        APP.LOGIN_USERNAME_LIMITER = limiter
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        try:
            body = json.dumps(
                {"username": "TEST@example.com", "password": "wrong"}
            )
            connection.request(
                "POST",
                "/api/login",
                body=body,
                headers={"Content-Type": "application/json"},
            )
            response = connection.getresponse()
            payload = json.loads(response.read())
            self.assertEqual(response.status, 429)
            self.assertEqual(response.getheader("Retry-After"), "60")
            self.assertIn("Çok fazla", payload["error"])
        finally:
            APP.LOGIN_USERNAME_LIMITER = original
            connection.close()

    def test_successful_http_login_queues_baraj_refresh(self):
        username = "baraj-login@ck.local"

        class StubEpiasClient:
            def __init__(self, *, username, password):
                self.username = username
                self.password = password

            def get_tgt(self):
                return "TGT-baraj-login"

        APP.LOGIN_LIMITER.reset("127.0.0.1")
        APP.LOGIN_USERNAME_LIMITER.reset(f"username:{username}")
        connection = http.client.HTTPConnection(
            "127.0.0.1",
            self.server.server_port,
            timeout=5,
        )
        try:
            with (
                mock.patch.object(APP.URETIM, "EpiasClient", StubEpiasClient),
                mock.patch.object(
                    APP.BACKGROUND_REFRESH,
                    "queue_baraj_refresh",
                ) as queue_refresh,
            ):
                connection.request(
                    "POST",
                    "/api/login",
                    body=json.dumps({"username": username, "password": "secret"}),
                    headers={"Content-Type": "application/json"},
                )
                response = connection.getresponse()
                payload = json.loads(response.read())

            self.assertEqual(response.status, 200)
            self.assertTrue(payload["authenticated"])
            queue_refresh.assert_called_once_with(username, "TGT-baraj-login")
            cookie = response.getheader("Set-Cookie") or ""
            token = cookie.split("=", 1)[1].split(";", 1)[0]
            APP.AUTH.revoke(token)
        finally:
            connection.close()

    def test_opening_baraj_page_queues_archive_refresh(self):
        with mock.patch.object(
            APP.BACKGROUND_REFRESH,
            "queue_baraj_refresh",
        ) as queue_refresh:
            status, content, _ = self.get("/baraj/")

        self.assertEqual(status, 200)
        self.assertIn(b"Baraj Aktif", content)
        queue_refresh.assert_called_once_with("test@ck.local", "TGT-test")

    def test_baraj_excel_refresh_endpoint_uses_authenticated_session(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1",
            self.server.server_port,
            timeout=5,
        )
        result = {
            "ok": True,
            "updated": True,
            "date": "2026-08-17",
            "recordCount": 81,
            "action": "refreshed",
            "reason": "EPİAŞ günü Excel arşivinde yenilendi.",
        }
        try:
            with mock.patch.object(
                APP,
                "_refresh_baraj_archive_now",
                return_value=result,
            ) as refresh_archive:
                connection.request(
                    "POST",
                    "/baraj/api/archive-refresh",
                    headers={
                        "Cookie": f"{APP.AUTH.cookie_name}={self.token}",
                        "Accept": "application/json",
                    },
                )
                response = connection.getresponse()
                payload = json.loads(response.read())

            self.assertEqual(response.status, 200)
            self.assertEqual(payload["date"], "2026-08-17")
            self.assertEqual(payload["recordCount"], 81)
            refresh_archive.assert_called_once()
        finally:
            connection.close()

    def test_login_is_a_clean_shared_portal_page(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        connection.request("GET", "/login")
        response = connection.getresponse()
        html = response.read().decode("utf-8")
        self.assertEqual(response.status, 200)
        self.assertNotIn('class="ck-suite-nav"', html)
        self.assertNotIn('href="/portal-shell.css"', html)
        self.assertNotIn('href="/piyasa/"', html)
        self.assertIn('href="/suite-assets/icon-192.png?v=7"', html)
        self.assertIn('href="/favicon.ico?v=4"', html)
        self.assertIn(
            'href="/suite-assets/apple-touch-icon.png?v=7"',
            html,
        )
        self.assertIn("CK ENERJİ YÖNETİM PANELİ", html)
        self.assertIn("Enerjinin nabzı", html)
        self.assertIn("5</strong><span>entegre panel", html)
        self.assertIn("Sistem Yönü Tahmini", html)
        self.assertIn(
            "piyasa, baraj, üretim, tüketim ve Sistem Yönü Tahmini",
            html,
        )
        self.assertIn("Gerçek Zamanlı Tüketim", html)
        self.assertIn("10 saniyede 3 hatalı", html)
        self.assertIn("<span>Tüketim</span>", html)
        self.assertIn("<span>Sistem Yönü Tahmini</span>", html)
        self.assertIn('class="mobile-topbar"', html)
        self.assertIn('class="mobile-status"', html)
        self.assertIn('class="mobile-panel-tags"', html)
        self.assertIn('id="ckSuiteLoading"', html)
        self.assertIn("Oturum kontrol ediliyor", html)
        self.assertIn("background: transparent", html)
        self.assertIn('src="/suite-loading.js?v=4"', html)
        self.assertIn("ck-suite-loading-failsafe", html)
        self.assertIn("Sunucu uyandırılıyor", html)
        self.assertIn("Panel yükleniyor", html)
        self.assertIn("Oturum kontrol ediliyor", html)
        self.assertNotIn("CK UEVM", html)
        connection.close()

        status, content, _ = self.get("/manifest.webmanifest")
        manifest = json.loads(content)
        self.assertEqual(status, 200)
        self.assertEqual(manifest["name"], "CK Enerji Yönetim Paneli")
        self.assertEqual(
            manifest["icons"][0]["src"],
            "/suite-assets/icon-192.png?v=7",
        )
        self.assertEqual(
            [icon["sizes"] for icon in manifest["icons"]],
            ["192x192", "512x512"],
        )

        status, content, _ = self.get("/sw.js")
        worker = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("ck-enerji-shell-v35", worker)
        self.assertIn("NAVIGATION_TIMEOUT_MS", worker)
        self.assertIn('const LOADING_PATH = "/panel-hazirlaniyor"', worker)
        self.assertIn('const READY_HEADER = "X-CK-Panel"', worker)
        self.assertIn("isCKPanelResponse", worker)
        self.assertIn("if (!useLoadingFallback)", worker)
        self.assertIn("/panel-hazirlaniyor", worker)
        self.assertIn('url.searchParams.has("ck_ready")', worker)
        self.assertIn('url.pathname === "/health"', worker)
        self.assertNotIn("ck-uretim-logo.svg", worker)

        status, content, _ = self.get("/piyasa/")
        piyasa_html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertNotIn('id="ckSuiteLoading"', piyasa_html)
        self.assertNotIn("Panel hazırlanıyor...", piyasa_html)
        self.assertNotIn("/suite-loading.js", piyasa_html)

        status, content, loading_headers = self.get("/panel-hazirlaniyor")
        loading_html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertEqual(loading_headers.get("X-CK-Panel"), "ready")
        self.assertIn("Sunucu uyandırılıyor", loading_html)
        self.assertIn('src="/panel-loading.js?v=6"', loading_html)
        self.assertIn('data-loading-step="0"', loading_html)
        self.assertIn("Sunucu uyandırılıyor", loading_html)

        status, content, _ = self.get("/panel-loading.js")
        loading_script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("AbortController", loading_script)
        self.assertIn('payload?.status === "ok"', loading_script)
        self.assertIn('target.searchParams.set("ck_ready"', loading_script)
        self.assertNotIn("targetResponse", loading_script)
        self.assertIn("window.location.replace(target.href)", loading_script)

        status, content, _ = self.get("/suite-loading.js")
        suite_loading_script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("maxVisibleMs = 12000", suite_loading_script)
        self.assertIn("completeSessionCheck", suite_loading_script)
        self.assertIn('"DOMContentLoaded", finishInitialLoad', suite_loading_script)
        self.assertIn('"pageshow"', suite_loading_script)

        status, content, _ = self.get("/login.js")
        login_script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("completeSessionCheck", login_script)
        self.assertIn('cache: "no-store"', login_script)

        for path in (
            "/suite-assets/icon-192.png",
            "/suite-assets/icon-512.png",
            "/suite-assets/apple-touch-icon.png",
            "/favicon.ico",
        ):
            with self.subTest(path=path):
                status, _, headers = self.get(path)
                self.assertEqual(status, 200)
                self.assertEqual(headers.get_content_type(), "image/png")

        status, content, headers = self.get("/login.css")
        login_css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("input::-ms-reveal", login_css)
        self.assertIn("input::-webkit-credentials-auto-fill-button", login_css)
        self.assertIn("Dar masa", login_css)
        self.assertIn("(min-width: 521px) and (max-width: 900px)", login_css)
        self.assertIn("Mobil giriş: kurumsal kart", login_css)
        self.assertIn(".mobile-status", login_css)
        self.assertIn(".mobile-panel-tags", login_css)
        self.assertIn("backdrop-filter: blur(16px)", login_css)
        self.assertIn("overscroll-behavior: none", login_css)
        self.assertEqual(headers.get_content_type(), "text/css")

    def test_logged_out_page_is_public_and_links_back_to_login(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        connection.request("GET", "/oturum-kapatildi")
        response = connection.getresponse()
        html = response.read().decode("utf-8")
        self.assertEqual(response.status, 200)
        self.assertIn("<title>Oturum Kapatıldı | CK Enerji</title>", html)
        self.assertIn("Oturum başarıyla kapatıldı.", html)
        self.assertIn(
            "Tarayıcınızı kapatabilirsiniz, ya da tekrar giriş yapabilirsiniz.",
            html,
        )
        self.assertIn('class="logout-login-link" href="/login"', html)
        self.assertIn("Tekrar giriş yap", html)
        self.assertNotIn('class="ck-suite-nav"', html)
        connection.close()

        status, content, headers = self.get("/oturum-kapatildi.css")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".logout-card", css)
        self.assertIn(".logout-login-link", css)
        self.assertEqual(headers.get_content_type(), "text/css")

    def test_logout_revokes_session_before_showing_confirmation(self):
        token = APP.AUTH.create_session("logout@ck.local", "TGT-logout")
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        connection.request(
            "POST",
            "/api/logout",
            headers={"Cookie": f"{APP.AUTH.cookie_name}={token}"},
        )
        response = connection.getresponse()
        payload = json.loads(response.read())
        self.assertEqual(response.status, 200)
        self.assertFalse(payload["authenticated"])
        self.assertIn("Max-Age=0", response.getheader("Set-Cookie"))
        self.assertIsNone(APP.AUTH.get_session(token))
        connection.close()

    def test_legacy_login_and_dashboard_paths_are_canonicalized(self):
        cases = {
            "/login/": "/login",
            "/dashboard": "/piyasa/",
            "/panel/": "/piyasa/",
            "/index.html": "/piyasa/",
        }
        for path, expected_location in cases.items():
            with self.subTest(path=path):
                connection = http.client.HTTPConnection(
                    "127.0.0.1", self.server.server_port, timeout=5
                )
                connection.request(
                    "GET",
                    path,
                    headers={
                        "Cookie": f"{APP.AUTH.cookie_name}={self.token}"
                    },
                )
                response = connection.getresponse()
                response.read()
                self.assertEqual(response.status, 303)
                self.assertEqual(
                    response.getheader("Location"),
                    expected_location,
                )
                connection.close()

    def test_authenticated_first_visit_opens_piyasa(self):
        connection = http.client.HTTPConnection(
            "127.0.0.1", self.server.server_port, timeout=5
        )
        connection.request(
            "GET",
            "/",
            headers={"Cookie": f"{APP.AUTH.cookie_name}={self.token}"},
        )
        response = connection.getresponse()
        response.read()
        self.assertEqual(response.status, 303)
        self.assertEqual(response.getheader("Location"), "/piyasa/")
        connection.close()

    def test_piyasa_page_paths_are_mounted_under_piyasa(self):
        status, content, _ = self.get("/piyasa/")
        html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("<title>CK Enerji Piyasa Paneli</title>", html)
        self.assertIn('src="/piyasa/app.js?v=62"', html)
        self.assertIn('value="year-weekday"', html)
        self.assertIn("Önceki yıl · aynı hafta ve gün", html)
        self.assertIn(
            'class="suite-footer" data-suite-footer="piyasa"',
            html,
        )
        self.assertIn('src="/suite-assets/ck-logo-white.png?v=1" alt="CK Enerji"', html)
        self.assertIn('id="piyasaFooterUpdated"', html)
        self.assertIn('href="/piyasa/styles.css?v=41"', html)
        self.assertNotIn('id="detail-yal"', html)
        self.assertNotIn('id="detail-yat"', html)
        self.assertIn('href="/piyasa/" aria-current="page"', html)
        self.assertIn('src="/piyasa-charts.js?v=20"', html)
        self.assertIn("data-chart-engine", html)
        self.assertIn('href="/piyasa-suite.css?v=35"', html)
        self.assertIn('src="/theme-sync.js?v=4"', html)
        self.assertIn(
            'href="/suite-assets/apple-touch-icon.png?v=7"', html
        )
        self.assertNotIn("cdn.jsdelivr.net/npm/apexcharts", html)
        self.assertNotIn("cdn.sheetjs.com", html)
        self.assertIn(
            'id="login-view" class="login-shell" style="display:none!important"',
            html,
        )
        self.assertIn('id="app-view" class="app-shell"', html)
        sidebar = html.split('<aside class="sidebar">', 1)[1].split(
            "</aside>", 1
        )[0]
        self.assertNotIn("<small>Piyasa Paneli</small>", sidebar)
        self.assertIn('class="piyasa-header-actions"', html)
        header_actions = html.split('class="piyasa-header-actions"', 1)[1].split(
            "</header>", 1
        )[0]
        self.assertLess(
            header_actions.index('id="theme-toggle"'),
            header_actions.index('class="user-pill"'),
        )
        self.assertIn("EPİAŞ · EPİAŞ canlı", html)
        self.assertIn('id="menu-close"', html)
        self.assertIn('id="sidebar-overlay"', html)
        self.assertIn('id="sidebar-lock"', html)
        self.assertIn('class="sidebar-lock-button"', html)
        self.assertIn('aria-label="Yan menüyü sabitle"', html)
        self.assertIn('aria-label="Menüyü aç" aria-expanded="false"', html)
        self.assertIn('data-currency="EUR"', html)
        self.assertIn('data-currency="USD"', html)
        self.assertIn('aria-label="PTF fiyat birimi"', html)
        self.assertIn('id="price-chart-title"', html)
        self.assertIn('id="smf-missing-chart-key"', html)
        self.assertIn("SMF yayınlanmadı", html)
        self.assertIn('data-chart-mode="full"', html)
        self.assertIn('data-chart-mode="simple"', html)
        self.assertIn('data-chart-mode="ptf"', html)
        self.assertIn('aria-label="Grafik görünümü"', html)
        self.assertIn('id="currency-note"', html)
        self.assertIn('href="#next-day-ptf"', html)
        self.assertIn('id="next-day-ptf-chart"', html)
        self.assertIn('id="next-day-ptf-status"', html)
        self.assertIn('class="next-day-ptf-schedule"', html)
        self.assertIn("kesinleşmemiş PTF saat 13.00’te", html)
        self.assertIn("kesinleşmiş PTF ise saat 14.00’te", html)

    def test_piyasa_never_shows_its_old_login_screen(self):
        status, content, _ = self.get("/piyasa/app.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertNotIn("show('login')", script)
        self.assertIn("window.location.replace('/login')", script)
        self.assertIn("window.location.replace('/oturum-kapatildi')", script)
        self.assertIn("/piyasa/api/export.xlsx", script)
        self.assertNotIn("window.XLSX", script)
        self.assertIn("ck:themechange", script)
        self.assertIn("EPİAŞ · EPİAŞ canlı", script)
        self.assertIn("fontFamily:SUITE_FONT", script)
        self.assertIn("ptfByCurrency", script)
        self.assertIn("ptfAverageByCurrency", script)
        self.assertIn("smfAverageByCurrency?.TRY", script)
        self.assertNotIn("tryPerUnit", script)
        self.assertNotIn("SMF aynı EPİAŞ gösterge kuruyla", script)
        self.assertIn("ck-market-currency", script)
        self.assertIn("ck-market-v6:", script)
        self.assertIn("smfPublished", script)
        self.assertIn("epias-ptf-direct", script)
        self.assertIn("supportsCurrencyPayload", script)
        self.assertIn("marketRefreshTimer", script)
        self.assertIn("scheduleMarketDataRefresh", script)
        self.assertIn("hasPendingSmf", script)
        self.assertIn("marketCacheTtl", script)
        self.assertIn("smfIncomplete", script)
        self.assertIn("clientCacheMs", script)
        self.assertIn("&refresh=1", script)
        self.assertIn("constrainMarketDate", script)
        self.assertIn("input.max=today", script)
        self.assertIn("Bugünden ileri bir tarih seçilemez.", script)
        self.assertIn("/piyasa/api/next-day-ptf", script)
        self.assertIn("renderNextDayPtf", script)
        self.assertIn("scheduleNextDayPtfRefresh", script)
        self.assertIn("Kesinleşmemiş PTF", script)
        self.assertIn("Kesinleşmiş PTF", script)
        self.assertIn("isSmfUnpublished", script)
        self.assertIn("unpublishedSmfValue", script)
        self.assertIn("publishedSmfValue", script)
        self.assertIn("lastPublishedSmfIndex", script)
        self.assertIn("isTrailingUnpublishedSmf", script)
        self.assertIn("firstTrailingUnpublishedSmfIndex", script)
        self.assertIn("smfBridgeValue", script)
        self.assertIn("index>last", script)
        self.assertIn("SMF geçiş", script)
        self.assertIn("SMF yayınlanmadı", script)
        self.assertIn("hideMarkers:true", script)
        self.assertIn("hideNullInTooltip:true", script)
        self.assertIn("hideInTooltip:true", script)
        self.assertIn("hideInLegend:true", script)
        self.assertNotIn("markerOnly:true", script)
        self.assertIn("colors.push('#8a67e8')", script)
        self.assertIn("colors.push('#d5573b')", script)
        self.assertIn("#d5573b", script)
        self.assertIn("{name:'PTF',data:currentRows.map(r=>priceValue(r,'ptf')),fill:false}", script)
        self.assertIn("{name:'SMF',data:currentRows.map(publishedSmfValue),fill:false}", script)
        self.assertIn("chart:{type:'line',height:320", script)
        self.assertIn("fill:{opacity:0}", script)
        self.assertIn("ck-sidebar-collapsed", script)
        self.assertIn("ck-sidebar-pinned", script)
        self.assertIn("setSidebarPinned", script)
        self.assertIn("suite-sidebar-pinned", script)
        self.assertIn("window.addEventListener('storage'", script)
        self.assertIn("desktopSidebar", script)
        self.assertIn("desktopLock", script)
        self.assertIn("canPinSidebar", script)
        self.assertIn("hover:hover", script)
        self.assertIn("pointer:fine", script)
        self.assertIn("setDesktopSidebarHover", script)
        self.assertIn("suite-sidebar-hovered", script)
        self.assertIn("suite-sidebar-dismissed", script)
        self.assertIn("mouseenter", script)
        self.assertNotIn("sharedMarketDate", script)
        self.assertNotIn("CKDateSync", script)
        self.assertIn("window.CKTracking?.publish", script)
        self.assertIn("desktopSidebarPointerInside", script)
        self.assertIn("clearMarketDashboard", script)
        self.assertIn("ensureChartEngine", script)
        self.assertIn("loadChartEngineScript", script)
        self.assertIn("'/piyasa-charts.js?v=20'", script)
        self.assertIn("'piyasa-charts.js?v=20'", script)
        self.assertNotIn("/piyasa/piyasa/piyasa-charts.js", script)
        self.assertIn("renderMarketCharts", script)
        self.assertIn("ptfSmfCommonHours", script)
        self.assertIn("yalnızca ${commonHours} ortak saatte", script)
        self.assertIn("setMarketConnectionPart('main','error'", script)
        self.assertIn("previousYearSameWeekday", script)
        self.assertIn("isoWeekParts", script)
        self.assertIn("dateFromIsoWeek", script)
        self.assertIn("önceki yılın aynı hafta ve gününe", script)
        self.assertIn("comparisonBaseline", script)
        self.assertIn("URLSearchParams", script)
        self.assertIn("syncMarketUrl", script)
        self.assertIn("applyInitialMarketUrlState", script)
        self.assertIn("compareDate", script)
        self.assertIn("priceChartMode", script)
        self.assertIn("ck-market-chart-mode", script)
        self.assertIn("showComparison", script)
        self.assertIn("priceChartMode==='full'", script)
        self.assertIn("priceChartMode!=='ptf'", script)

        status, content, headers = self.get("/piyasa/styles.css")
        styles = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".chart-key .smf-missing", styles)
        self.assertIn(".chart-key .smf-missing{color:inherit;font-weight:inherit}", styles)
        self.assertIn(".chart-key .smf-missing:before{background:#d5573b;box-shadow:none}", styles)
        self.assertNotIn("html[data-theme=dark] .chart-key .smf-missing{color:#fff}", styles)
        self.assertIn(".chart-mode", styles)
        self.assertIn(".sidebar-lock-button", styles)
        self.assertIn("@media(min-width:1025px) and (hover:hover) and (pointer:fine)", styles)
        self.assertIn("#d5573b", styles)
        self.assertEqual(headers.get_content_type(), "text/css")

    def test_piyasa_local_chart_and_styles_are_served(self):
        status, content, headers = self.get("/piyasa-charts.js")
        self.assertEqual(status, 200)
        chart_script = content.decode("utf-8")
        self.assertIn("window.ApexCharts = LocalEnergyChart", chart_script)
        self.assertIn("window.__ckEnergyCharts", chart_script)
        self.assertIn(
            'if (value === null || value === undefined || value === "") return null',
            chart_script,
        )
        self.assertIn("updateOptions(options)", chart_script)
        self.assertIn("this.renderedHeight", chart_script)
        self.assertIn("tooltip.offsetHeight", chart_script)
        self.assertIn("spaceAbove < tooltipHeight", chart_script)
        self.assertIn("tooltip.dataset.placement", chart_script)
        self.assertIn("const compact = width <= 700", chart_script)
        self.assertIn("const labelStep = Math.max", chart_script)
        self.assertIn("const labelIndexes", chart_script)
        self.assertIn("const visualRatioFor", chart_script)
        self.assertIn("const center = xFor(categoryIndex)", chart_script)
        self.assertNotIn("categoryIndex + 0.5", chart_script)
        self.assertIn("strokeOptions.dashArray", chart_script)
        self.assertIn("item.fill !== false", chart_script)
        self.assertIn("item.markerOnly", chart_script)
        self.assertIn("item.markerRadius", chart_script)
        self.assertIn("item.markerFill", chart_script)
        self.assertIn("item.hideMarkers", chart_script)
        self.assertIn("item.hideInLegend", chart_script)
        self.assertIn("item.hideInTooltip", chart_script)
        self.assertIn("item.hideNullInTooltip", chart_script)
        self.assertIn("item.tooltipFormatter", chart_script)
        self.assertIn("const segments = []", chart_script)
        self.assertIn("currentSegment", chart_script)
        self.assertIn("segments.forEach", chart_script)
        self.assertIn("labelIndexes.length - 1", chart_script)
        self.assertIn("closestDistance", chart_script)
        self.assertIn("xaxisLabels.compactFontSize", chart_script)
        self.assertIn('class: "energy-chart-x-label"', chart_script)
        self.assertIn(
            "index % labelStep === 0 || index === categories.length - 1",
            chart_script,
        )
        self.assertEqual(headers.get_content_type(), "text/javascript")

    def test_all_panels_include_shared_chart_fullscreen_controls(self):
        for path in ("/piyasa/", "/baraj/", "/uretim/", "/tuketim/"):
            with self.subTest(path=path):
                status, content, _ = self.get(path)
                html = content.decode("utf-8")
                self.assertEqual(status, 200)
                self.assertIn('href="/chart-fullscreen.css?v=6"', html)
                self.assertIn('src="/chart-fullscreen.js?v=6"', html)

        status, content, headers = self.get("/chart-fullscreen.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("#hourly-data", script)
        self.assertIn("#basinRegimeChart", script)
        self.assertIn(".trend-panel", script)
        self.assertIn("#consumptionChart", script)
        self.assertNotIn("#consumptionForecastChart", script)
        self.assertIn("suite-chart-maximized", script)
        self.assertIn("suite-chart-viewer-canvas", script)
        self.assertIn("viewerCanvas.replaceChildren", script)
        self.assertIn("setApexHeight", script)
        self.assertIn("forceApexDomHeight", script)
        self.assertIn("scheduleApexResize", script)
        self.assertIn("window.__ckEnergyCharts", script)
        self.assertNotIn('panel: "#direction-chart"', script)
        self.assertIn('event.key === "Escape"', script)
        self.assertEqual(headers.get_content_type(), "text/javascript")

        status, content, headers = self.get("/chart-fullscreen.css")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".suite-chart-fullscreen-button", css)
        self.assertIn(".suite-chart-maximized", css)
        self.assertIn(".suite-chart-viewer-canvas", css)
        self.assertIn('[data-chart-kind="apex"]', css)
        self.assertIn("height: 100% !important", css)
        self.assertIn("height: 100dvh !important", css)
        self.assertIn(".energy-chart > svg", css)
        self.assertIn(".energy-chart-legend", css)
        self.assertIn(
            "body.suite-chart-fullscreen-open.ck-suite-uretim .tooltip",
            css,
        )
        self.assertIn("z-index: 200020", css)
        self.assertIn("env(safe-area-inset-top", css)
        self.assertIn('html[data-theme="dark"]', css)
        self.assertEqual(headers.get_content_type(), "text/css")

        status, content, headers = self.get("/piyasa-suite.css")
        self.assertEqual(status, 200)
        css = content.decode("utf-8")
        self.assertIn(".ck-suite-piyasa .metric-grid", css)
        self.assertIn("repeat(12, minmax(0, 1fr))", css)
        self.assertIn(".ck-suite-piyasa #direction-chart", css)
        self.assertIn("overflow: hidden !important", css)
        self.assertIn(".ck-suite-piyasa .piyasa-header-actions", css)
        self.assertIn(".ck-suite-piyasa .menu-close", css)
        self.assertIn(".ck-suite-piyasa .sidebar-lock-button", css)
        self.assertIn(
            "@media (min-width: 1025px) and (hover: hover) and (pointer: fine)",
            css,
        )
        self.assertIn(".ck-suite-piyasa.suite-sidebar-pinned .sidebar-lock-button", css)
        self.assertIn("minmax(220px, 1.2fr)", css)
        self.assertIn("grid-template-columns: 42px minmax(0, 1fr) 42px", css)
        self.assertIn(".ck-suite-piyasa .last-update", css)
        self.assertIn("display: none", css)
        self.assertIn("max-width: calc(100% - 16px)", css)
        self.assertIn("transform: none", css)
        self.assertIn("transform: translateX(-105%)", css)
        self.assertIn("translateX(calc(-100% + 22px))", css)
        self.assertIn("suite-sidebar-hovered", css)
        self.assertIn("suite-sidebar-dismissed", css)
        self.assertIn("z-index: 100030", css)
        self.assertIn("display: grid !important", css)
        self.assertIn(".ck-suite-piyasa #xlsx-button", css)
        self.assertIn(".ck-suite-piyasa #today-button", css)
        self.assertIn(".ck-suite-piyasa .metric-head i", css)
        self.assertIn("padding: 8px 0 16px", css)
        self.assertIn(
            ".ck-suite-piyasa.suite-sidebar-collapsed .main-content",
            css,
        )
        self.assertIn(".ck-suite-piyasa #quantity-chart", css)
        self.assertIn("margin-inline: -10px", css)
        self.assertIn(".ck-suite-piyasa .direction-hour strong", css)
        self.assertIn("writing-mode: horizontal-tb", css)
        self.assertIn(".ck-suite-piyasa .direction-hour span", css)
        self.assertIn(".ck-suite-piyasa .table th:last-child", css)
        self.assertIn('input[type="date"]::-webkit-calendar-picker-indicator', css)
        self.assertIn(".date-nav button:disabled", css)
        self.assertIn("border-color: #47658f", css)
        self.assertIn("min-width: 960px", css)
        self.assertIn("min-width: 780px", css)
        self.assertIn("padding-inline: 12px", css)
        self.assertIn("table-layout: fixed", css)
        self.assertIn("width: calc(100% / 6)", css)
        self.assertIn("text-align: left", css)
        self.assertIn("white-space: nowrap", css)
        self.assertIn("display: inline-flex", css)
        self.assertIn('html[data-theme="dark"] .ck-suite-piyasa .direction.up', css)
        self.assertIn('html[data-theme="dark"] .ck-suite-piyasa .direction.down', css)
        self.assertIn(
            'html[data-theme="dark"] .ck-suite-piyasa .direction-hour.missing',
            css,
        )
        self.assertIn('html[data-theme="dark"] .ck-suite-piyasa .menu-button', css)
        self.assertIn(".ck-suite-piyasa h1", css)
        self.assertIn("font-size: clamp(25px, 2.2vw, 36px)", css)
        self.assertIn(".live-dot.warning i", css)
        self.assertEqual(headers.get_content_type(), "text/css")

    def test_baraj_inline_api_paths_are_mounted_under_baraj(self):
        status, content, _ = self.get("/baraj/")
        html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(
            "<title>CK Enerji Baraj Doluluk Paneli</title>",
            html,
        )
        self.assertIn("/baraj/api/active-fullness", html)
        self.assertIn("/baraj/api/archive-refresh", html)
        self.assertIn('id="excelRefreshButton" class="btn btn-primary"', html)
        self.assertIn("refreshExcelArchive", html)
        self.assertIn("payload.error || payload.reason", html)
        self.assertIn('href="/baraj/" aria-current="page"', html)
        self.assertNotIn("cdn.jsdelivr.net", html)
        self.assertNotIn("@tabler/core", html)
        self.assertNotIn("showLogin();", html)
        self.assertIn("window.location.replace('/oturum-kapatildi')", html)
        self.assertIn(
            'class="page page-center login-screen" style="display:none!important"',
            html,
        )
        self.assertIn('id="dashboard" class="page"', html)
        self.assertIn('id="baraj-summary"', html)
        self.assertIn('id="baraj-list"', html)
        self.assertIn('id="baraj-regime"', html)
        self.assertIn('id="baraj-regime-analysis"', html)
        self.assertIn('class="baraj-analysis-section mb-3"', html)
        self.assertIn('class="baraj-regime-overview-card"', html)
        self.assertIn('id="basin-risk"', html)
        self.assertIn('id="basinRiskBody"', html)
        self.assertIn("Havzaların takip önceliği", html)
        self.assertIn("Neden bu sırada?", html)
        self.assertIn("Bu sıralama nasıl hesaplanıyor?", html)
        self.assertIn("resmî bir risk sınıflandırması değil", html)
        self.assertIn("baraj-risk-reason", html)
        self.assertIn('class="baraj-risk-summary"', html)
        self.assertIn("baraj-risk-change", html)
        self.assertIn("renderBasinRisks", html)
        self.assertIn("7 gün düşüş", html)
        self.assertIn("30 gün düşüş", html)
        self.assertIn('id="basinWeeklyDrop"', html)
        self.assertIn('id="basinMonthlyDrop"', html)
        self.assertIn("Dün / 7 gün / 30 gün", html)
        self.assertIn("periodComparisons.daily", html)
        self.assertIn("periodDropText", html)
        self.assertIn("periodComparisons.weekly", html)
        self.assertIn("%30 kritik seviye", html)
        self.assertIn('id="baraj-map"', html)
        self.assertIn('id="turkeyMapShape"', html)
        self.assertIn('id="basinMapDams"', html)
        self.assertIn("baraj-map-legend", html)
        self.assertIn("Harita doluluk renkleri", html)
        self.assertIn("fullness-low", html)
        self.assertIn("fullness-mid", html)
        self.assertIn("fullness-high", html)
        self.assertIn("basinFullnessColor", html)
        self.assertIn("path.style.setProperty('--basin-fill'", html)
        self.assertIn("fullness-known", html)
        self.assertIn("polygonLabelPoint(feature)", html)
        self.assertIn("collisionSafeLabelPoint", html)
        self.assertIn("pointInsideRing", html)
        self.assertIn("occupiedLabels", html)
        self.assertIn("BASIN_LABEL_ANCHORS", html)
        self.assertIn("'Marmara':[29.02, 40.96]", html)
        self.assertIn("BASIN_LABEL_FONT_BOOST", html)
        self.assertIn("geometryPath(feature.geometry)", html)
        self.assertIn("/baraj/turkiye-havzalari.geojson", html)
        self.assertIn("baraj-basin-shape", html)
        self.assertNotIn("BASIN_MAP_COORDINATES", html)
        self.assertNotIn("TURKEY_MAP_LAKES", html)
        self.assertNotIn("baraj-map-lakes", html)
        self.assertNotIn("baraj-map-coordinate-grid", html)
        self.assertNotIn("function layoutBasinMapLabels()", html)
        self.assertIn("group.append(path, label); svg.append(group)", html)
        self.assertIn("renderBasinMap(selected || null)", html)
        self.assertNotIn("Natural Earth 1:110m", html)
        self.assertIn("T.C. Tarım ve Orman Bakanlığı CBS", html)
        self.assertIn('id="baraj-compare"', html)
        self.assertIn('data-compare-mode="date"', html)
        self.assertIn('id="compareFirst"', html)
        self.assertIn("comparisonMode === 'date' ? (items[1] || items[0])", html)
        self.assertIn("comparisonMode === 'date' ? items[0]", html)
        self.assertIn('id="compareSecond"', html)
        self.assertIn('id="basinSelect"', html)
        self.assertIn('id="basinMapPanelToggle"', html)
        self.assertIn("$('basinSelect').addEventListener('change', renderBasinHistory)", html)
        self.assertNotIn("setBasinMapPanel(true)", html)
        self.assertIn('class="baraj-basin-toolbar"', html)
        self.assertIn('id="basinToolbarPeriod"', html)
        self.assertIn("Havzalar alınamadı", html)
        self.assertIn("/baraj/api/basin-history", html)
        self.assertIn('id="basinDamTableBody"', html)
        self.assertIn('id="basinXlsxButton"', html)
        self.assertIn("/baraj/api/basin-export.xlsx?basin=", html)
        self.assertIn('id="baraj-dam-history"', html)
        self.assertIn('id="basinDamSelect"', html)
        self.assertIn('id="basinDamHistoryChart"', html)
        self.assertIn('id="basinDamHistoryBody"', html)
        self.assertIn('id="basinDamWeeklyChange"', html)
        self.assertIn('id="basinDamMonthlyChange"', html)
        self.assertIn("damPeriodComparison(points, 7)", html)
        self.assertIn("damPeriodComparison(points, 30)", html)
        self.assertIn('id="basinDamXlsxButton"', html)
        self.assertIn("/baraj/api/dam-export.xlsx?basin=", html)
        self.assertIn('id="sortSelect"', html)
        self.assertIn('id="barajXlsxButton"', html)
        self.assertIn('XLSX İNDİR</button>', html)
        self.assertNotIn('data-download-menu="baraj-list"', html)
        self.assertIn('id="barajDateSelect"', html)
        self.assertIn("initialBarajUrlState", html)
        self.assertIn("barajUrlState", html)
        self.assertIn("syncBarajUrlState", html)
        self.assertIn("applyInitialBarajUrlState", html)
        self.assertIn("BARAJ_CLIENT_CACHE_PREFIX", html)
        self.assertIn("readBarajClientCache", html)
        self.assertIn("load(true); loadBasinHistory()", html)
        self.assertIn("params.set('basin', basin)", html)
        self.assertIn("params.set('sort', sort)", html)
        self.assertNotIn("CKDateSync", html)
        self.assertIn("window.CKTracking?.publish", html)
        self.assertIn("option.textContent = displayDate(value)", html)
        self.assertNotIn("archiveDates.has(value) ? 'Arşiv' : 'EPİAŞ'", html)
        self.assertNotIn('id="barajDataSource"', html)
        self.assertIn("/baraj/api/export.xlsx?", html)
        self.assertIn("Doluluk: yüksekten düşüğe", html)
        self.assertIn("Baraj adı: A–Z", html)
        self.assertIn("Basit ort. doluluk", html)
        self.assertIn("BASİT ORTALAMA AKTİF DOLULUK", html)
        self.assertIn("chart.clientHeight || 390", html)
        self.assertIn("let yMin = Math.max(0", html)
        self.assertIn("pointsByDate", html)
        self.assertIn("sort((first, second) => first.timestamp - second.timestamp)", html)

        self.assertIn("clearBarajOverview", html)
        self.assertIn("setConnectionState('error'", html)
        self.assertIn("Deneysel tükenme", html)
        self.assertIn('class="suite-sidebar"', html)
        self.assertIn("data-suite-theme-toggle", html)
        self.assertIn("EPİAŞ · EPİAŞ canlı", html)
        self.assertIn('class="suite-menu-close"', html)
        self.assertIn('href="/module-suite.css?v=64"', html)
        self.assertIn('src="/module-suite.js?v=17"', html)
        self.assertIn('src="/theme-sync.js?v=4"', html)
        self.assertNotIn('src="/date-sync.js', html)
        self.assertNotIn("tracking-center.js", html)
        self.assertNotIn("data-tracking-toggle", html)
        self.assertIn(
            'href="/suite-assets/apple-touch-icon.png?v=7"', html
        )
        self.assertIn(
            'class="suite-footer" data-suite-footer="baraj"',
            html,
        )

        status, content, _ = self.get("/baraj/manifest.webmanifest")
        manifest = json.loads(content)
        self.assertEqual(status, 200)
        self.assertEqual(
            [icon["sizes"] for icon in manifest["icons"]],
            ["192x192", "512x512"],
        )
        self.assertIn('id="barajFooterUpdated"', html)
        self.assertIn('href="/portal-shell.css?v=11"', html)
        self.assertIn('class="suite-command-toggle"', html)
        self.assertIn('class="suite-command-menu"', html)
        self.assertIn('src="/command-center.js?v=3"', html)

    def test_baraj_serves_official_basin_geojson_locally(self):
        status, content, headers = self.get("/baraj/turkiye-havzalari.geojson")
        self.assertEqual(status, 200)
        self.assertEqual(headers.get_content_type(), "application/geo+json")
        payload = json.loads(content.decode("utf-8"))
        self.assertEqual(payload["type"], "FeatureCollection")
        self.assertEqual(len(payload["features"]), 25)
        names = {
            feature["properties"]["HAVZA_ADI"]
            for feature in payload["features"]
        }
        self.assertIn("Kızılırmak", names)
        self.assertIn("Fırat - Dicle", names)
        self.assertTrue(
            all(
                feature["geometry"]["type"] in {"Polygon", "MultiPolygon"}
                for feature in payload["features"]
            )
        )

    def test_baraj_archive_date_is_served_from_pivot(self):
        status, content, _ = self.get(
            "/baraj/api/active-fullness?date=2026-06-24"
        )
        payload = json.loads(content)
        self.assertEqual(status, 200)
        self.assertEqual(payload["source"], "excel")
        self.assertEqual(payload["selectedDate"], "2026-06-24")
        self.assertEqual(payload["sourceLabel"], "Arşiv")
        self.assertEqual(
            payload["archiveDates"],
            APP._baraj_archive()["availableDates"],
        )
        self.assertTrue(
            any(item["dam"] == "ADATEPE" for item in payload["items"])
        )

    def test_baraj_archive_date_can_be_downloaded_as_xlsx(self):
        status, content, headers = self.get(
            "/baraj/api/export.xlsx?sort=name-asc&date=2026-06-24"
        )
        self.assertEqual(status, 200)
        self.assertIn(
            "ck-enerji-baraj-aktif-2026-06-24.xlsx",
            headers["Content-Disposition"],
        )
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            summary_xml = archive.read("xl/worksheets/sheet1.xml").decode(
                "utf-8"
            )
            list_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertIn("Arşiv", summary_xml)
            self.assertIn("ADATEPE", list_xml)

    def test_selected_basin_history_can_be_downloaded_as_xlsx(self):
        original_history = APP._baraj_basin_history
        APP._baraj_basin_history = lambda _client: {
            "startDate": "2026-06-24",
            "endDate": "2026-07-21",
            "basins": [
                {
                    "name": "Ceyhan",
                    "points": [
                        {
                            "date": "2026-07-21",
                            "average": 65,
                            "minimum": 65,
                            "maximum": 65,
                            "damCount": 1,
                        }
                    ],
                    "analysis": {"regime": "Azalan rejim"},
                    "dams": [
                        {
                            "name": "ADATEPE",
                            "points": [
                                {
                                    "date": "2026-07-21",
                                    "activeFullnessAmount": 65,
                                    "source": "EPİAŞ",
                                }
                            ],
                        }
                    ],
                }
            ],
        }
        try:
            status, content, headers = self.get(
                "/baraj/api/basin-export.xlsx?basin=Ceyhan"
            )
        finally:
            APP._baraj_basin_history = original_history

        self.assertEqual(status, 200)
        self.assertIn(
            "ck-enerji-havza-baraj-doluluk.xlsx",
            headers["Content-Disposition"],
        )
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            detail_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertIn("ADATEPE", detail_xml)

    def test_selected_dam_history_can_be_downloaded_as_xlsx(self):
        original_history = APP._baraj_basin_history
        APP._baraj_basin_history = lambda _client: {
            "startDate": "2026-06-24",
            "endDate": "2026-07-21",
            "basins": [{
                "name": "Ceyhan",
                "dams": [{"name": "ADATEPE", "points": [
                    {"date": "2026-06-24", "activeFullnessAmount": 70, "source": "Arşiv"},
                    {"date": "2026-07-21", "activeFullnessAmount": 65, "source": "EPİAŞ"},
                ]}],
            }],
        }
        try:
            status, content, headers = self.get(
                "/baraj/api/dam-export.xlsx?basin=Ceyhan&dam=ADATEPE"
            )
        finally:
            APP._baraj_basin_history = original_history
        self.assertEqual(status, 200)
        self.assertIn("ck-enerji-baraj-doluluk-gecmisi.xlsx", headers["Content-Disposition"])
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            history_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertIn("2026-06-24", history_xml)
            self.assertIn("2026-07-21", history_xml)

    def test_baraj_xlsx_export_uses_selected_sort(self):
        original_active_fullness = APP._active_fullness
        APP._active_fullness = lambda _client: {
            "availableDates": ["2026-07-20"],
            "items": [
                {
                    "dam": "Zamantı",
                    "basin": "Seyhan",
                    "activeFullnessAmount": 20,
                    "date": "2026-07-20",
                },
                {
                    "dam": "Altınkaya",
                    "basin": "Kızılırmak",
                    "activeFullnessAmount": 70,
                    "date": "2026-07-20",
                },
            ],
        }
        try:
            status, content, headers = self.get(
                "/baraj/api/export.xlsx?sort=name-asc"
            )
        finally:
            APP._active_fullness = original_active_fullness

        self.assertEqual(status, 200)
        self.assertEqual(
            headers.get_content_type(),
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
        self.assertIn(
            "ck-enerji-baraj-aktif-2026-07-20.xlsx",
            headers["Content-Disposition"],
        )
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            list_xml = archive.read("xl/worksheets/sheet2.xml").decode("utf-8")
            self.assertLess(list_xml.index("Altınkaya"), list_xml.index("Zamantı"))

    def test_consumption_page_and_assets_are_mounted_under_tuketim(self):
        status, content, _ = self.get("/tuketim/")
        html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("<title>CK Enerji Tüketim Paneli</title>", html)
        self.assertIn('class="ck-suite-page ck-suite-tuketim"', html)
        self.assertIn('href="/tuketim/" aria-current="page"', html)
        self.assertIn('href="/tuketim/consumption.css?v=13"', html)
        self.assertIn('src="/tuketim/consumption.js?v=19"', html)
        self.assertIn('id="consumptionCompareDate" type="date"', html)
        self.assertNotIn('id="consumptionCompare"', html)
        self.assertIn('id="consumption-summary"', html)
        self.assertIn('id="consumption-chart"', html)
        self.assertIn('id="consumption-forecast"', html)
        self.assertIn('class="consumption-chart-forecast"', html)
        self.assertIn("TAHMİN ÖZETİ", html)
        self.assertIn("SABİTLENEN TAHMİN", html)
        self.assertIn("Saatlik Tüketim ve Tahmin", html)
        self.assertIn("Seçilen gün tahmini", html)
        self.assertNotIn("<h2>Saatlik Tüketim Tahmini</h2>", html)
        self.assertNotIn('id="consumptionForecastChart"', html)
        self.assertIn('id="consumption-table"', html)
        self.assertIn('data-suite-theme-toggle', html)
        self.assertIn('class="suite-menu-close"', html)
        self.assertIn('id="sidebar-lock"', html)
        self.assertIn('suite-sidebar-lock-button sidebar-lock-button', html)
        self.assertIn('href="/module-suite.css?v=64"', html)
        self.assertIn('src="/module-suite.js?v=17"', html)
        self.assertIn(
            'class="suite-footer" data-suite-footer="tuketim"',
            html,
        )

        status, content, headers = self.get("/tuketim/consumption.css")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".consumption-kpis", css)
        self.assertIn(".consumption-hours", css)
        self.assertIn(".consumption-forecast-summary", css)
        self.assertIn(".consumption-forecast-meta", css)
        self.assertIn(".consumption-chart-forecast", css)
        self.assertIn(".consumption-legend .forecast-today", css)
        self.assertIn(".consumption-kpi > div i", css)
        self.assertIn("background: rgba(55, 113, 221, .14)", css)
        self.assertIn('html[data-theme="dark"]', css)
        self.assertIn("@media (max-width: 820px)", css)
        self.assertEqual(headers.get_content_type(), "text/css")

        status, content, headers = self.get("/tuketim/consumption.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("/tuketim/api/data?date=", script)
        self.assertIn("/tuketim/api/forecast?baseDate=", script)
        self.assertIn("&targetDate=", script)
        self.assertIn("tomorrowForecast", script)
        self.assertIn("lockedForecastRange", script)
        self.assertIn("1 SAAT 10 DK ÖNCE SABİT", script)
        self.assertIn("chartColors.push(\"#ff7a1a\")", script)
        self.assertIn("legend: {show: false}", script)
        self.assertIn("/tuketim/api/export.xlsx?date=", script)
        self.assertIn("input.max = today", script)
        self.assertIn("ck:themechange", script)
        self.assertNotIn("CKDateSync", script)
        self.assertIn("window.CKTracking?.publish", script)
        self.assertIn("clearConsumptionDashboard", script)
        self.assertIn('setConnectionState("error"', script)
        self.assertIn("comparisonFailed", script)
        self.assertIn("renderForecast", script)
        self.assertIn('compactFontSize: "7.2"', script)
        self.assertIn("labels: {step: 3", script)
        self.assertIn("CLIENT_CACHE_PREFIX", script)
        self.assertIn("cachedApi", script)
        self.assertIn("sessionStorage.setItem", script)
        self.assertEqual(headers.get_content_type(), "text/javascript")

    def test_system_direction_forecast_page_is_mounted(self):
        status, content, _ = self.get("/sistem-yonu-tahmini/")
        html = content.decode("utf-8")

        self.assertEqual(status, 200)
        self.assertIn("<title>CK Enerji Sistem Yönü Tahmini</title>", html)
        self.assertIn('class="ck-suite-page ck-suite-sistem"', html)
        self.assertIn(
            'href="/sistem-yonu-tahmini/" class="suite-nav-forecast" aria-current="page"',
            html,
        )
        self.assertIn("Sistem Yönü Tahmini", html)
        self.assertIn('class="suite-sidebar"', html)
        self.assertIn('data-suite-scroll="#system-timeline"', html)
        self.assertIn('data-suite-scroll="#system-samples"', html)
        self.assertIn('data-suite-scroll="#system-validation"', html)
        self.assertIn('id="system-validation"', html)
        self.assertNotIn('class="validation-toolbar"', html)
        self.assertNotIn('id="validationDate"', html)
        self.assertNotIn('id="validationRefresh"', html)
        self.assertNotIn('id="validationNote"', html)
        self.assertIn('id="historicalForecastDate"', html)
        self.assertIn('id="historicalForecastPrev"', html)
        self.assertIn('id="historicalForecastNext"', html)
        self.assertIn('id="historicalForecastRun"', html)
        self.assertIn('id="historicalForecastLive"', html)
        self.assertLess(
            html.index("01 / SAATLİK TAHMİN"),
            html.index('id="validationMount"'),
        )
        self.assertIn("02 / TAHMİN DOĞRULAMA", html)
        self.assertIn("03 / OLASILIK", html)
        self.assertIn("04 / METOT", html)
        self.assertIn("05 / GEÇMİŞ ÖRNEKLER", html)
        self.assertIn("Geçmiş tahmin testi", html)
        self.assertIn('id="validationDirectionTimeline"', html)
        self.assertIn('id="validationHourlyQuality"', html)
        self.assertIn('id="validationDailyQuality"', html)
        self.assertIn("TAHMİN BAŞARI LABORATUVARI", html)
        self.assertIn(">Gerçekleşen saat</span>", html)
        self.assertNotIn('class="observed" hidden><i>', html)
        self.assertIn('id="validationHourLatest"', html)
        self.assertIn('data-suite-user-email', html)
        self.assertIn('id="sidebar-lock"', html)
        self.assertIn('href="/module-suite.css?v=64"', html)
        self.assertIn('href="/system-direction-forecast.css?v=22"', html)
        self.assertIn('src="/module-suite.js?v=17"', html)
        self.assertIn('src="/system-direction-forecast.js?v=22"', html)
        self.assertIn('class="suite-footer" data-suite-footer="sistem"', html)
        self.assertIn('id="suiteFooterUpdated"', html)
        self.assertNotIn('class="system-header-actions"', html)
        self.assertNotIn('class="consumption-live-status"', html)
        self.assertNotIn("EPİAŞ canlı veri", html)

        status, content, headers = self.get("/system-direction-forecast.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn('/sistem-yonu-tahmini/api/validation', script)
        self.assertIn('params.set("includeValidation", "1")', script)
        self.assertIn("sessionStorage", script)
        self.assertIn("renderValidation", script)
        self.assertIn("renderValidationDirection", script)
        self.assertIn("renderQualityMetrics", script)
        self.assertIn("focusLatestValidationHour", script)
        self.assertIn("validation-direction-hour", script)
        self.assertIn("actual-lane", script)
        self.assertIn("forecast-lane", script)
        self.assertIn("activeValidationDate", script)
        self.assertNotIn('$(\"validationDate\")', script)
        self.assertNotIn('$(\"validationRefresh\")', script)
        self.assertIn('params.set("historical", "1")', script)
        self.assertIn("historicalForecastDate", script)
        self.assertIn("historicalForecastPrev", script)
        self.assertIn("historicalForecastNext", script)
        self.assertIn("shiftIsoDate", script)
        self.assertIn("activeHistoricalDate", script)
        self.assertIn("isoDaysAgo(365)", script)
        self.assertIn("validationMount.replaceWith(validation)", script)

        self.assertIn("await loadValidation(force)", script)
        self.assertIn("contextTooltip", script)
        self.assertIn("contextInputs", script)
        self.assertIn('["Sapma ML", weights.learning]', script)
        self.assertEqual(headers.get_content_type(), "text/javascript")

        status, content, headers = self.get(
            "/system-direction-forecast.css"
        )
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".validation-direction-track", css)
        self.assertIn(".validation-direction-hour.miss", css)
        self.assertIn(".validation-direction-marker.forecast", css)
        self.assertIn(".validation-quality-cards", css)
        self.assertIn(".history-picker-controls", css)
        self.assertIn(".history-date-nav", css)
        self.assertIn(".history-date-step", css)
        self.assertIn(
            "grid-template-columns: minmax(0, 1fr) max-content max-content",
            css,
        )
        self.assertIn("box-sizing: border-box", css)
        self.assertIn("scroll-snap-type: x mandatory", css)
        self.assertIn(
            'html[data-theme="dark"] .validation-direction-marker.actual.waiting',
            css,
        )
        self.assertEqual(headers.get_content_type(), "text/css")

    def test_android_app_user_agent_receives_native_shell_styles(self):
        request = urllib.request.Request(
            self.base_url + "/piyasa/",
            headers={
                "Cookie": f"{APP.AUTH.cookie_name}={self.token}",
                "User-Agent": "Mozilla/5.0 CKEnerjiAndroid/3.0",
                "Accept": "text/html",
            },
        )
        with urllib.request.urlopen(request, timeout=5) as response:
            html = response.read().decode("utf-8")

        self.assertIn('href="/android-app.css?v=19"', html)
        self.assertIn('class="sidebar"', html)
        status, content, headers = self.get("/android-app.css?v=19")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".ck-suite-nav", css)
        self.assertIn(".suite-sidebar-overlay", css)
        self.assertIn(".ck-suite-piyasa .sidebar", css)
        self.assertIn(".ck-suite-sistem .suite-sidebar nav", css)
        self.assertIn("flex: 0 0 48px !important", css)
        self.assertIn(".ck-suite-piyasa .main-content", css)
        self.assertIn(".ck-suite-piyasa .table-panel tbody tr", css)
        self.assertIn('content: "PANEL / GENEL BAKIŞ"', css)
        self.assertIn(
            "grid-template-columns: repeat(2, minmax(0, 1fr))",
            css,
        )
        self.assertIn(
            "grid-template-columns: 46px minmax(0, 1fr) 46px",
            css,
        )
        self.assertIn("grid-column: 1 / -1 !important", css)
        self.assertIn(
            '.consumption-date-control input[type="date"]',
            css,
        )
        self.assertIn("background: var(--app-surface-soft) !important", css)
        self.assertIn("scroll-snap-type: x mandatory", css)
        self.assertIn(".ck-suite-baraj #excelRefreshButton", css)
        self.assertEqual(headers.get_content_type(), "text/css")

        for path in (
            "/baraj/",
            "/uretim/",
            "/tuketim/",
            "/sistem-yonu-tahmini/",
        ):
            with self.subTest(path=path):
                module_request = urllib.request.Request(
                    self.base_url + path,
                    headers={
                        "Cookie": f"{APP.AUTH.cookie_name}={self.token}",
                        "User-Agent": "Mozilla/5.0 CKEnerjiAndroid/3.0",
                        "Accept": "text/html",
                    },
                )
                with urllib.request.urlopen(module_request, timeout=5) as response:
                    module_html = response.read().decode("utf-8")
                self.assertIn('class="suite-sidebar"', module_html)
                self.assertIn('href="/android-app.css?v=19"', module_html)

    def test_consumption_api_and_xlsx_use_shared_epias_session(self):
        fixture = {
            "date": "2026-07-20",
            "source": "EPİAŞ Şeffaflık Platformu",
            "updatedAt": "2026-07-20T10:00:00Z",
            "summary": {
                "latest": 32_500,
                "latestHour": "01:00",
                "average": 31_750,
                "maximum": 32_500,
                "maximumHour": "01:00",
                "minimum": 31_000,
                "minimumHour": "00:00",
                "total": 63_500,
                "availableHours": 2,
                "missingHours": 22,
            },
            "rows": [
                {"hour": 0, "time": "00:00", "consumption": 31_000},
                {"hour": 1, "time": "01:00", "consumption": 32_500},
            ],
        }
        forecast_fixture = {
            "baseDate": "2026-07-20",
            "date": "2026-07-21",
            "summary": {"forecastHours": 24, "trainingDays": 14},
            "rows": [{"hour": 0, "time": "00:00", "forecast": 32_000}],
        }
        original = APP._consumption_dashboard
        original_forecast = APP._consumption_forecast
        forecast_calls = []
        APP._consumption_dashboard = lambda *_args, **_kwargs: fixture
        APP._consumption_forecast = (
            lambda *args, **kwargs: forecast_calls.append((args, kwargs))
            or forecast_fixture
        )
        try:
            status, content, _ = self.get(
                "/tuketim/api/data?date=2026-07-20"
            )
            payload = json.loads(content)
            self.assertEqual(status, 200)
            self.assertEqual(payload["summary"]["latest"], 32_500)

            status, content, _ = self.get(
                "/tuketim/api/forecast?baseDate=2026-07-20&targetDate=2026-07-20"
            )
            forecast_payload = json.loads(content)
            self.assertEqual(status, 200)
            self.assertEqual(forecast_payload["summary"]["forecastHours"], 24)
            self.assertEqual(
                forecast_calls[-1][1]["target_date"],
                "2026-07-20",
            )

            status, content, headers = self.get(
                "/tuketim/api/export.xlsx?date=2026-07-20"
            )
        finally:
            APP._consumption_dashboard = original
            APP._consumption_forecast = original_forecast

        self.assertEqual(status, 200)
        self.assertIn(
            "ck-enerji-tuketim-2026-07-20.xlsx",
            headers["Content-Disposition"],
        )
        self.assertTrue(content.startswith(b"PK"))

    def test_uretim_script_paths_are_mounted_under_uretim(self):
        status, content, _ = self.get("/uretim/app.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn('fetch(`/uretim/api/dashboard?', script)
        self.assertIn('window.location.replace("/login")', script)
        self.assertNotIn('/uretim/>', script)
        self.assertNotIn("Kaynak bazında UEÇM yayımlamıyor", script)
        self.assertNotIn("Yayımlanmıyor</td>", script)
        self.assertIn("dataAlertClose", script)
        self.assertIn('addEventListener("click", hideDataAlert)', script)
        self.assertIn("const sortedGroups = [...groups].sort", script)
        self.assertIn("elements.mixLegend.innerHTML = sortedGroups", script)
        self.assertLess(
            script.index("<td>${groupLabels[source.group]}</td>"),
            script.index('<td><span class="source-name">'),
        )
        self.assertIn("constrainDateRange", script)
        self.assertIn("const currentMonthStart = new Date", script)
        self.assertIn("new Date(end.getFullYear(), end.getMonth(), 1, 12)", script)
        self.assertIn("elements.start.max = today", script)
        self.assertIn("elements.end.max = today", script)
        self.assertNotIn("${kpi.index}", script)
        self.assertIn("clearDashboard", script)
        self.assertIn('window.location.replace("/oturum-kapatildi")', script)
        self.assertIn('publishConnectionState("error"', script)
        self.assertIn("data.period.comparableHours < data.period.hours", script)
        self.assertNotIn("EPİAŞ eksik veri", script)
        self.assertIn(
            'elements.status.querySelector("span").textContent = "EPİAŞ canlı"',
            script,
        )
        self.assertNotIn("CKDateSync", script)
        self.assertIn("window.CKTracking?.publish", script)
        self.assertNotIn("elements.presets", script)
        self.assertNotIn("quickReportRange", script)
        self.assertNotIn("dailyReportButton", script)

    def test_uretim_page_uses_equalized_logo_size(self):
        status, content, _ = self.get("/uretim/")
        html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("<title>CK Enerji Üretim Paneli</title>", html)
        self.assertIn('class="ck-suite-page ck-suite-uretim"', html)
        self.assertIn('src="/suite-assets/ck-mark.png?v=3"', html)
        self.assertIn('class="suite-sidebar"', html)
        self.assertIn("data-suite-theme-toggle", html)
        self.assertIn("EPİAŞ · EPİAŞ canlı", html)
        self.assertIn('class="suite-menu-close"', html)
        self.assertIn("UEVM &amp; UEÇM Üretim Özeti", html)
        self.assertIn('id="dataAlertClose"', html)
        self.assertIn('aria-label="Uyarıyı kapat"', html)
        self.assertIn(
            '<div class="eyebrow">PANEL / GENEL BAKIŞ</div>',
            html,
        )
        self.assertLess(
            html.index('class="eyebrow"'),
            html.index("UEVM &amp; UEÇM Üretim Özeti"),
        )
        self.assertNotIn('<th class="numeric">Fark</th>', html)
        self.assertNotIn(
            'title="Kaynak bazında yayımlanmıyor"',
            html,
        )
        self.assertLess(html.index("<th>Grup</th>"), html.index("<th>Kaynak</th>"))
        self.assertIn('href="/module-suite.css?v=64"', html)
        self.assertNotIn("07 / SAATLİK DENGE", html)
        self.assertNotIn("03 / DENGE", html)
        self.assertNotIn('id="balanceVisual"', html)
        self.assertNotIn("Günlük saatlik denge özeti", html)
        self.assertNotIn('id="dailySummaryTable"', html)
        self.assertNotIn("08 / SAATLIK ÖZET", html)
        self.assertNotIn("Saatlik UEVM − UEÇM özet", html)
        self.assertNotIn("09 / HIZLI RAPORLAR", html)
        self.assertNotIn('id="dailyReportButton"', html)
        self.assertNotIn('id="monthlyReportButton"', html)
        self.assertNotIn('data-range="30"', html)
        self.assertNotIn("Son 30 g\u00fcn", html)
        self.assertIn('src="/module-suite.js?v=17"', html)
        self.assertIn('src="/theme-sync.js?v=4"', html)
        self.assertIn('src="/uretim/app.js?v=19"', html)
        self.assertIn('href="/uretim/manifest.webmanifest"', html)
        self.assertIn(
            'href="/suite-assets/apple-touch-icon.png?v=7"',
            html,
        )
        self.assertIn(
            'class="suite-footer" data-suite-footer="uretim"',
            html,
        )

        status, content, _ = self.get("/uretim/app.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("DASHBOARD_CACHE_KEY", script)
        self.assertIn("readDashboardCache", script)
        self.assertIn("sessionStorage.setItem", script)
        self.assertIn("normalizeGroupShares", script)

        status, content, _ = self.get("/uretim/manifest.webmanifest")
        manifest = json.loads(content)
        self.assertEqual(status, 200)
        self.assertEqual(
            [icon["src"] for icon in manifest["icons"]],
            [
                "/suite-assets/icon-192.png?v=7",
                "/suite-assets/icon-512.png?v=7",
            ],
        )
        self.assertIn(
            '<img src="/suite-assets/ck-logo-white.png?v=1" alt="CK Enerji - Her an sizinle" />',
            html,
        )
        footer = html.split(
            'class="suite-footer" data-suite-footer="uretim"',
            1,
        )[1].split("</footer>", 1)[0]
        self.assertNotIn("CK ÜRETİM", footer)
        sidebar = html.split('<aside class="suite-sidebar">', 1)[1].split(
            "</aside>", 1
        )[0]
        self.assertNotIn("<small>", sidebar)
        self.assertNotIn(
            'src="/uretim/assets/ck-uretim-logo.svg"',
            html,
        )

        status, content, _ = self.get("/portal-shell.css")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".ck-suite-uretim .topbar .brand-mark", css)
        self.assertIn("body.ck-suite-page .suite-footer", css)
        self.assertIn("margin: 48px 0 0 252px", css)
        self.assertIn("grid-template-columns: 1fr", css)
        self.assertIn("width: 54px", css)
        self.assertIn("padding: 5px", css)
        self.assertIn("transform: none", css)
        self.assertIn("text-decoration-line: underline", css)
        self.assertIn("suite-motion-enabled", css)
        self.assertIn("suiteSectionArrive", css)
        self.assertIn(".ck-suite-nav a.suite-nav-forecast", css)
        self.assertNotIn(".suite-nav-short", css)
        self.assertNotIn(".suite-tracking-drawer", css)
        self.assertNotIn(".suite-tracking-toggle", css)

        status, content, headers = self.get("/suite-assets/ck-logo.png")
        self.assertEqual(status, 200)
        self.assertEqual(headers.get_content_type(), "image/png")
        self.assertTrue(content.startswith(b"\x89PNG\r\n\x1a\n"))
        self.assertGreater(len(content), 10_000)
        self.assertLess(len(content), 200_000)

        status, content, headers = self.get("/suite-assets/ck-mark.png")
        self.assertEqual(status, 200)
        self.assertEqual(headers.get_content_type(), "image/png")
        self.assertTrue(content.startswith(b"\x89PNG\r\n\x1a\n"))
        self.assertGreater(len(content), 5_000)
        self.assertLess(len(content), 100_000)

    def test_shared_module_theme_assets_are_served(self):
        status, content, headers = self.get("/module-suite.css")
        css = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn(".suite-sidebar", css)
        self.assertIn(".ck-suite-sistem .suite-sidebar *", css)
        self.assertIn("flex: 0 0 48px !important", css)
        self.assertIn("overflow: hidden !important", css)
        self.assertIn(".suite-sidebar-lock-button", css)
        self.assertIn(".sidebar-lock-button", css)
        self.assertIn(
            "@media (min-width: 1025px) and (hover: hover) and (pointer: fine)",
            css,
        )
        self.assertIn("body.suite-sidebar-pinned .suite-sidebar-lock-button", css)
        self.assertIn("body.suite-sidebar-pinned .sidebar-lock-button", css)
        self.assertIn(".ck-suite-baraj #baraj-summary", css)
        self.assertIn(".ck-suite-baraj .baraj-basin-toolbar", css)
        self.assertIn(".ck-suite-baraj .baraj-map-card", css)
        self.assertIn(".ck-suite-baraj .baraj-map-legend", css)
        self.assertIn(".ck-suite-baraj .baraj-basin-feature.active", css)
        self.assertIn(".ck-suite-baraj .baraj-map-legend .fullness-low", css)
        self.assertIn(".ck-suite-baraj .baraj-map-legend .fullness-mid", css)
        self.assertIn(".ck-suite-baraj .baraj-map-legend .fullness-high", css)
        self.assertIn(".ck-suite-baraj .baraj-basin-feature.fullness-unknown", css)
        self.assertIn("fill: var(--basin-fill", css)
        self.assertIn(".ck-suite-baraj .baraj-basin-shape", css)
        self.assertIn(".ck-suite-baraj .baraj-basin-map-label", css)
        self.assertIn(".ck-suite-baraj .baraj-map-dams", css)
        self.assertIn(".ck-suite-baraj .baraj-basin-detail", css)
        self.assertIn(".ck-suite-baraj .baraj-basin-table", css)
        self.assertIn(".ck-suite-baraj .baraj-dam-history-card", css)
        self.assertIn(".ck-suite-baraj #basinDamSelect", css)
        self.assertIn(".ck-suite-baraj .baraj-risk-table td.neutral", css)
        self.assertIn("repeat(auto-fit, minmax(160px, 1fr))", css)
        self.assertIn(".ck-suite-baraj #baraj-compare", css)
        self.assertIn(".ck-suite-baraj .baraj-compare-results", css)
        self.assertIn(".ck-suite-baraj .baraj-compare-difference", css)
        self.assertIn(
            'html[data-theme="dark"] .ck-suite-baraj .baraj-basin-detail',
            css,
        )
        self.assertIn(
            ".ck-suite-baraj .baraj-basin-select-wrap::after",
            css,
        )
        self.assertIn("--tblr-table-striped-bg: #121e33", css)
        self.assertIn("background-color: #121e33 !important", css)
        self.assertIn(
            ".ck-suite-uretim .hero-grid > div:first-child",
            css,
        )
        self.assertIn('"start end"', css)
        self.assertIn('"submit submit"', css)
        self.assertIn(".date-form label:nth-of-type(2)", css)
        self.assertIn("grid-area: submit", css)
        self.assertIn("@media (min-width: 740px) and (max-width: 1220px)", css)
        self.assertIn("grid-template-columns: minmax(270px, .85fr)", css)
        self.assertIn("width: clamp(270px, 30vw, 330px)", css)
        self.assertIn(".ck-suite-uretim .mix-panel .donut-layout", css)
        self.assertIn(".ck-suite-uretim .balance-panel .balance-copy", css)
        self.assertIn(
            'html[data-theme="dark"] .ck-suite-uretim .balance-panel',
            css,
        )
        self.assertIn("background: #fff", css)
        self.assertIn(
            'html[data-theme="dark"] .ck-suite-uretim .balance-panel {\n'
            "  border-color: #293850;\n"
            "  color: #fff;\n"
            "  background: #121e33;\n"
            "}",
            css,
        )
        self.assertIn("max-width: 520px", css)
        self.assertIn("min-height: 58px", css)
        self.assertIn("flex: 1 1 auto", css)
        self.assertIn("grid-template-rows: repeat(4, minmax(58px, 1fr))", css)
        self.assertIn("overflow-wrap: normal", css)
        self.assertIn("word-break: normal", css)
        self.assertIn("#barajDateSelect", css)
        self.assertIn(".ck-suite-baraj #excelRefreshButton", css)
        self.assertIn('input[type="date"]::-webkit-calendar-picker-indicator', css)
        self.assertIn("border-color: #47658f", css)
        self.assertIn('html[data-theme="dark"]', css)
        self.assertIn('grid-template-areas:', css)
        self.assertIn('"components share"', css)
        self.assertIn(".suite-live-dot.warning i", css)
        self.assertIn(".source-card:nth-child(1) .source-icon", css)
        self.assertIn("rgba(218, 157, 35, .16)", css)
        self.assertIn(".source-share", css)
        self.assertNotIn(".ck-suite-uretim .preset", css)
        self.assertEqual(headers.get_content_type(), "text/css")

        status, content, headers = self.get("/module-suite.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("suite-sidebar-open", script)
        self.assertIn("suite-sidebar-collapsed", script)
        self.assertIn("suite-sidebar-hovered", script)
        self.assertIn("suite-sidebar-dismissed", script)
        self.assertIn("suite-sidebar-pinned", script)
        self.assertIn("setDesktopSidebarHover", script)
        self.assertIn("setSidebarPinned", script)
        self.assertIn("#sidebar-lock", script)
        self.assertIn("desktopLock", script)
        self.assertIn("canPinSidebar", script)
        self.assertIn(
            'window.matchMedia("(min-width: 1025px) and (hover: hover) and (pointer: fine)")',
            script,
        )
        self.assertIn("desktopPointerInside", script)
        self.assertIn('sidebar.addEventListener("mouseenter"', script)
        self.assertIn('"ck-sidebar-collapsed"', script)
        self.assertIn('"ck-sidebar-pinned"', script)
        self.assertIn('window.addEventListener("storage"', script)
        self.assertIn('window.matchMedia("(min-width: 821px)")', script)
        self.assertIn('fetch("/api/session"', script)
        self.assertIn("EPİAŞ · EPİAŞ canlı", script)
        self.assertIn("ck:connectionstate", script)
        self.assertIn('body.dataset.epiasState', script)
        self.assertIn("EPİAŞ · Eksik veri", script)
        self.assertIn("suppressUretimWarning", script)
        self.assertIn("navigationLockUntil", script)
        self.assertIn("getBoundingClientRect", script)
        self.assertIn("const positions = sections.map", script)
        self.assertIn("position.top > nearest.top", script)
        self.assertIn("currentPosition.section.link", script)
        self.assertNotIn("sections[sections.length - 1]", script)
        self.assertIn("suite-section-arriving", script)
        self.assertIn('window.location.replace("/oturum-kapatildi")', script)
        self.assertNotIn("IntersectionObserver", script)
        self.assertEqual(headers.get_content_type(), "text/javascript")

        status, content, headers = self.get("/theme-sync.js")
        script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn('const STORAGE_KEY = "ck-theme"', script)
        self.assertIn('window.addEventListener("storage"', script)
        self.assertIn("window.CKTheme", script)
        self.assertIn("suite-page-leaving", script)
        self.assertIn("connectPageMotion", script)
        self.assertIn("connectRoutePrefetching", script)
        self.assertIn('prefetch.rel = "prefetch"', script)
        self.assertNotIn("window.setTimeout(() => window.location.assign(target.href), 190)", script)
        self.assertEqual(headers.get_content_type(), "text/javascript")


    def test_tv_mode_and_command_center_assets_are_served(self):
        status, content, headers = self.get("/tv/")
        html = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("ENERJİ KOMUTA MERKEZİ", html)
        self.assertEqual(html.count('data-slide="'), 6)
        self.assertIn('id="tvFullscreen"', html)
        self.assertIn('href="tv.css?v=26"', html)
        self.assertIn('src="tv-settings.js?v=17"', html)
        self.assertIn('src="tv.js?v=25"', html)
        self.assertIn('id="tvRefreshMarket"', html)
        self.assertIn('id="tvPause" type="button" aria-pressed="false" disabled', html)
        self.assertIn('id="tvProfile"', html)
        self.assertIn('value="Varsayılan" selected', html)
        self.assertIn('id="tvProfileName"', html)
        self.assertIn('id="tvProfileCreator"', html)
        self.assertIn('id="tvProfileAdd"', html)
        self.assertIn('id="tvFixedDashboard"', html)
        self.assertIn('Görüntüleme biçimi', html)
        self.assertIn('name="tvMode" value="rotate"', html)
        self.assertIn('name="tvMode" value="fixed"', html)
        self.assertIn('id="tvSettingsStatus"', html)
        self.assertIn('id="tvAlertPanel"', html)
        self.assertNotIn('<strong id="tvDeviation">—</strong><small>%</small>', html)
        self.assertNotIn('<strong id="tvDamAverage">—</strong><small>%</small>', html)
        self.assertIn('class="tv-side-kpis tv-market-kpis"', html)
        self.assertIn('class="tv-side-kpis tv-consumption-kpis"', html)
        self.assertIn('name="tvSlide" value="currency"', html)
        self.assertIn('id="tvCurrencySource"', html)
        self.assertEqual(headers.get_content_type(), "text/html")

        status, content, headers = self.get("/tv/tv.js")
        script = content.decode("utf-8")
        self.assertIn('setText("tvDeviation", fmtPercent(', script)
        self.assertIn('setText("tvDamAverage", Number.isFinite', script)
        self.assertEqual(status, 200)
        self.assertIn("/api/command-center?date=", script)
        self.assertIn("requestFullscreen", script)
        self.assertIn("ROTATE_MS = 15000", script)
        self.assertIn('addEventListener("touchstart"', script)
        self.assertIn('addEventListener("touchend"', script)
        self.assertIn("touchStartX", script)
        self.assertIn("marketFreshness.smfIncomplete", script)
        self.assertIn("scheduleDataRefresh(data)", script)
        self.assertIn("refreshCurrency=1", script)
        self.assertIn("row.smfPublished === false ? null : row.smf", script)
        self.assertIn('addEventListener("visibilitychange"', script)
        self.assertIn("DATA_REUSE_MS = 60000", script)
        self.assertIn("sessionStorage.setItem", script)
        self.assertIn("readCachedReport", script)
        self.assertIn("showAlerts", script)
        self.assertIn("currentAlerts", script)
        self.assertIn("setHeaderActionsReady", script)
        self.assertIn("setHeaderActionsReady(false)", script)
        self.assertIn('key: "currency", label: "Enerji ve Döviz", index: 5', script)
        self.assertIn('renderCurrencyPair("USD"', script)
        self.assertIn('renderCommodity("BRENT"', script)
        self.assertIn('renderInvestingCurrency("USD"', script)
        self.assertEqual(headers.get_content_type(), "text/javascript")

        status, content, headers = self.get("/tv/tv.css")
        stylesheet = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("@media(max-width:600px)", stylesheet)
        self.assertIn("100dvh", stylesheet)
        self.assertIn(".tv-market-kpis", stylesheet)
        self.assertIn(".tv-consumption-kpis", stylesheet)
        self.assertIn(".tv-alert-panel", stylesheet)
        self.assertIn(".tv-currency-layout", stylesheet)
        self.assertEqual(headers.get_content_type(), "text/css")

        status, content, _ = self.get("/command-center.js")
        command_script = content.decode("utf-8")
        self.assertEqual(status, 200)
        self.assertIn("data-suite-report-link", command_script)
        self.assertIn("/rapor?date=", command_script)
        self.assertIn("suite-command-toggle", command_script)
        self.assertIn("setCommandMenu", command_script)
        self.assertIn("aria-expanded", command_script)

    def test_executive_report_http_page_and_xlsx_endpoint(self):
        fake_report = {
            "date": "2026-07-22",
            "generatedAt": "2026-07-22T14:20:00+03:00",
            "modules": {},
            "damSummary": {},
            "errors": {"market": "Örnek uyarı"},
            "availableModules": [],
        }
        original = APP._executive_dashboard
        dashboard_options = []
        APP._executive_dashboard = lambda selected_date, client, **kwargs: dashboard_options.append(kwargs) or {
            **fake_report,
            "date": selected_date,
        }
        try:
            status, content, headers = self.get(
                "/rapor?date=2026-07-22&print=1"
            )
            html = content.decode("utf-8")
            self.assertEqual(status, 200)
            self.assertIn("GÜNLÜK YÖNETİCİ RAPORU", html)
            self.assertIn('data-auto-print="true"', html)
            self.assertIn("Takvim kıyasını yükle", html)
            self.assertEqual(headers.get_content_type(), "text/html")

            status, content, headers = self.get(
                "/api/executive-report.xlsx?date=2026-07-22"
            )
            self.assertEqual(status, 200)
            self.assertTrue(content.startswith(b"PK"))
            self.assertEqual(
                dashboard_options,
                [{"include_comparisons": False}, {}],
            )
            self.assertIn(
                "ck-enerji-yonetici-raporu-2026-07-22.xlsx",
                headers.get("Content-Disposition", ""),
            )
        finally:
            APP._executive_dashboard = original


if __name__ == "__main__":
    unittest.main()
