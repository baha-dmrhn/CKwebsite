...
class SuiteHelpersTests(unittest.TestCase):
    ...
    def test_background_refresh_defaults_use_safe_intervals(self):
        ...
        self.assertEqual(snapshot["barajSchedule"]["hours"], [17, 18])
        self.assertEqual(snapshot["barajSchedule"]["startHour"], 17)
        self.assertEqual(snapshot["barajSchedule"]["endHour"], 18)
        timezone = APP.URETIM.TR_TZ
        self.assertIsNone(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 16, 59, tzinfo=timezone))
        )
        self.assertEqual(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 17, 0, tzinfo=timezone)),
            "2026-08-05T17",
        )
        self.assertEqual(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 18, 59, tzinfo=timezone)),
            "2026-08-05T18",
        )
        self.assertIsNone(
            service._baraj_schedule_slot(datetime(2026, 8, 5, 19, 0, tzinfo=timezone))
        )

    def test_background_refresh_baraj_hours_can_be_configured(self):
        with mock.patch.dict(
            os.environ,
            {"CK_BG_BARAJ_HOURS": "16, 18, 18, geçersiz, 25"},
        ):
            service = APP.BackgroundRefreshService()

        self.assertEqual(service.snapshot()["barajSchedule"]["hours"], [16, 18])
    ...
    def test_successful_login_queues_immediate_baraj_refresh(self):
        ...
        with (
            ...
            worker = service.queue_baraj_refresh_after_login(
                "user@ck.local",
                "TGT-login-refresh",
            )
    ...
    def test_simultaneous_logins_share_one_baraj_refresh(self):
        ...
        with (
            ...
            first = service.queue_baraj_refresh_after_login("first", "TGT-first")
            self.assertTrue(started.wait(timeout=2))
            second = service.queue_baraj_refresh_after_login("second", "TGT-second")
            release.set()
...
class SuiteHttpTests(unittest.TestCase):
        ...
        try:
            with (
                ...
                mock.patch.object(
                    ...
                    "queue_baraj_refresh_after_login",
                ) as queue_refresh,
            ):
                connection.request(
                    "POST",
    ...
    def test_baraj_inline_api_paths_are_mounted_under_baraj(self):
        ...
        self.assertIn('src="/module-suite.js?v=16"', html)
        self.assertIn('src="/theme-sync.js?v=4"', html)
