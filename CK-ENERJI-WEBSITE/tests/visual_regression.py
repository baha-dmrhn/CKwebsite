...
if ROOT.name != "CK-ENERJI-WEBSITE":
    raise RuntimeError(
        "Görsel test yalnızca CK-ENERJI-WEBSITE projesinden çalıştırılabilir."
    )
SPEC = importlib.util.spec_from_file_location("ck_visual_app", ROOT / "app.py")
...
def command_center_payload() -> dict:
    ...
    currency = {
        ...
        "sourceUrl": "https://tr.investing.com/currencies/streaming-forex-rates-majors",
        "updatedAt": "2026-07-20T14:00:00Z",
        "availablePairs": 2,
        "available": True,
        "fallback": False,
        "stale": False,
        "pairs": [
            {"symbol": "USD/TRY", "base": "USD", "label": "Amerikan Doları", "value": 40.2512, "change": 0.0821, "changePercent": 0.21, "previousClose": 40.1691, "direction": "up", "quoteTimeLabel": "Canlı · 14:00", "source": "Investing.com"},
            {"symbol": "EUR/TRY", "base": "EUR", "label": "Euro", "value": 46.8034, "change": -0.0412, "changePercent": -0.09, "previousClose": 46.8446, "direction": "down", "quoteTimeLabel": "Canlı · 14:00", "source": "Investing.com"},
        ],
    }
    ...
    return {
        ...
        "errors": {},
        "availableModules": ["market", "nextDayPtf", "dams", "production", "consumption", "currency"],
    }
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/__visual_boot":
            ...
            script = (
                ...
                f"localStorage.setItem('ck-theme',{json.dumps(theme)});"
...
SCENARIOS = (
    ...
    ("login-desktop", "/login", "light", 1365, 768, False),
)
...
def remove_browser_profile(profile: Path) -> None:
    """Remove a disposable browser profile after file handles settle."""
...
def main() -> int:
    ...
    token = APP.AUTH.create_session("visual@ck.local", "TGT-visual")
    ...
    try:
        ...
        for name, target, theme, width, height, authenticated in scenarios:
            ...
            # Edge'in profil dosyalarÄ± test durdurulursa kilitli kalabilir.
            # OnlarÄ± proje dizininde deÄŸil sistem geÃ§ici alanÄ±nda tutuyoruz.
            temporary_directory = tempfile.mkdtemp(prefix=f"ck-visual-{name}-")
