...
def _replace_raw_day(
    ...
    matching_rows = [
        ...
        if _day(worksheet.cell(row, 1).value) == selected_day
    ]
    for row in reversed(matching_rows):
        worksheet.delete_rows(row)
    _append_raw_rows(worksheet, rows)
...
def _replace_pivot_day(
    ...
    date_column = next(
        (
            column
            for column in _pivot_date_columns(worksheet, header_row)
            if _day(worksheet.cell(header_row, column).value) == selected_day
        ),
        None,
    )
    if date_column is None:
        _append_pivot_day(worksheet, selected_day, rows)
        return
    ...
    for dam_key, row in dam_rows.items():
        worksheet.cell(row, date_column).value = value_by_dam.get(dam_key)
    worksheet.cell(total_row, date_column).value = _average(value_by_dam.values())

    date_columns = _pivot_date_columns(worksheet, header_row)
    all_values: list[float] = []
    for row in range(header_row + 1, total_row):
        values = [worksheet.cell(row, column).value for column in date_columns]
        worksheet.cell(row, grand_average_column).value = _average(values)
        all_values.extend(
            number for value in values if (number := _number(value)) is not None
        )
    worksheet.cell(total_row, grand_average_column).value = _average(all_values)
...
def refresh_active_fullness_day(
    workbook_path: str | Path,
    items: Iterable[dict[str, Any]],
    *,
    minimum_records: int = 50,
    ...
    """Son yayımlanan EPİAŞ gününü Excel'de ekler veya yerinde yeniler.

    Manuel yenileme yalnızca arşivdeki son günle aynı ya da ondan yeni bir günü
    işleyebilir. Böylece eski geçmiş günler kullanıcı işlemiyle değiştirilemez.
    """
    ...
    selected_day = max(normalized)
    rows = normalized[selected_day]
    if len(rows) < max(1, minimum_records):
        return ArchiveUpdateResult(
            False,
            reason=(
                f"{selected_day.isoformat()} günü yalnızca {len(rows)} geçerli "
                "kayıt içeriyor; Excel değiştirilmedi."
            ),
        )
    ...
    with _ARCHIVE_WRITE_LOCK:
        ...
        try:
            ...
            existing_dates = _existing_raw_dates(raw_sheet)
            latest_existing = max(existing_dates) if existing_dates else date.min
            if selected_day < latest_existing:
                return ArchiveUpdateResult(
                    False,
                    reason=(
                        "EPİAŞ'ın son yayımlanan günü Excel arşivinin son "
                        "tarihinden eski; geçmiş kayıtlar korunuyor."
                    ),
                )

            replacing = selected_day in existing_dates
            if replacing:
                _replace_raw_day(raw_sheet, selected_day, rows)
            else:
                _append_raw_rows(raw_sheet, rows)
            _replace_pivot_day(pivot_sheet, selected_day, rows)
            _refresh_pivot_metadata(workbook, raw_sheet, pivot_sheet)
            ...
            selected_date_text = selected_day.isoformat()
            return ArchiveUpdateResult(
                True,
                added_dates=() if replacing else (selected_date_text,),
                added_rows=0 if replacing else len(rows),
                refreshed_dates=(selected_date_text,) if replacing else (),
                refreshed_rows=len(rows) if replacing else 0,
                reason=(
                    "EPİAŞ günü Excel arşivinde yenilendi."
                    if replacing
                    else "EPİAŞ günü Excel arşivine eklendi."
                ),
            )
        finally:
            workbook.close()
