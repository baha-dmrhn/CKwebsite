...
Her başarılı site girişinden hemen sonra, kullanıcının geçici EPİAŞ TGT oturumu
ile Baraj Aktif verisi arka planda kontrol edilir; giriş yanıtı bu işlem için
bekletilmez. Çok kısa aralıkta eşzamanlı girişler EPİAŞ sınırlarını korumak için
tek istekte birleştirilir. Ayrıca aktif bir kullanıcı EPİAŞ oturumu varken
Türkiye saatiyle 17.00 ve 18.00 saatlerinde birer güvence kontrolü yapılır.
Saatler virgülle ayrılmış `CK_BG_BARAJ_HOURS` ortam değişkeniyle değiştirilebilir;
giriş kontrolü gerekirse `CK_BARAJ_REFRESH_ON_LOGIN=false` ile kapatılabilir.
