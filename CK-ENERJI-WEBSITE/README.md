...
- `CK_FORECAST_LEDGER_ENABLED`: varsayılan `true`
- `CK_FORECAST_LEDGER_PATH`: varsayılan
  `CK-ENERJI-WEBSITE/.forecast-ledger/system-direction.sqlite3`
...
`CK_FORECAST_LEDGER_PATH` kalıcı bir disk yoluna verilmelidir. Kalıcı disk
...
```powershell
python .\CK-ENERJI-WEBSITE\app.py
```
...
```powershell
python .\CK-ENERJI-WEBSITE\app.py --port 8080
```
...
`CK_LOGIN_MAX_ATTEMPTS`, `CK_LOGIN_WINDOW_SECONDS` ve
`CK_LOGIN_BLOCK_SECONDS` ortam değişkenleriyle değiştirilebilir.
...
Her başarılı site girişinden ve `/baraj/` ana sayfasının her açılışından hemen
sonra, kullanıcının geçici EPİAŞ TGT oturumu ile Baraj Aktif verisi arka planda
kontrol edilir; sayfa yanıtı bu işlem için bekletilmez. Çok kısa aralıkta
eşzamanlı girişler ve sayfa açılışları EPİAŞ sınırlarını korumak için tek istekte
birleştirilir. Baraj Aktif için saat bazlı veya kullanıcıdan bağımsız bir görev
yoktur; veri kullanıcının eriştiği saatin güncel EPİAŞ cevabından alınır.
Giriş/sayfa kontrolü gerekirse `CK_BARAJ_REFRESH_ON_ACCESS=false` ile kapatılabilir.
...
`CK_BARAJ_ARCHIVE_AUTO_UPDATE=false` ile kapatılabilir veya tam gün eşiği
`CK_BARAJ_ARCHIVE_MIN_RECORDS` ile değiştirilebilir.

Baraj sayfasındaki `Excel'i güncelle` düğmesi manuel ve yetkili bir istisnadır:
EPİAŞ'ın son yayımlanan gününü yeniden çeker; bu tarih Excel'in son günüyle
aynıysa o güne ait `Aktif Doluluk` satırlarını ve `Pivot` değerlerini yeniler,
daha yeniyse günü ekler. Arşivin son tarihinden eski günler bu düğmeyle de
değiştirilemez ve en az 50 geçerli kayıt güvenlik koşulu korunur.
...
`CK_EPIAS_JSON_CACHE_DIR` kalıcı bir Render disk yoluna verilmelidir. Kalıcı disk
...
Kaynak, EPİAŞ'ın
[`Baraj Aktif Doluluk`](https://seffaflik.epias.com.tr/electricity/dams/active-fullness)
ekranının kullandığı resmî `POST /v1/dams/data/active-fullness` servisidir.
Kontroller sitenin mevcut geçici TGT oturumunu kullanır; kullanıcı adı veya parola
dosyaya yazılmaz. Otomatik erişim kontrolünde Excel'e eklenen gün sonraki
otomatik kontrollerde yeniden yazılmaz; yalnızca yetkili manuel düğme son günü
yenileyebilir.
...
```powershell
python -m unittest discover -s .\CK-ENERJI-WEBSITE\tests -q
...
```powershell
python .\CK-ENERJI-WEBSITE\tests\visual_regression.py --update
```
...
Çalıştırılacak tek sunucu `CK-ENERJI-WEBSITE/app.py` dosyasıdır. Kök dizindeki
