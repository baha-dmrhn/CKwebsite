...
`CK_BG_BARAJ_START_HOUR` ve `CK_BG_BARAJ_END_HOUR` ile değiştirilebilir.
...
### Her gün 18.00 kalıcı Excel güncellemesi

`.github/workflows/baraj-archive.yml` görevi her gün `Europe/Istanbul` saat
dilimiyle 18.00'de EPİAŞ'ın
[`Baraj Aktif Doluluk`](https://seffaflik.epias.com.tr/electricity/dams/active-fullness)
ekranının kullandığı resmî `POST /v1/dams/data/active-fullness` servisini kontrol
eder. Yeni ve en az 50
geçerli baraj içeren gün varsa `Aktif_Doluluk-Delta - Kopya.xlsx` dosyasının
`Aktif Doluluk` ve `Pivot` sekmelerine ekler, yalnızca Excel dosyasını commit
ederek varsayılan dala gönderir. Bu commit mevcut Render bağlantısının yeni
Excel'i otomatik olarak yayımlamasını sağlar. Aynı gün ikinci kez yazılmaz;
yeni tam gün yoksa commit oluşturulmaz.

Görevin kullanıcı oturumundan bağımsız çalışması için GitHub deposunda
`Settings > Secrets and variables > Actions` bölümüne iki Repository Secret
eklenmelidir:

- `EPIAS_USERNAME`: EPİAŞ kullanıcı e-posta adresi
- `EPIAS_PASSWORD`: EPİAŞ parolası

Parola dosyaya, Excel'e veya çalışma günlüğüne yazılmaz. Kurulumdan sonra
`Actions > Günlük EPİAŞ Baraj Arşivi > Run workflow` ile bir kez elle çalıştırıp
ilk sonucu doğrulayabilirsiniz. Aynı işlem yerelde ortam değişkenleri tanımlıyken
şu komutla da denenebilir:

```powershell
python .\CK-ENERJI-WEBSITE\scripts\update_baraj_archive.py
```
