(() => {
  ...
  function apply(theme, announce = true) {
    ...
    if (announce && changed) {
      window.dispatchEvent(
        new CustomEvent("ck:themechange", {
  ...
  function connectPageMotion() {
    ...
    if (!body?.classList.contains("ck-suite-page")) return;

    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => body.classList.add("suite-page-ready"));
    });

    document.querySelectorAll(".ck-suite-nav a").forEach((link) => {
    ...
    document
      .querySelectorAll('.ck-suite-piyasa .sidebar nav a[href^="#"]')
      .forEach((link) => {
  ...
  function connectRoutePrefetching() {
    ...
    document
      .querySelectorAll('.ck-suite-nav a[href], .suite-command-menu a[href]')
      .forEach((link) => {
