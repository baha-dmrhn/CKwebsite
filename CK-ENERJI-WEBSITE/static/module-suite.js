(() => {
  ...
  const sidebarStorageKey = "ck-sidebar-collapsed";
  const sidebarPinnedStorageKey = "ck-sidebar-pinned";
  ...
  async function loadAccount() {
    try {
      ...
      const email = String(payload.username || "CK Enerji Kullanıcısı");
  ...
  function mirrorStatus() {
    ...
    const source = body.classList.contains("ck-suite-uretim")
    ...
    const update = () => {
      ...
      const suppressUretimWarning = body.classList.contains("ck-suite-uretim") && isWarning && !isError;
    ...
    window.addEventListener("ck:connectionstate", update);
  }
  ...
  function trackSections() {
    ...
    function syncActiveSection() {
      ...
      const positions = sections.map((section) => ({
        section,
        top: section.target.getBoundingClientRect().top,
      }));
      const passedSections = positions.filter((position) => position.top <= guideLine);
      let currentPosition = passedSections.reduce(
        (nearest, position) => !nearest || position.top > nearest.top ? position : nearest,
        null,
      );
      if (!currentPosition) {
        currentPosition = positions.reduce(
          (nearest, position) => position.top < nearest.top ? position : nearest,
        );
      }
      ...
      if (pageBottom) {
        currentPosition = positions.reduce(
          (lowest, position) => position.top > lowest.top ? position : lowest,
        );
      }
      setActiveLink(currentPosition.section.link);
    }
