(() => {
  ...
  function activeSettings(preferences) { return preferences.profiles[preferences.activeProfile]; }
  ...
  function currentPreferences() { return normalizePreferences(window.__ckTvPreferences || readPreferences()); }
  ...
  function formPreferences() { const preferences = currentPreferences(); preferences.profiles[preferences.activeProfile] = formSettings(); return preferences; }
  ...
  function bindSettingsPanel() {
    byId("tvSettings")?.addEventListener("click", (event) => { event.preventDefault(); byId("tvSettingsPanel")?.classList.contains("open") ? closePanel() : openPanel(); });
    ...
    byId("tvProfileNew")?.addEventListener("click", async () => { const name = normalizeName(window.prompt("Yeni TV profilinin adı:")); const preferences = formPreferences(); if (!preferences.profiles[name]) preferences.profiles[name] = normalize(formSettings()); preferences.activeProfile = name; fillForm(await persist(preferences)); });
    ...
    byId("tvSettingsSave")?.addEventListener("click", async () => { const button = byId("tvSettingsSave"); if (button) button.disabled = true; fillForm(await persist(formPreferences())); if (button) button.disabled = false; closePanel(); });
