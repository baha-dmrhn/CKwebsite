(() => {
  ...
  const CLIENT_CACHE_PREFIX = "ck-consumption-v1:";
  ...
  function comparisonDate() {
    return $("consumptionCompareDate").value || null;
  }

  function comparisonLabel() {
    const value = comparisonDate();
    return value ? displayDate(value) : "Karşılaştırma tarihi";
  }

  function constrainComparisonDate(announce = false) {
    const input = $("consumptionCompareDate");
    const today = todayTR();
    input.max = today;
    if (!input.value || input.value > today) {
      input.value = shiftedDate($("consumptionDate").value || today, -1);
      if (announce) showAlert("Karşılaştırma tarihi bugünden ileri olamaz.");
    }
  }
  ...
  $("consumptionCompareDate").addEventListener("change", () => {
    constrainComparisonDate(true);
    loadData();
  });
  ...
  window.addEventListener("ck:themechange", () => {
    if (state.data) {
      renderChart();
      renderForecast();
    }
