...
public class MainActivity extends Activity {
    ...
    private static final String APP_USER_AGENT = "CKEnerjiAndroid/3.0";
    private static final String RECOVER_LOADING_SCRIPT =
            "(async function(){"
                    + "try{"
                    ...
                    + "return key.indexOf('ck-enerji-shell-')===0;"
                    + "}).map(function(key){return caches.delete(key);}));"
                    + "}"
                    + "}catch(e){}"
                    + "window.location.replace('/login?ck_ready='+Date.now());"
                    + "})()";
    private static final String CONFIGURE_EMBEDDED_CHROME_SCRIPT =
            "(function(){"
                    ...
                    + "if(head.firstChild){head.insertBefore(viewport,head.firstChild);}"
                    + "else{head.appendChild(viewport);}"
                    + "}"
                    + "viewport.setAttribute('content','width=device-width,initial-scale=1,viewport-fit=cover');"
                    + "document.documentElement.style.setProperty('--ck-webview-width',window.innerWidth+'px');"
                    + "document.documentElement.style.setProperty('--ck-webview-height',window.innerHeight+'px');"
                    + "var id='ck-native-chrome-hide';"
                    ...
                    + ".ck-suite-nav,"
                    + ".suite-menu-button,"
                    + ".ck-suite-piyasa .menu-button"
                    + "{display:none!important;visibility:hidden!important}"
                    + ".suite-sidebar,.ck-suite-piyasa .sidebar"
                    + "{display:flex!important}"
                    + ".ck-suite-sistem .suite-sidebar"
                    + "{padding:34px 20px 24px!important}"
                    + ".ck-suite-sistem .suite-side-brand"
                    + "{min-height:58px!important;gap:13px!important;"
                    + "padding:0 8px 36px!important;font-size:18px!important}"
                    + ".ck-suite-sistem .suite-sidebar nav"
                    ...
                    + ".ck-suite-sistem .suite-sidebar nav a"
                    ...
                    + ".suite-sidebar-overlay,.ck-suite-piyasa .sidebar-overlay"
                    ...
                    + ");"
                    + "var marketSidebar=document.querySelector('.ck-suite-piyasa .sidebar');"
    ...
    private static final String TOGGLE_EMBEDDED_SIDEBAR_SCRIPT =
            "(function(){"
                    ...
                    + "var market=document.querySelector('.ck-suite-piyasa .sidebar');"
                    + "if(market){"
    ...
    private void configureWebView() {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                if (
                        "/panel-hazirlaniyor".equals(finishedUri.getPath())
                        || normalizedTitle.contains("panel hazırlanıyor")
                ) {
                    view.postDelayed(() -> {
                        view.evaluateJavascript(
                                "document.getElementById('panelLoadingMessage')!==null",
                                isLoading -> {
                                    if ("true".equals(isLoading)) {
                                        view.evaluateJavascript(
                                                RECOVER_LOADING_SCRIPT,
                                                ignored -> {}
                                        );
                                    }
                                }
                ...
                if (!normalizedTitle.contains("ck") && !normalizedTitle.contains("ck enerji")) {
    ...
    private DownloadListener downloadListener() {
        return (url, userAgent, contentDisposition, mimeType, contentLength) -> {
            try {
                ...
                request.setDescription("CK Enerji dosyası indiriliyor");
    ...
    private void buildUi() {
        ...
        errorTitle.setText(getString(com.ckenerji.app.R.string.offline_title));
        errorTitle.setTextSize(24);
        errorTitle.setGravity(Gravity.CENTER);
        errorTitle.setTypeface(null, 1);

        errorMessage = new TextView(this);
        errorMessage.setText(getString(com.ckenerji.app.R.string.offline_message));
        errorMessage.setTextSize(15);
        errorMessage.setGravity(Gravity.CENTER);
        errorMessage.setPadding(0, dp(10), 0, dp(18));

        retryButton = new Button(this);
        retryButton.setText(getString(com.ckenerji.app.R.string.retry));
        ...
        splashLogo.setImageResource(com.ckenerji.app.R.drawable.ck_enerji_app_icon);
    ...
    private void applyResponsiveChromeMetrics() {
        Configuration configuration = getResources().getConfiguration();
        int smallestWidth = configuration.smallestScreenWidthDp;
        int screenHeight = configuration.screenHeightDp;
        boolean tablet = smallestWidth >= 600;
        boolean compactLandscape =
                configuration.orientation == Configuration.ORIENTATION_LANDSCAPE
                        && screenHeight > 0
                        && screenHeight < 420;

        int topSpacerHeight = compactLandscape ? 52 : tablet ? 72 : 64;
        int bottomHeight = compactLandscape ? 62 : tablet ? 86 : 78;
        int bottomHorizontalPadding = tablet ? 10 : 4;
        int bottomTopPadding = compactLandscape ? 4 : tablet ? 9 : 7;
        int bottomBottomPadding = compactLandscape ? 3 : tablet ? 7 : 5;
        int iconSize = compactLandscape ? 34 : tablet ? 42 : 38;
        int iconInnerPadding = compactLandscape ? 7 : tablet ? 10 : 9;
        int labelHeight = compactLandscape ? 18 : tablet ? 24 : 22;
        float labelTextSize = compactLandscape ? 9f : tablet ? 11f : 10f;
        int menuButtonSize = compactLandscape ? 44 : tablet ? 52 : 48;
        int menuMargin = tablet ? 18 : 14;
        int menuTopMargin = compactLandscape ? 8 : 12;

        if (appMenuSpacer != null) {
            appMenuSpacer.setLayoutParams(new LinearLayout.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT,
                    dp(topSpacerHeight)
            ));
        }
        if (bottomNavigation != null) {
            bottomNavigation.setPadding(
                    dp(bottomHorizontalPadding),
                    dp(bottomTopPadding),
                    dp(bottomHorizontalPadding),
                    dp(bottomBottomPadding)
            );
        ...
        if (appMenuButton != null) {
            appMenuButton.setTextSize(compactLandscape ? 22 : 24);
            FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(
                    dp(menuButtonSize),
                    dp(menuButtonSize)
            );
            params.gravity = Gravity.TOP | Gravity.START;
            params.leftMargin = dp(menuMargin);
            params.topMargin = dp(menuTopMargin);
            appMenuButton.setLayoutParams(params);
        }
        for (int index = 0; index < bottomItems.length; index++) {
            if (bottomItems[index] != null) {
                bottomItems[index].setPadding(dp(1), compactLandscape ? 0 : dp(2), dp(1), 0);
            }
            if (bottomIcons[index] != null) {
                bottomIcons[index].setPadding(
                        dp(iconInnerPadding),
                        dp(iconInnerPadding),
                        dp(iconInnerPadding),
                        dp(iconInnerPadding)
                );
                bottomIcons[index].setLayoutParams(
                        new LinearLayout.LayoutParams(dp(iconSize), dp(iconSize))
                );
            }
            if (bottomLabels[index] != null) {
                bottomLabels[index].setTextSize(labelTextSize);
                bottomLabels[index].setLayoutParams(
                        new LinearLayout.LayoutParams(
                                ViewGroup.LayoutParams.MATCH_PARENT,
                                dp(labelHeight)
                        )
    ...
    private void applyNativeTheme(boolean dark) {
        ...
        getSharedPreferences("ck_native_preferences", MODE_PRIVATE)
                .edit()
