# CK Enerji Android APK

Bu klasör, CK Enerji web panelini Android telefonda uygulama olarak açan APK projesidir.
...
3. **Build CK Enerji Android APK** workflow'unu aç.
4. Çalışma bittikten sonra **CK-Enerji-APK** artifact'ini indir.
5. ZIP içindeki `CK-Enerji.apk` dosyasını Android telefona gönderip kur.
...
2. `CK-ENERJI-ANDROID` klasörünü seç
...
```powershell
keytool -genkeypair -v -keystore ck-release.jks -alias ck-release -keyalg RSA -keysize 2048 -validity 10000
```

`ck-release.jks` dosyasını güvenli ve yedekli bir yerde saklayın. Ardından
dosyanın Base64 değerini oluşturun:

```powershell
[Convert]::ToBase64String([IO.File]::ReadAllBytes("ck-release.jks"))
```

GitHub deposunda `Settings > Secrets and variables > Actions` bölümüne şu
Repository Secrets değerlerini ekleyin:

- `CK_ANDROID_KEYSTORE_BASE64`: Yukarıdaki Base64 değeri
- `CK_ANDROID_KEYSTORE_PASSWORD`: Keystore parolası
- `CK_ANDROID_KEY_ALIAS`: `ck-release`
- `CK_ANDROID_KEY_PASSWORD`: Anahtar parolası
